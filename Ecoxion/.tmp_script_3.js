    ...
    const ECOXION_PLAN_CONFIG_TTL_MS = 2 * 60 * 1000;
    let ecoxionPlanConfigCache = { ts: 0, config: null };
    ...
    const DEFAULT_STATE = Object.freeze({
      ...
      ecoxionMiniGamePlan: 'free',
      ecoxionMiniGamePlanSyncedAt: 0
    });

    function normalizeState(raw) {
      const incoming = (raw && typeof raw === 'object') ? raw : {};
      const allowedPlans = ['free', 'plus', 'pro', 'ultra'];
      const safeSubscriptionPlan = normalizeEcoxionPlanId(incoming.subscriptionPlan || 'free');
      const safeMiniGamePlan = normalizeEcoxionPlanId(incoming.ecoxionMiniGamePlan || safeSubscriptionPlan || 'free');
      return {
        ...DEFAULT_STATE,
        ...incoming,
        ...
        subscriptionPlan: allowedPlans.includes(safeSubscriptionPlan) ? safeSubscriptionPlan : 'free',
        ecoxionMiniGamePlan: allowedPlans.includes(safeMiniGamePlan) ? safeMiniGamePlan : 'free',
        ecoxionMiniGamePlanSyncedAt: Number(incoming.ecoxionMiniGamePlanSyncedAt || 0)
      };
    }
    ...
    const defaultExtensions = {
      "notes": {
        ...
        <div id="notes-panel">
          ${isV2 ? `
            <div style="margin-bottom: 16px;padding:12px;background:linear-gradient(135deg, rgba(100,167,255,.1), rgba(179,136,255,.1));border-radius:8px;border:1px solid var(--accent);margin-bottom:16px;">
              ...
              <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">
                <input type="text" id="note-search" placeholder="ðŸ” Buscar notas..." style="flex:1;min-width:200px;padding:8px;border-radius:6px;background:var(--bg-1);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);">
                <input type="text" id="note-tags" placeholder="ðŸ·ï¸ Etiquetas (separadas por coma)" style="flex:1;min-width:200px;padding:8px;border-radius:6px;background:var(--bg-1);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);">
              </div>
            </div>
          ` : `
            <div style="margin-bottom:16px;padding:12px;background:rgba(255,215,0,.1);border-radius:8px;border:1px solid rgba(255,215,0,.3);text-align:center;">
              <p style="margin:0 0 12px;color:var(--warn);font-size:13px;">✨ <strong>V2 disponible</strong> - Despierta esta extensión para funciones avanzadas</p>
          ...
          <div style="margin-bottom: 16px;">
            ...
            ${isV2 ? `<button class="btn" onclick="exportNotes()" style="margin-top:8px;margin-left:8px;">ðŸ“„ Exportar PDF</button>` : ''}
          </div>
      ...
      "blog-creator": {
        ...
        <div id="blog-creator" style="padding:20px; color:var(--ink-1);">
          <div class="card" style="margin-bottom:24px; background:rgba(255,255,255,0.02); border-color:rgba(100,167,255,0.2);">
            <h3 style="margin:0 0 16px; color:var(--accent); font-size:18px;">âœï¸ Crear Nueva Entrada</h3>
            <div style="display:flex; flex-direction:column; gap:12px;">
              <input type="text" id="blog-title" placeholder="Título impactante..." style="width:100%; padding:14px; border-radius:12px; background:var(--bg-2); border:1px solid rgba(255,255,255,0.08); color:var(--ink-0); font-size:16px; font-weight:600;">
              <textarea id="blog-content" placeholder="Escribe el contenido de tu blog aquí..." style="width:100%; padding:14px; border-radius:12px; background:var(--bg-2); border:1px solid rgba(255,255,255,0.08); color:var(--ink-0); min-height:180px; resize:vertical; font-family:inherit;"></textarea>
      ...
      "clicky-coin": {
        ...
        icon: '<span class="icon-ecoxionum"></span>',
        price: 0,
        ...
        <div id="clicky-coin-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:20px;background:linear-gradient(135deg, rgba(255,215,0,.1), rgba(255,182,72,.1));border-radius:12px;border:1px solid rgba(255,215,0,.3);text-align:center;">
            <h4 style="margin:0 0 12px;color:#FFD700;display:flex;align-items:center;justify-content:center;gap:8px;">
              <span><span class="icon-ecoxionum"></span></span>
            ...
            <div style="margin-bottom:16px;">
              <div id="clicky-coin-button" style="width: 120px; height: 120px; margin: 0px auto; background: linear-gradient(135deg, rgb(255, 215, 0), rgb(255, 165, 0)); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 48px; cursor: pointer; box-shadow: rgba(255, 215, 0, 0.4) 0px 4px 20px; transition: 0.2s; user-select: none; position: relative; overflow: hidden; opacity: 1;" onclick="clickCoin()" onmousedown="this.style.transform='scale(0.95)'" onmouseup="this.style.transform='scale(1)'" onmouseleave="this.style.transform='scale(1)'">ðŸª™</div>
            ...
            <div style="padding:12px;background:rgba(255,215,0,.1);border-radius:8px;border:1px solid rgba(255,215,0,.2);margin-top:16px;">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                ...
                <span id="earned-today" style="color:#FFD700;font-weight:600;font-size:16px;">0 ðŸª™</span>
              </div>
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
              ...
              <div style="display:flex;justify-content:space-between;align-items:center;">
                ...
                <span id="total-clicky-earned" style="color:var(--ok);font-weight:600;font-size:16px;">0 ðŸª™</span>
              </div>
            </div>
            ...
            <div style="margin-top:16px;padding:10px;background:rgba(100,167,255,.1);border-radius:8px;border:1px solid var(--accent);">
              <p style="margin:0;font-size:11px;color:var(--ink-1);">
                �a�️ <strong>Límite diario:</strong> Máximo 50 clics por día (100-150 Ecoxionums diarios)
              </p>
            </div>
            ...
            <div id="autoclicker-warning" style="display:none;margin-top:12px;padding:12px;background:rgba(255,107,107,.15);border-radius:8px;border:2px solid var(--danger);text-align:center;">
              <p style="margin:0;font-size:12px;color:var(--danger);font-weight:600;">
                �a�️ Auto-clicker detectado. Los clics demasiado rápidos no son válidos.
              </p>
            </div>
            
            <div style="margin-top:20px;padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
              <h4 style="margin:0 0 12px;font-size:15px;">ðŸ’° Retiro de Ecoxionums</h4>
              ...
              <div style="display:flex;flex-direction:column;gap:8px;">
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px;background:rgba(255,255,255,.05);border-radius:6px;">
                  ...
                  <span id="withdrawable-amount" style="color:var(--ok);font-weight:700;font-size:16px;">0 ðŸª™</span>
                </div>
                <button id="withdraw-btn" class="btn primary" onclick="withdrawEcoxionums()" style="width: 100%; opacity: 0.6;" disabled="">
                  ðŸŒŠ Retirar a Ocean Pay
                </button>
      ...
      "eco-generator": {
        ...
        <div id="eco-generator-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:24px;background:linear-gradient(135deg, rgba(80, 200, 120, 0.1), rgba(0, 150, 255, 0.1));border-radius:16px;border:1px solid rgba(80, 200, 120, 0.3);position:relative;overflow:hidden;">
            <div style="position:absolute;top:-20px;right:-20px;font-size:120px;opacity:0.05;pointer-events:none;">âš¡</div>
            
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
              <h4 style="margin:0;color:var(--accent);font-size:18px;display:flex;align-items:center;gap:8px;">
                <span style="background:rgba(80, 200, 120, 0.2);padding:6px;border-radius:8px;">âš¡</span>
            ...
            <div id="daily-reward-status" style="margin-bottom:20px;text-align:center;"><p style="color:var(--ok);margin:0;font-weight:700;">? ?El reactor est? listo!</p></div>

            <button id="claim-daily-btn" class="btn primary" onclick="claimDailyReward()" style="width: 100%; padding: 14px; font-size: 15px; font-weight: 600; box-shadow: rgba(0, 0, 0, 0.1) 0px 4px 12px; opacity: 1;">Reclamar Recompensa</button>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.05);text-align:center;">
              ...
              <div id="total-earned" style="font-size:20px;font-weight:700;color:var(--ok);">0 ðŸª™</div>
            </div>
            <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.05);text-align:center;">
          ...
          <div style="margin-top:16px;padding:12px 16px;background:linear-gradient(90deg, rgba(255, 171, 0, 0.1), transparent);border-left:3px solid #ffab00;border-radius:0 8px 8px 0;">
            <p style="margin:0;font-size:12px;color:#ffab00;">
              <strong>�a� Bono de Racha:</strong> +25% tras 3 días, +50% tras 7 días. ¡No pierdas tu racha!
            </p>
          </div>
        </div>
      `;
      ...
      "eco-luck": {
        ...
        <div id="eco-luck-panel" style="padding:16px;">
          <div style="border:1px solid rgba(255,215,0,0.28);border-radius:16px;background:linear-gradient(145deg, rgba(255,215,0,0.08), rgba(255,136,0,0.05));padding:16px;margin-bottom:16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
              <h4 style="margin:0;font-size:20px;color:#ffd86a;font-weight:800;">Ecoxionum Fortuna</h4>
              <div style="font-size:11px;color:#ffd86a;background:rgba(255,215,0,0.12);padding:4px 10px;border-radius:999px;border:1px solid rgba(255,215,0,0.28);">Alta volatilidad</div>
            </div>
            <p style="margin:8px 0 0;color:var(--ink-1);font-size:13px;">Inviertes <b>25 <span class="icon-ecoxionum"></span></b> por tirada y puedes recuperar desde <b>0</b> hasta <b>250 <span class="icon-ecoxionum"></span></b>.</p>
            <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin-top:12px;">
              <div style="padding:8px;border-radius:10px;background:rgba(255,255,255,.03);text-align:center;font-size:11px;">x10<br><b id="eco-luck-chance-10" style="color:#8cff8c">3%</b></div>
              <div style="padding:8px;border-radius:10px;background:rgba(255,255,255,.03);text-align:center;font-size:11px;">x3<br><b id="eco-luck-chance-3" style="color:#8ed2ff">17%</b></div>
              <div style="padding:8px;border-radius:10px;background:rgba(255,255,255,.03);text-align:center;font-size:11px;">x1.5<br><b id="eco-luck-chance-15" style="color:#ffd86a">30%</b></div>
              <div style="padding:8px;border-radius:10px;background:rgba(255,255,255,.03);text-align:center;font-size:11px;">Nada<br><b id="eco-luck-chance-0" style="color:#ff8f8f">50%</b></div>
            </div>
            <div id="eco-luck-plan-boost" style="margin-top:10px;font-size:12px;color:#bde6ff;">â„¹ï¸ Plan base activo: puedes mejorar tu suerte con planes premium.</div>
          </div>

          <div style="position:relative;height:220px;display:flex;align-items:center;justify-content:center;margin-bottom:18px;">
            <div style="position:absolute;width:210px;height:210px;border-radius:50%;border:2px dashed rgba(255,255,255,0.1);animation:spin 24s linear infinite;"></div>
            <button id="eco-luck-button" class="btn" onclick="playEcoLuck()" style="width:170px;height:170px;border-radius:50%;border:none;background:radial-gradient(circle at 30% 30%, #2a2a2a, #000);box-shadow:0 0 30px rgba(255, 215, 0, 0.15);position:relative;cursor:pointer;overflow:hidden;transition:transform 0.2s;">
              <div style="position:absolute;inset:4px;border-radius:50%;background:conic-gradient(#FFD700 0% 25%, #FF4500 25% 50%, #9C27B0 50% 75%, #2196F3 75% 100%);opacity:0.22;"></div>
              <div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:2;">
                ...
                <span style="font-size:12px;font-weight:800;color:#FFF;margin-top:4px;">TIRAR</span>
              </div>
            </button>
            <div id="luck-result-pop" style="position:absolute;font-size:30px;font-weight:800;text-shadow:0 4px 12px rgba(0,0,0,0.5);opacity:0;pointer-events:none;z-index:10;transform:scale(0.5);"></div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-bottom:10px;">
            <div style="background:var(--bg-1);padding:10px;border-radius:10px;text-align:center;">
              <div style="font-size:10px;color:var(--ink-1);text-transform:uppercase;">Ãšltimo resultado</div>
              <div id="eco-luck-last-result" style="font-size:16px;font-weight:700;">-</div>
            </div>
            <div style="background:var(--bg-1);padding:10px;border-radius:10px;text-align:center;">
              <div style="font-size:10px;color:var(--ink-1);text-transform:uppercase;">Multiplicador</div>
              <div id="eco-luck-multiplier" style="font-size:16px;font-weight:700;color:#ffd86a;">x0</div>
            </div>
          </div>

          <div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;">
            <div style="background:var(--bg-1);padding:10px;border-radius:10px;text-align:center;"><span style="display:block;font-size:10px;color:var(--ink-1);text-transform:uppercase;">Jugadas</span><span id="eco-luck-plays" style="font-size:16px;font-weight:700;">0</span></div>
            <div style="background:var(--bg-1);padding:10px;border-radius:10px;text-align:center;"><span style="display:block;font-size:10px;color:var(--ink-1);text-transform:uppercase;">Aciertos</span><span id="eco-luck-winrate" style="font-size:16px;font-weight:700;color:#8ed2ff;">0%</span></div>
            <div style="background:var(--bg-1);padding:10px;border-radius:10px;text-align:center;"><span style="display:block;font-size:10px;color:var(--ink-1);text-transform:uppercase;">Mejor</span><span id="eco-luck-best" style="font-size:16px;font-weight:700;color:#FFD700;">0</span></div>
            <div style="background:var(--bg-1);padding:10px;border-radius:10px;text-align:center;"><span style="display:block;font-size:10px;color:var(--ink-1);text-transform:uppercase;">Neto</span><span id="eco-luck-net" style="font-size:16px;font-weight:700;">0</span></div>
          </div>
        </div>
      `;
        },
          ...
          return `
            <div id="eco-stock-panel" style="padding:16px;">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
                <div style="font-size:18px;font-weight:700;color:var(--accent);">ðŸ“ˆ Mercado de Valores</div>
                <div style="font-size:12px;padding:4px 8px;background:rgba(16,185,129,0.1);color:var(--ok);border-radius:4px;font-weight:600;" id="stock-market-status">Mercado Abierto ðŸŸ¢</div>
              </div>
              ...
              <div style="background:var(--bg-1);border-radius:12px;padding:16px;border:1px solid rgba(255,255,255,0.08);">
                 <h4 style="margin:0 0 12px;color:var(--ink-0);">ðŸ’¼ Tu Portafolio</h4>
        ...
        <div id="calculator-panel" style="padding:16px;">
          <div style="max-width:400px;margin:0 auto;">
            ...
            <div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:8px;margin-top:8px;">
              ...
              <button class="calc-btn" onclick="calcSqrt()">âˆš</button>
            </div>
          </div>
        </div>
        <style>
          ...
          <div style="max-width:400px;margin:0 auto;">
            <div style="text-align:center;margin-bottom:24px;padding:40px 20px;background:var(--bg-1);border-radius:50%;width:280px;height:280px;margin:0 auto 30px;display:flex;flex-direction:column;justify-content:center;align-items:center;border:4px solid var(--accent);box-shadow:0 0 30px rgba(var(--accent-rgb), 0.2);position:relative;">
              ...
              <svg style="position:absolute;top:-4px;left:-4px;width:280px;height:280px;pointer-events:none;transform:rotate(-90deg);">
                <circle cx="140" cy="140" r="136" fill="none" stroke="var(--bg-2)" stroke-width="4"></circle>
                <circle id="pomodoro-progress" cx="140" cy="140" r="136" fill="none" stroke="var(--accent)" stroke-width="4" stroke-dasharray="854" stroke-dashoffset="0" style="transition:stroke-dashoffset 1s linear;"></circle>
              </svg>
            </div>
            
            <div style="display:flex;justify-content:center;gap:16px;margin-bottom:30px;">
              <button id="pomodoro-start-btn" class="btn primary" onclick="pomodoroStart()" style="width:60px;height:60px;border-radius:50%;font-size:24px;display:flex;align-items:center;justify-content:center;">â–¶</button>
              ...
              <button id="pomodoro-reset-btn" class="btn" onclick="pomodoroReset()" style="width:60px;height:60px;border-radius:50%;background:var(--bg-2);font-size:24px;display:flex;align-items:center;justify-content:center;">â†º</button>
            </div>
      ...
      "image-gallery": {
        ...
        <div id="gallery-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            <h4 style="margin:0 0 12px;font-size:16px;">âž• Agregar Imagen</h4>
        ...
        <div id="gallery-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.9);z-index:10000;padding:20px;overflow:auto;" onclick="closeGalleryModal()">
          <div style="max-width:90vw;max-height:90vh;margin:auto;position:relative;" onclick="event.stopPropagation()">
            <button onclick="closeGalleryModal()" style="position:absolute;top:-40px;right:0;background:var(--danger);color:white;border:none;padding:8px 16px;border-radius:8px;cursor:pointer;font-weight:600;">âœ• Cerrar</button>
      ...
      "survey-rewards": {
        ...
        <div id="survey-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:20px;background:linear-gradient(135deg, rgba(100,167,255,.1), rgba(179,136,255,.1));border-radius:12px;border:1px solid var(--accent);text-align:center;">
            <h4 style="margin:0 0 12px;color:var(--accent);">ðŸ“‹ Encuestas Disponibles</h4>
          ...
          <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            <h4 style="margin:0 0 12px;font-size:15px;">�x` Tus Estadísticas</h4>
            <div style="display:flex;flex-direction:column;gap:8px;">
              <div style="display:flex;justify-content:space-between;align-items:center;">
              ...
              <div style="display:flex;justify-content:space-between;align-items:center;">
                ...
                <span id="surveys-earned" style="font-weight:600;color:var(--ok);">0 <span class="icon-ecoxionum"></span></span>
              </div>
            </div>
          </div>
        </div>
      ...
      "comms-generator": {
            ...
            <div class="comms-container">
              <div class="comms-form">
                ...
                <div style="margin-top:20px; display:flex; gap:10px;">
                  <button class="btn primary" onclick="downloadComm()" style="flex:1;">ðŸ’¾ Guardar / Publicar</button>
                  <button class="btn" onclick="resetComm()" style="width:50px;">â†º</button>
                </div>
              </div>
        ...
        async onEnable() {
          window.applyTextPreset = function () {
            ...
            const presets = {
              ...
              urgent: {
                ...
                body: "ATENCI�N: Se ha detectado una anomalía en el sector 7.\n\nEvacuar inmediatamente. Esto no es un simulacro.",
                template: "alert"
              },
              decree: {
                title: "DECRETO REAL",
          ...
          window.updateCommsPreview = function () {
            ...
            const author = document.getElementById('comm-author').value || 'ADMINISTRACIÃ“N';
      ...
      "trivia-challenge": {
        ...
        <div id="trivia-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:20px;background:linear-gradient(135deg, rgba(147,51,234,.15), rgba(79,70,229,.15));border-radius:16px;border:2px solid rgba(147,51,234,.4);text-align:center;">
            <h3 style="margin:0 0 8px;color:#a855f7;font-size:24px;">ðŸŽ¯ Trivia Challenge</h3>
            ...
            <button id="trivia-start-btn" class="btn primary" onclick="startTrivia()" style="width:100%;font-size:16px;padding:14px;">ðŸŽ® Iniciar Trivia</button>
          </div>
          <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            <h4 style="margin:0 0 12px;font-size:15px;">�x` Estadísticas</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      ...
      "music-player": {
        ...
        <div id="music-panel" style="padding:16px;">
          ...
          <div style="display:flex;gap:12px;justify-content:center;margin-bottom:20px;">
            ...
            <button id="music-play-btn" class="btn primary" onclick="musicToggle()" style="width:70px;height:70px;border-radius:50%;font-size:28px;">â–¶ï¸</button>
            <button class="btn" onclick="musicNext()" style="width:50px;height:50px;border-radius:50%;font-size:20px;">⏭️</button>
          </div>
          <div style="margin-bottom:16px;">
            <label style="display:block;margin-bottom:8px;font-size:13px;color:var(--ink-1);">ðŸ”Š Volumen</label>
          ...
          <div style="padding:16px;background:var(--bg-1);border-radius:12px;">
            <h4 style="margin:0 0 12px;font-size:14px;">ðŸ“» Estaciones</h4>
            <div id="music-stations" style="display:flex;flex-direction:column;gap:8px;">
              <div class="music-station" onclick="selectStation(0)" style="padding:12px;background:rgba(255,255,255,.03);border-radius:8px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:all .2s;">
                <span style="font-size:24px;">ðŸŽ¸</span><div><div style="font-weight:600;">Lo-Fi Beats</div><div style="font-size:12px;color:var(--ink-1);">Relax & Focus</div></div>
              </div>
              <div class="music-station" onclick="selectStation(1)" style="padding:12px;background:rgba(255,255,255,.03);border-radius:8px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:all .2s;">
                <span style="font-size:24px;">ðŸŽ¹</span><div><div style="font-weight:600;">Classical</div><div style="font-size:12px;color:var(--ink-1);">Piano & Orchestra</div></div>
              </div>
              <div class="music-station" onclick="selectStation(2)" style="padding:12px;background:rgba(255,255,255,.03);border-radius:8px;cursor:pointer;display:flex;align-items:center;gap:12px;transition:all .2s;">
                <span style="font-size:24px;">ðŸŽ¤</span><div><div style="font-weight:600;">Pop Hits</div><div style="font-size:12px;color:var(--ink-1);">Top Charts</div></div>
              </div>
            </div>
          </div>
        </div>
      ...
      "habit-tracker": {
        ...
        <div id="habit-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            ...
            <div style="display:flex;gap:8px;">
              ...
              <select id="habit-icon" style="padding:10px;border-radius:8px;background:var(--bg-2);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);font-size:18px;">
                <option>ðŸ’ª</option><option>ðŸ“š</option><option>ðŸƒ</option><option>ðŸ’§</option><option>ðŸ§˜</option><option>ðŸ’¤</option><option>ðŸ¥—</option><option>âœï¸</option>
              </select>
          ...
          <div style="margin-top:20px;padding:16px;background:linear-gradient(135deg, rgba(16,185,129,.1), rgba(52,211,153,.1));border-radius:12px;border:1px solid rgba(16,185,129,.3);">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              <span style="color:var(--ink-1);">ðŸ† Ecoxionums por rachas:</span>
              <span id="habit-earnings" style="font-weight:700;color:var(--ok);">0 <span class="icon-ecoxionum"></span></span>
            </div>
          </div>
        </div>
      `;
        ...
        <div id="weather-panel" style="padding:16px;">
          <div style="margin-bottom:20px;">
            <div style="display:flex;gap:8px;">
              ...
              <button class="btn primary" onclick="searchWeather()">ðŸ”</button>
            </div>
          </div>
          <div id="weather-display" style="padding:32px;background:linear-gradient(135deg, rgba(59,130,246,.15), rgba(147,197,253,.15));border-radius:20px;border:1px solid rgba(59,130,246,.3);text-align:center;">
            <div id="weather-icon" style="font-size:72px;margin-bottom:8px;">ðŸŒ¤ï¸</div>
          ...
          <div id="weather-details" style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:16px;">
            ...
            <div style="padding:16px;background:var(--bg-1);border-radius:12px;text-align:center;">
              <div style="font-size:24px;margin-bottom:4px;">ðŸŒ¡ï¸</div>
      ...
      "eco-miner": {
        ...
        <div id="miner-panel" style="padding:16px;">
          <div style="padding:24px;background:linear-gradient(135deg, rgba(245,158,11,.12), rgba(251,191,36,.12));border-radius:16px;border:2px solid rgba(245,158,11,.4);text-align:center;margin-bottom:20px;">
            <div id="miner-animation" style="font-size:64px;margin-bottom:16px;">â›ï¸</div>
        ...
        <div id="arcade-panel" style="padding:20px; color:var(--ink-0);">
          ...
          <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:20px;">
            ...
            <div onclick="startArcadeGame('snake')" class="arcade-card">
              <div class="arcade-screen" style="background:linear-gradient(to bottom, #001100, #002200);">
                <div style="font-size:48px; filter:drop-shadow(0 0 10px #0f0);">ðŸ</div>
              </div>
              <div class="arcade-info">
                <h4>Cyber Snake</h4>
            ...
            <div onclick="startArcadeGame('memory')" class="arcade-card">
              <div class="arcade-screen" style="background:linear-gradient(to bottom, #000022, #000044);">
                <div style="font-size:48px; filter:drop-shadow(0 0 10px #00f);">ðŸ§ </div>
              </div>
              <div class="arcade-info">
                <h4>Memory Matrix</h4>
                <p>Pon a prueba tu memoria visual contra el reloj.</p>
                <div class="arcade-score">
                  <span>ðŸ† Best Time:</span>
          ...
          <div id="arcade-game-container" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.95); z-index:5000; padding:20px; overflow:auto;">
            <div style="max-width:800px; margin:0 auto; display:flex; flex-direction:column; align-items:center; height:100%; justify-content:center;">
              <button onclick="closeArcadeGame()" style="position:absolute; top:20px; right:20px; background:transparent; border:none; color:#fff; font-size:16px; cursor:pointer;">âœ• CERRAR</button>
        ...
        <div id="chat-panel" style="padding:20px; color:var(--ink-1); display:flex; flex-direction:column; height:450px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
            <h3 style="margin:0; font-size:18px; color:var(--accent); display:flex; align-items:center; gap:8px;">
              <span>ðŸ’¬</span> Quick Chat (Ecoxion Community)
            </h3>
          ...
          <div style="display:flex; gap:10px; background:rgba(255,255,255,0.03); padding:10px; border-radius:16px; border:1px solid rgba(255,255,255,0.08);">
            <input type="text" id="chat-input" placeholder="Envía un mensaje a la comunidad..." style="flex:1; padding:12px; border:none; background:transparent; color:var(--ink-0); font-size:14px; outline:none;" onkeypress="if(event.key==='Enter')sendChatMessage()">
      ...
      "mini-calendar": {
        ...
        <div id="calendar-panel" style="padding:20px;">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:24px;">
            <button class="btn" onclick="calendarPrevMonth()" style="width:40px; height:40px; padding:0; border-radius:50%; display:flex; align-items:center; justify-content:center;">â—€</button>
            <h3 id="calendar-month-title" style="margin:0; font-size:20px; color:var(--ink-0); font-weight:700;">Diciembre 2025</h3>
            <button class="btn" onclick="calendarNextMonth()" style="width:40px; height:40px; padding:0; border-radius:50%; display:flex; align-items:center; justify-content:center;">â–¶</button>
          </div>
          ...
          <div style="background:var(--bg-1); border-radius:16px; padding:20px; box-shadow:0 8px 24px rgba(0,0,0,0.1);">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px;">
              <h4 style="margin:0; font-size:16px; color:var(--ink-0);">ðŸ“… Agenda</h4>
      ...
      "eclipser": {
        id: "eclipser",
        name: "Eclipser",
        description: "Visualiza eclipses en 3D, participa en eventos globales y gana Ecoxionums.",
        rarity: "epica",
        type: "active",
        version: "1.0.0",
        icon: "\uD83C\uDF18",
        ...
        getPanelHTML() {
          return `
        <div id="eclipser-panel" style="padding:16px;display:grid;gap:12px;">
          <div class="card" style="padding:14px;background:linear-gradient(135deg, rgba(56,189,248,.15), rgba(37,99,235,.12));border:1px solid rgba(56,189,248,.35);">
            <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;">
              <div>
                <h3 style="margin:0 0 4px;color:var(--ink-0);">Eclipser OG</h3>
                <div style="font-size:12px;color:var(--ink-1);">Eventos globales de eclipse + eclipses personalizados para Ultra.</div>
              </div>
              <div style="display:flex;gap:8px;flex-wrap:wrap;">
                <button class="btn" onclick="refreshEclipserData()">Actualizar</button>
                <button class="btn primary" id="eclipser-open-any-btn" type="button" onclick="window.openAnyEclipserViewer && window.openAnyEclipserViewer(); return false;">Abrir visor ahora</button>
              </div>
            </div>
          </div>
          <div class="card" style="padding:14px;">
            <div style="font-weight:700;margin-bottom:8px;">Proximos Eclipses</div>
            <div id="eclipser-upcoming" style="display:grid;gap:8px;color:var(--ink-1);font-size:13px;">Cargando...</div>
          </div>
          <div class="card" style="padding:14px;">
            <div style="font-weight:700;margin-bottom:8px;">Eclipse Activo</div>
            <div id="eclipser-active" style="display:grid;gap:8px;color:var(--ink-1);font-size:13px;">Buscando evento activo...</div>
          </div>
          <div id="eclipser-ultra-zone" class="card" style="padding:14px;display:none;">
            <div style="font-weight:700;margin-bottom:8px;">Ultra Nova: Eclipse Personalizado</div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;">
              <input id="eclipser-custom-name" placeholder="Nombre del eclipse" style="grid-column:1/3;padding:10px;border-radius:8px;background:var(--bg-2);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);" />
              <select id="eclipser-custom-type" style="padding:10px;border-radius:8px;background:var(--bg-2);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);">
                <option value="solar_partial">Solar Parcial</option>
                <option value="solar_total">Solar Total</option>
                <option value="lunar_partial">Lunar Parcial</option>
                <option value="lunar_total">Lunar Total</option>
                <option value="annular">Anular</option>
              </select>
              <input id="eclipser-custom-start" type="datetime-local" style="padding:10px;border-radius:8px;background:var(--bg-2);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);" />
              <input id="eclipser-custom-end" type="datetime-local" style="padding:10px;border-radius:8px;background:var(--bg-2);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);" />
              <button class="btn primary" onclick="createCustomEclipserEvent()" style="grid-column:1/3;">Crear Eclipse Personalizado (90% menos recompensa)</button>
            </div>
          </div>
        </div>
      `;
        },
        async onEnable() { initEclipser(); }
      }
    };
    ...
    <div class="card" style="margin-bottom:12px;padding:16px;">
      ...
      <div style="display:flex;gap:8px;">
        <button class="btn primary" onclick="viewNoteDetail(${i})" style="flex:1;">ðŸ“– Ver Detalle</button>
        <button class="btn" onclick="deleteNote(${i})">ðŸ—‘ï¸ Eliminar</button>
      </div>
    </div>
  `).join('');
    }
    ...
    <div class="missions-container" style="max-width:700px;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
        ...
        <button class="btn" onclick="this.closest('.missions-modal').remove()" style="padding:8px 12px;">âœ• Cerrar</button>
      </div>
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding-top:16px;border-top:1px solid rgba(255,255,255,.1);">
        ...
        <button class="btn" onclick="deleteNote(${index}); this.closest('.missions-modal').remove();" style="background:rgba(255,107,107,.2);border-color:var(--danger);color:var(--danger);">ðŸ—‘ï¸ Eliminar</button>
      </div>
    </div>
  `;
    ...
    <div class="card" style="margin-bottom:12px;padding:16px;border-left:3px solid var(--accent);">
      <h4 style="margin:0 0 8px;">ðŸŒŒ Bienvenido a Ecoxion</h4>
  ...
  `;
    ...
    async function loadBlogs() {
      ...
      try {
        ...
        list.innerHTML = blogs.map(blog => {
          ...
          return `
            <div class="card" style="margin:0; padding:20px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:16px; transition:all 0.3s; cursor:default;" onmouseenter="this.style.background='rgba(255,255,255,0.05)'; this.style.borderColor='rgba(100,167,255,0.3)';" onmouseleave="this.style.background='rgba(255,255,255,0.03)'; this.style.borderColor='rgba(255,255,255,0.08)';">
              ...
              <div style="margin-top:16px; display:flex; gap:10px;">
                <button class="btn" style="padding:6px 12px; font-size:12px; opacity:0.8;" onclick="showAlert('info', 'Guardado', 'Copiado al portapapeles')">ðŸ”— Enlace</button>
        ...
        list.innerHTML = '<div style="color:var(--danger); text-align:center; padding:20px;">âš ï¸ Error al conectar con la red de blogs.</div>';
      }
    }
    ...
    async function updateGeneratorPanel() {
      ...
      if (totalEl) totalEl.innerHTML = `${(generatorData.totalEarned || 0).toLocaleString()} <span class="icon-ecoxionum"></span>`;
      ...
      if (canClaim) {
        statusEl.innerHTML = '<p style="color:var(--ok);margin:0;font-weight:700;">�S& ¡Energía acumulada!</p>';
        claimBtn.disabled = false;
        claimBtn.innerHTML = 'Reclamar Recompensa';
        claimBtn.style.opacity = '1';
      } else {
    ...
    window.claimDailyReward = async function () {
      ...
      if (newStreak > 1) message += `. Racha: ${newStreak} días �x�`;
    ...
    async function updateEcoLuckPanel(currentState = null) {
      ...
      const multiplierEl = document.getElementById('eco-luck-multiplier');
      const winRateEl = document.getElementById('eco-luck-winrate');
      ...
      if (!data.lastResult) {
        ...
        if (multiplierEl) multiplierEl.textContent = 'x0';
      } else {
        const lr = data.lastResult;
        const sign = lr.net > 0 ? '+' : lr.net < 0 ? '-' : '';
        const color = lr.net > 0 ? 'var(--ok)' : lr.net < 0 ? 'var(--danger)' : 'var(--ink-1)';
        lastResultEl.innerHTML = `<span style="color:${color};font-weight:700;">${sign}${Math.abs(lr.net)} <span class="icon-ecoxionum"></span></span>`;
        if (multiplierEl) multiplierEl.textContent = `x${Number(lr.multiplier || 0).toLocaleString('es-ES')}`;
      }

      playsEl.textContent = (data.plays || 0).toString();
      bestEl.innerHTML = `${(data.bestPrize || 0).toLocaleString()} <span class="icon-ecoxionum"></span>`;

      const netSign = net > 0 ? '+' : net < 0 ? '-' : '';
      netEl.innerHTML = `${netSign}${Math.abs(net).toLocaleString()} <span class="icon-ecoxionum"></span>`;
      netEl.style.color = net > 0 ? 'var(--ok)' : net < 0 ? 'var(--danger)' : 'var(--ink-0)';
      if (winRateEl) {
        const wr = data.plays > 0 ? Math.round((data.wins / data.plays) * 100) : 0;
        winRateEl.textContent = `${wr}%`;
      }

      await loadEcoxionPlanConfig(false);
      const odds = getEcoLuckOddsByPlan(state.subscriptionPlan || 'free');
      const chance10El = document.getElementById('eco-luck-chance-10');
      const chance3El = document.getElementById('eco-luck-chance-3');
      const chance15El = document.getElementById('eco-luck-chance-15');
      const chance0El = document.getElementById('eco-luck-chance-0');
      const planBoostEl = document.getElementById('eco-luck-plan-boost');
      if (chance10El) chance10El.textContent = `${odds.tier10}%`;
      if (chance3El) chance3El.textContent = `${odds.tier3}%`;
      if (chance15El) chance15El.textContent = `${odds.tier15}%`;
      if (chance0El) chance0El.textContent = `${odds.tier0}%`;
      if (planBoostEl) {
        const planName = getEcoxionPlanName(state.subscriptionPlan || 'free');
        const isBoosted = normalizeEcoxionPlanId(state.subscriptionPlan || 'free') !== 'free';
        planBoostEl.innerHTML = isBoosted
          ? `⭐ <strong>${planName}</strong> activo: mejor suerte aplicada en Ecoxionum Fortuna.`
          : `â„¹ï¸ Plan base activo: puedes mejorar tu suerte con planes premium.`;
      }
    }

    function getEcoLuckOddsByPlan(planId = 'free') {
      const plan = normalizeEcoxionPlanId(planId);
      const cfg = getFortuneOddsConfig();
      const row = (cfg && typeof cfg === 'object' && cfg[plan]) ? cfg[plan] : (cfg?.free || FORTUNE_ODDS_DEFAULT.free);
      return {
        tier0: Number(row?.tier0 ?? FORTUNE_ODDS_DEFAULT.free.tier0),
        tier15: Number(row?.tier15 ?? FORTUNE_ODDS_DEFAULT.free.tier15),
        tier3: Number(row?.tier3 ?? FORTUNE_ODDS_DEFAULT.free.tier3),
        tier10: Number(row?.tier10 ?? FORTUNE_ODDS_DEFAULT.free.tier10)
      };
    }
    ...
    window.playEcoLuck = async function () {
      ...
      let multiplier = 0;
      const odds = getEcoLuckOddsByPlan(state.subscriptionPlan || 'free');
      const threshold0 = odds.tier0;
      const threshold15 = threshold0 + odds.tier15;
      const threshold3 = threshold15 + odds.tier3;

      if (roll < threshold0) {
        prize = 0; multiplier = 0;
      } else if (roll < threshold15) {
      ...
      const tier = multiplier >= 10 ? 'legendario' : multiplier >= 3 ? 'epico' : multiplier > 0 ? 'comun' : 'fallo';

      // Actualizar datos
      data.plays = (data.plays || 0) + 1;
      data.wins = (data.wins || 0) + (prize > 0 ? 1 : 0);
      ...
      data.lastResult = { net, prize, multiplier, tier, at: Date.now() };
      ...
      if (resultPop) {
        resultPop.innerHTML = prize > 0 ? `+${prize} <span class="icon-ecoxionum"></span>` : 'Sin premio';
    ...
    /* =========================
       CLICKY COIN - EXTENSIÃ“N GRATUITA
       ========================= */
    async function updateClickyCoinPanel(currentState = null) {
      ...
      if (earnedEl) earnedEl.innerHTML = `${(clickyData.earnedToday || 0)} <span class="icon-ecoxionum"></span>`;
      if (totalClicksEl) totalClicksEl.textContent = (clickyData.totalClicks || 0).toLocaleString();
      if (totalEl) totalEl.innerHTML = `${(clickyData.totalEarned || 0).toLocaleString()} <span class="icon-ecoxionum"></span>`;
      ...
      if (coinBtn) {
        if ((clickyData.clicksToday || 0) >= 50) {
          ...
          coinBtn.innerHTML = '�S&<br><span style="font-size:14px;">Límite alcanzado</span>';
        } else {
          coinBtn.style.opacity = '1';
          coinBtn.style.cursor = 'pointer';
          coinBtn.onclick = clickCoin;
          coinBtn.innerHTML = '<span class="icon-ecoxionum"></span>';
        }
      }
    }
    ...
    window.clickCoin = async function () {
      ...
      try {
        ...
        if (animContainer) {
          ...
          plusText.innerHTML = `+${reward} <span class="icon-ecoxionum"></span>`;
    ...
    window.withdrawEcoxionums = async function () {
      ...
      try {
        ...
        showAlert('success', 'Retiro exitoso', `Has retirado ${withdrawableAmount.toLocaleString()} Ecoxionums a tu cuenta de Ocean Pay. ðŸŽ‰`);
      } catch (error) {
    ...
    async function updateWithdrawButton(currentState = null) {
      ...
      withdrawableEl.innerHTML = `${withdrawableAmount.toLocaleString()} <span class="icon-ecoxionum"></span>`;
    ...
    const wordBattle = {
      ...
      init() {
        ...
        // Listeners
        this.socket.on('connect', () => console.log('ðŸ”Œ Conectado a Word Battle Server'));
        ...
        this.socket.on('wb_word_accepted', (data) => {
          this.showMessage(`âœ… ${data.word} (+10pts)`, 'success');
        ...
        this.socket.on('wb_player_damage', (data) => {
          ...
          if (p) this.showMessage(`ðŸ’” ${p.name} pierde una vida`, 'error');
        });

        this.socket.on('wb_player_eliminated', (data) => {
          const p = this.players.find(pl => pl.id === data.playerId);
          if (p) this.showMessage(`ðŸ’€ ${p.name} eliminado`, 'error');
      ...
      updateLobbyUI() {
        ...
        list.innerHTML = this.players.map(p => `
          <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:var(--bg-2);border:1px solid rgba(255,255,255,.1);border-radius:8px;margin-bottom:8px;">
            ...
            ${p.id === this.players[0]?.id ? '<span style="color:var(--accent);">ðŸ‘‘ Host</span>' : ''}
          </div>
        `).join('');
        ...
        if (this.isHost) {
          ...
          startBtn.textContent = this.players.length < 2 ? 'ðŸŽ® Jugar Solo (Prueba)' : 'ðŸŽ® Iniciar Partida';
        } else {
      ...
      updatePlayersList(currentTurnId) {
        ...
        grid.innerHTML = this.players.map(p => {
          ...
          const livesHtml = Array(3).fill(0).map((_, i) => i < p.lives ? 'â¤ï¸' : 'ðŸ–¤').join('');
      ...
      showResults(winner) {
        ...
        podium.innerHTML = sorted.slice(0, 3).map((p, i) => `
          <div class="wb-podium-place ${i === 0 ? 'first' : i === 1 ? 'second' : 'third'}">
            <div class="position">${i === 0 ? 'ðŸ¥‡' : i === 1 ? 'ðŸ¥ˆ' : 'ðŸ¥‰'}</div>
    ...
    function drawGameModel(canvas, model) {
      ...
      switch (model) {
        ...
        case 'cards':
          drawShadow();
          ctx.fillStyle = '#fff';
          roundRect(ctx, w * 0.2, h * 0.2, w * 0.4, h * 0.6, 5, true, false);
          ctx.fill();
          ctx.strokeStyle = themeColor;
          ctx.stroke();
          ctx.fillStyle = themeColor;
          roundRect(ctx, w * 0.4, h * 0.3, w * 0.4, h * 0.6, 5, true, false);
          ctx.fill();
          ctx.strokeStyle = '#fff';
          ctx.stroke();
          break;
        case 'dice':
          drawShadow();
          ctx.fillStyle = '#fff';
          roundRect(ctx, w * 0.25, h * 0.25, w * 0.5, h * 0.5, 8, true, false);
          ctx.fill();
          ctx.fillStyle = '#333';
        ...
        case 'calculator':
          drawShadow();
          ctx.fillStyle = '#333';
          roundRect(ctx, w * 0.25, h * 0.2, w * 0.5, h * 0.7, 5, true, false);
          ctx.fill();
          ctx.fillStyle = '#9cf';
    ...
    const dailyRewards = {
      ...
      async updateDailyUI() {
        ...
        if (btn && timer) {
          if (timeSince >= oneDay) {
            btn.disabled = false;
            btn.textContent = 'ðŸŽ Reclamar recompensa';
        ...
        if (nextAmount) {
          ...
          nextAmount.innerHTML = `+${this.calculateDailyReward(projectedStreak)} <span class="icon-ecoxionum"></span>`;
        }
      ...
      async updateMissions() {
        const state = await this.ensureDailyMissionState();
        await saveState(state);
        const missions = [
        ...
        <div class="dr-mission-card ${completed ? 'completed' : ''}">
          <div class="dr-mission-header">
            <div class="dr-mission-title">${completed ? 'âœ…' : 'ðŸ“Œ'} ${m.title}</div>
            <div class="dr-mission-reward">+${m.reward} <span class="icon-ecoxionum"></span></div>
          </div>
          ...
          <div style="text-align:right;font-size:0.85em;color:var(--ink-1);margin-top:4px;">
            ${m.current}/${m.target} ${completed ? 'âœ“ Completada' : ''}
          </div>
        </div>
      `;
        }).join('');
      ...
      async ensureDailyMissionState(stateInput = null) {
        ...
        if (!Array.isArray(state.dailyRewards.openedExtensionsToday)) state.dailyRewards.openedExtensionsToday = [];

        const today = new Date().toDateString();
        if (state.dailyRewards.missionDay !== today) {
          state.dailyRewards.missionDay = today;
          state.dailyRewards.openedExtensionsToday = [];
          state.dailyRewards.missions.open_5_extensions = 0;
          state.dailyRewards.missions.spend_30_min = 0;
          state.dailyRewards.missions.play_3_minigames = 0;
          state.dailyRewards.claims.open_5_extensions = '';
          state.dailyRewards.claims.spend_30_min = '';
          state.dailyRewards.claims.play_3_minigames = '';
        }
        return state;
      },

      async tryGrantMissionReward(missionId, rewardAmount, successMessage) {
        const state = await this.ensureDailyMissionState();
        const today = new Date().toDateString();
        if (state.dailyRewards.claims?.[missionId] === today) return false;

        state.dailyRewards.claims[missionId] = today;
        state.dailyRewards.missionsCompleted = (state.dailyRewards.missionsCompleted || 0) + 1;
        state.dailyRewards.totalEarned = (state.dailyRewards.totalEarned || 0) + rewardAmount;
        await saveState(state);
        await awardEcoxionums(rewardAmount, `Mision diaria: ${missionId}`);
        showAlert('success', '¡Misión completada!', successMessage || `+${rewardAmount} Ecoxionums`);
        if (currentTab === 'rewards') await this.updateMissions();
        return true;
      },

      async trackOpenedExtension(extId) {
        if (!extId) return;
        const state = await this.ensureDailyMissionState();
        const opened = state.dailyRewards.openedExtensionsToday;
        if (!opened.includes(extId)) {
          opened.push(extId);
          state.dailyRewards.missions.open_5_extensions = opened.length;
          await saveState(state);
          if (opened.length >= 5) {
            await this.tryGrantMissionReward(
              'open_5_extensions',
              35,
              'Explorador completada: abriste 5 extensiones diferentes. +35 Ecoxionums'
            );
      ...
      async trackEcoxionTime(seconds = 60) {
        if (document.hidden) return;
        const increment = Math.max(1, Number(seconds) || 0);
        const state = await this.ensureDailyMissionState();
        state.dailyRewards.missions.spend_30_min = (state.dailyRewards.missions.spend_30_min || 0) + increment;
        await saveState(state);

        if (state.dailyRewards.missions.spend_30_min >= 1800) {
          await this.tryGrantMissionReward(
            'spend_30_min',
            45,
            'Tiempo en Ecoxion completada: 30 minutos acumulados. +45 Ecoxionums'
          );
        } else if (currentTab === 'rewards') {
          await this.updateMissions();
        }
      ...
      miniGameTicker: null,
      miniGameTickBusy: false,

      getMiniGamePlayLimit(planId = 'free') {
        const safePlan = normalizeEcoxionPlanId(planId);
        if (safePlan === 'pro' || safePlan === 'ultra') return 5;
        return 3;
      },

      getMiniGameResetCost() {
        return 280;
      },

      getMiniGameCooldownHours(planId) {
        const safePlan = normalizeEcoxionPlanId(planId);
        if (safePlan === 'ultra') return 2;
        if (safePlan === 'pro') return 6;
        if (safePlan === 'plus') return 12;
        return 24;
      },

      getMiniGameCooldownLabel(planId) {
        const hrs = this.getMiniGameCooldownHours(planId);
        return `Reset ${hrs}h`;
      },

      getMiniGameWindowMs(planId) {
        return this.getMiniGameCooldownHours(planId) * 60 * 60 * 1000;
      },

      formatMiniGameTime(ms) {
      ...
      getMiniGamePlanFromState(state) {
        return normalizeEcoxionPlanId(state?.ecoxionMiniGamePlan || state?.subscriptionPlan || 'free');
      },

      normalizeMiniGameEntry(raw, nowTs, planId) {
        const now = Number(nowTs) || Date.now();
        const windowMs = this.getMiniGameWindowMs(planId);
        const limit = this.getMiniGamePlayLimit(planId);
        const safeRaw = (raw && typeof raw === 'object') ? raw : {};

        let plays = Number(safeRaw.playsInWindow);
        let windowStartAt = Number(safeRaw.windowStartAt);
        let migrated = false;

        if (!Number.isFinite(plays) || plays < 0 || !Number.isFinite(windowStartAt) || windowStartAt <= 0) {
          const legacyPlays = Math.max(0, Number(safeRaw.playsToday || 0));
          const legacyDate = typeof safeRaw.lastDate === 'string' ? new Date(safeRaw.lastDate) : null;
          const todayStart = new Date(now);
          todayStart.setHours(0, 0, 0, 0);
          const sameDay = legacyDate instanceof Date
            && !Number.isNaN(legacyDate.getTime())
            && legacyDate.toDateString() === new Date(now).toDateString();
          plays = sameDay ? legacyPlays : 0;
          windowStartAt = sameDay ? todayStart.getTime() : now;
          migrated = true;
        }

        plays = Math.max(0, Math.min(limit, Math.floor(plays)));
        ...
        const normalized = {
          playsInWindow: plays,
          windowStartAt,
          lastPlayedAt: Number(safeRaw.lastPlayedAt) || 0,
          // Legacy mirrors (compatibilidad con snapshots viejos y futuras lecturas)
          playsToday: plays,
      ...
      getMiniGameCooldown(gameData, planId) {
        const now = Date.now();
        const { normalized } = this.normalizeMiniGameEntry(gameData, now, planId);
        const windowMs = this.getMiniGameWindowMs(planId);
        const limit = this.getMiniGamePlayLimit(planId);
        const plays = normalized.playsInWindow;
        const stillInWindow = (now - normalized.windowStartAt) < windowMs;
        const locked = plays >= limit && stillInWindow;
        const remainingMs = locked ? Math.max(0, windowMs - (now - normalized.windowStartAt)) : 0;
        return {
          plays,
          locked,
          remainingMs,
          remainingLabel: locked ? this.formatMiniGameTime(remainingMs) : 'Listo',
          windowLabel: this.getMiniGameCooldownLabel(planId),
          normalized
        };
      },

      async syncMiniGamePlan(forceSync = false) {
        const state = await loadState();
        let changed = false;
        let planId = this.getMiniGamePlanFromState(state);
        const now = Date.now();
        const lastSyncAt = Number(state.ecoxionMiniGamePlanSyncedAt || 0);
        const shouldSyncServer = !!(state.oceanPayLinked && state.userId)
          && (forceSync || !lastSyncAt || (now - lastSyncAt) > (2 * 60 * 1000));
        ...
        if (shouldSyncServer) {
          try {
            ...
            if (res.ok) {
              ...
              const safeServerPlan = normalizeEcoxionPlanId(payload?.plan || 'free');
              if (safeServerPlan !== planId) {
                planId = safeServerPlan;
                changed = true;
              }
              if (state.subscriptionPlan !== safeServerPlan) {
                state.subscriptionPlan = safeServerPlan;
                changed = true;
              }
            }
          } catch (e) {
            console.warn('No se pudo sincronizar plan de minijuegos:', e);
          }
        }

        if (state.ecoxionMiniGamePlan !== planId) {
          state.ecoxionMiniGamePlan = planId;
          changed = true;
        }
        if (shouldSyncServer || !lastSyncAt) {
          state.ecoxionMiniGamePlanSyncedAt = now;
          changed = true;
        }
      ...
      async initMiniGameData(gameId) {
        const { state, planId } = await this.syncMiniGamePlan(false);
        if (!state.dailyRewards) state.dailyRewards = {};
        if (!state.dailyRewards.miniGames) state.dailyRewards.miniGames = {};

        const now = Date.now();
        const raw = state.dailyRewards.miniGames[gameId] || {};
        const cooldown = this.getMiniGameCooldown(raw, planId);
        const nextEntry = { ...cooldown.normalized };
        const prev = raw || {};
        const changed = prev.playsInWindow !== nextEntry.playsInWindow
          || prev.windowStartAt !== nextEntry.windowStartAt
          || prev.playsToday !== nextEntry.playsToday
          || prev.lastDate !== nextEntry.lastDate
          || prev.lastPlayedAt !== nextEntry.lastPlayedAt
          || !prev;

        if (changed) {
          state.dailyRewards.miniGames[gameId] = nextEntry;
          await saveState(state);
        }
      ...
      async registerMiniGamePlay(gameId) {
        ...
        const cooldown = this.getMiniGameCooldown(raw, planId);
        if (cooldown.locked) {
          return { blocked: true, cooldown, planId };
        }

        const nextEntry = { ...cooldown.normalized };
        if (nextEntry.playsInWindow <= 0) nextEntry.windowStartAt = now;
        nextEntry.playsInWindow = Math.max(0, Math.min(limit, nextEntry.playsInWindow + 1));
        nextEntry.lastPlayedAt = now;
        nextEntry.playsToday = nextEntry.playsInWindow;
        nextEntry.lastDate = new Date(now).toDateString();
        state.dailyRewards.miniGames[gameId] = nextEntry;
        await saveState(state);

        return {
          blocked: false,
          cooldown: this.getMiniGameCooldown(nextEntry, planId),
          planId
        };
      },

      async resetMiniGameCooldown(gameId) {
        const { state, planId } = await this.syncMiniGamePlan(true);
        const safePlan = normalizeEcoxionPlanId(planId);
        if (safePlan !== 'ultra') {
          return { ok: false, reason: 'ultra_required', planId };
        }
        ...
        const cooldown = this.getMiniGameCooldown(raw, planId);
        if (!cooldown.locked) {
          return { ok: false, reason: 'not_locked', planId, cooldown };
        }

        const resetCost = this.getMiniGameResetCost();
        const currentBalance = Number(state.ecoxionums || 0);
        if (currentBalance < resetCost) {
          return { ok: false, reason: 'insufficient_balance', required: resetCost, balance: currentBalance, planId };
        }

        let nextBalance = Math.max(0, currentBalance - resetCost);
        try {
          if (state.oceanPayLinked && state.userId && localStorage.getItem('opToken')) {
            const debitRes = await fetch(`${API_GLOBAL}/ocean-pay/ecoxionums/change`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('opToken') || ''}`
              ...
              body: JSON.stringify({
                userId: state.userId,
                amount: -resetCost,
                reason: `Reset minijuego ${gameId}`
              })
            });
            if (debitRes.ok) {
              const debitData = await debitRes.json();
              if (typeof debitData?.newBalance === 'number') {
                nextBalance = debitData.newBalance;
              }
            }
          }
        } catch (e) {
          console.warn('No se pudo debitar reset en servidor. Aplicando en local.', e);
        }

        state.ecoxionums = nextBalance;
        state.dailyRewards.miniGames[gameId] = {
          playsInWindow: 0,
      ...
      async promptMiniGameReset(gameId) {
        const game = miniGamesConfig[gameId];
        const gameName = game?.name || 'Minijuego';
        const cost = this.getMiniGameResetCost();
        const state = await loadState();
        const planId = this.getMiniGamePlanFromState(state);

        if (planId !== 'ultra') {
          showAlert('info', 'Plan Ultra Nova requerido', `La recarga instantánea cuesta ${cost} Ecoxionums y está disponible solo en Ultra Nova.`);
          switchTab('subscriptions');
          return;
        }

        const confirmed = confirm(`¿Recargar ahora "${gameName}" por ${cost} Ecoxionums?`);
        if (!confirmed) return;

        const result = await this.resetMiniGameCooldown(gameId);
        if (!result?.ok) {
          if (result?.reason === 'insufficient_balance') {
            showAlert('error', 'Saldo insuficiente', `Necesitas ${cost} Ecoxionums para recargar este minijuego.`);
            return;
          }
          if (result?.reason === 'not_locked') {
            showAlert('info', 'No es necesario', `${gameName} ya está disponible.`);
            return;
          }
          if (result?.reason === 'ultra_required') {
            showAlert('info', 'Plan Ultra Nova requerido', 'Este reset es exclusivo de Ultra Nova.');
            switchTab('subscriptions');
            return;
          }
          showAlert('error', 'No se pudo recargar', 'Inténtalo de nuevo en unos segundos.');
          return;
        }

        showAlert('success', 'Recarga aplicada', `${gameName} quedó disponible. -${cost} Ecoxionums.`);
        await this.renderMiniGamesSection();
      },

      stopMiniGameRealtimeTicker() {
        if (this.miniGameTicker) {
          clearInterval(this.miniGameTicker);
          this.miniGameTicker = null;
        }
      },

      startMiniGameRealtimeTicker() {
        this.stopMiniGameRealtimeTicker();
        this.miniGameTicker = setInterval(async () => {
          if (this.miniGameTickBusy) return;
          this.miniGameTickBusy = true;
          try {
            await this.refreshMiniGameRealtimeUI();
          } finally {
            this.miniGameTickBusy = false;
          }
        }, 1000);
      },

      async refreshMiniGameRealtimeUI() {
        const container = document.getElementById('dr-minigames-container');
        if (!container || currentTab !== 'rewards') {
          this.stopMiniGameRealtimeTicker();
          return;
        }
        ...
        const state = await loadState();
        const planId = this.getMiniGamePlanFromState(state);
        const playLimit = this.getMiniGamePlayLimit(planId);
        const miniGamesData = state.dailyRewards?.miniGames || {};

        let requiresFullRender = false;
        container.querySelectorAll('[data-minigame-card="1"]').forEach((card) => {
          const gameId = card.getAttribute('data-game-id');
          const cd = this.getMiniGameCooldown(miniGamesData[gameId] || {}, planId);
          const plays = cd.plays;
          const progress = (plays / playLimit) * 100;
          const wasLocked = card.getAttribute('data-locked') === '1';

          if (wasLocked !== cd.locked) {
            requiresFullRender = true;
            return;
          }

          card.setAttribute('data-locked', cd.locked ? '1' : '0');
          const statusEl = card.querySelector('.dr-mini-status');
          const playsEl = card.querySelector('.dr-mini-plays');
          const progressEl = card.querySelector('.dr-mini-progress');
          const cooldownTimeEl = card.querySelector('.dr-cooldown-time-live');
          const pillEl = card.querySelector('.dr-cooldown-pill-text');

          if (statusEl) statusEl.textContent = cd.locked ? 'AGOTADO' : 'DISPONIBLE';
          if (statusEl) statusEl.style.color = cd.locked ? 'var(--warn)' : 'var(--ok)';
          if (playsEl) playsEl.textContent = `${plays}/${playLimit}`;
          if (progressEl) progressEl.style.width = `${progress}%`;
          if (cooldownTimeEl) cooldownTimeEl.textContent = cd.remainingLabel;
          if (pillEl) pillEl.textContent = `⏱️ Recarga: ${cd.remainingLabel}`;
        });
      ...
      async renderMiniGamesSection() {
        ...
        const playLimit = this.getMiniGamePlayLimit(planId);
        const resetCost = this.getMiniGameResetCost();
        const isUltraPlan = normalizeEcoxionPlanId(planId) === 'ultra';
        ...
        if (!isUltraPlan) {
          ...
          upsell.style.cssText = 'grid-column:1/-1; padding:14px 16px; border-radius:14px; border:1px solid rgba(255,184,77,0.35); background:linear-gradient(90deg, rgba(255,184,77,0.12), rgba(255,140,64,0.08)); color:#ffd08a; display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap;';
          upsell.innerHTML = `
            <div style="font-size:13px; line-height:1.35;">
              <b>Tip Ultra Nova:</b> puedes recargar minijuegos al instante por ${resetCost} Ecoxionums cuando se agotan.
            </div>
            <button data-mini-upgrade-banner="1" style="border-radius:10px; padding:8px 12px; border:1px solid rgba(255,255,255,0.25); background:rgba(255,255,255,0.08); color:var(--ink-0); font-weight:700; cursor:pointer;">Suscribirme</button>
          `;
          container.appendChild(upsell);
          const upsellBtn = upsell.querySelector('[data-mini-upgrade-banner]');
          if (upsellBtn) upsellBtn.addEventListener('click', () => switchTab('subscriptions'));
        }

        Object.values(miniGamesConfig).forEach(game => {
          const gameData = miniGamesData[game.id] || {};
          const cooldown = this.getMiniGameCooldown(gameData, planId);
          ...
          card.setAttribute('data-minigame-card', '1');
          card.setAttribute('data-game-id', game.id);
          card.setAttribute('data-locked', isLocked ? '1' : '0');
          ...
          card.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:start;">
              ...
              <div style="text-align:right;">
                <span style="font-size:10px; text-transform:uppercase; font-weight:700; color:var(--ink-2); letter-spacing:1px;">Estado · ${cooldown.windowLabel}</span>
                <div class="dr-mini-status" style="font-size:12px; font-weight:700; color:${isLocked ? 'var(--warn)' : 'var(--ok)'}">
                  ${isLocked ? 'AGOTADO' : 'DISPONIBLE'}
                </div>
                ${isLocked ? `<div class="dr-cooldown-time dr-cooldown-time-live">${cooldown.remainingLabel}</div>` : ''}
              </div>
            </div>
            <div>
            ...
            <div style="margin-top:auto;">
              <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:6px;">
                ...
                <span class="dr-mini-plays" style="font-weight:700; color:var(--ink-0);">${plays}/${playLimit}</span>
              </div>
              <div style="height:6px; background:rgba(255,255,255,0.05); border-radius:3px; overflow:hidden;">
                <div class="dr-mini-progress" style="height:100%; width:${progress}%; background:linear-gradient(90deg, var(--accent), var(--accent-2)); transition:width 0.5s ease;"></div>
              </div>
            </div>
            ${isLocked ? `
              <div style="margin-top:8px; position:relative; z-index:3;">
                ${isUltraPlan ? `
                  <button data-mini-reset="${game.id}" style="width:100%; border:none; border-radius:12px; padding:10px 12px; font-weight:700; color:#07121f; background:linear-gradient(90deg,#41f3ff,#7ef9ff); cursor:pointer;">
                    âš¡ Recargar ahora Â· ${resetCost} Ecoxionums
                  </button>
                ` : `
                  <div style="padding:10px 12px; border-radius:12px; background:rgba(255,184,77,0.08); border:1px solid rgba(255,184,77,0.28); color:#ffd08a; font-size:12px; line-height:1.35;">
                    Reset instantáneo disponible con <b>Ultra Nova</b>. Puedes recargar minijuegos agotados por ${resetCost} Ecoxionums.
                  </div>
                  <button data-mini-upgrade="ultra" style="margin-top:8px; width:100%; border-radius:12px; padding:9px 12px; font-weight:700; color:var(--ink-0); background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.18); cursor:pointer;">
                    Ver Ultra Nova
                  </button>
                `}
              </div>
            ...
            ${isLocked ? `
              <div class="dr-cooldown-overlay">
                <div class="dr-cooldown-pill">
                  ...
                  <span class="dr-cooldown-pill-text">⏱️ Recarga: ${cooldown.remainingLabel}</span>
                </div>
              </div>
            ` : ''}
          `;
          ...
          const resetBtn = card.querySelector('[data-mini-reset]');
          if (resetBtn) {
            resetBtn.addEventListener('click', async (ev) => {
              ev.preventDefault();
              ev.stopPropagation();
              await this.promptMiniGameReset(game.id);
            });
          }
          const upgradeBtn = card.querySelector('[data-mini-upgrade]');
          if (upgradeBtn) {
            upgradeBtn.addEventListener('click', (ev) => {
              ev.preventDefault();
              ev.stopPropagation();
              switchTab('subscriptions');
            });
        ...
        this.startMiniGameRealtimeTicker();
      },
      ...
      async playClickGame() {
        ...
        if (gameData?.cooldown?.locked) {
          showAlert('warning', 'Límite alcanzado', `Disponible en ${gameData.cooldown.remainingLabel} (${gameData.cooldown.windowLabel}).`);
          return;
        }
        ...
        const clickBtn = overlay.querySelector('#cg-clickBtn');
        const hitArea = overlay.querySelector('#cg-hitArea');
        const clicksEl = overlay.querySelector('#cg-clicks');
        const timeEl = overlay.querySelector('#cg-time');
        const processingEl = overlay.querySelector('#cg-processing');
        const cancelBtn = overlay.querySelector('#cg-cancelBtn');
        ...
        ['click', 'pointerdown', 'touchstart'].forEach((evtName) => {
          const options = { passive: evtName !== 'touchstart' };
          hitArea.addEventListener(evtName, handleClickHit, options);
          clickBtn.addEventListener(evtName, handleClickHit, options);
        });
        ...
        const interval = setInterval(async () => {
          ...
          if (timeLeft <= 0) {
            ...
            const playReg = await this.registerMiniGamePlay('clickGame');
            if (playReg?.blocked) {
              overlay.remove();
              showAlert('warning', 'Límite alcanzado', `Disponible en ${playReg.cooldown?.remainingLabel || 'pronto'} (${playReg.cooldown?.windowLabel || ''}).`);
              this.renderMiniGamesSection();
              return;
            }
            ...
            setTimeout(() => {
              overlay.remove();
              showAlert('success', '¡Completado!', `${clicks} clicks = +${reward} <span class="icon-ecoxionum"></span>`);
              this.renderMiniGamesSection();
            }, 800);
          }
        }, 1000);
      ...
      async playMemoryGame() {
        ...
        overlay.innerHTML = `
          <div class="minigame-modal" style="max-width:500px;">
            <h2 style="margin:0 0 10px;">ðŸ§  Memoria</h2>
        ...
        const grid = overlay.querySelector('#mg-grid');
        const movesEl = overlay.querySelector('#mg-moves');
        const processingEl = overlay.querySelector('#mg-processing');
        const cancelBtn = overlay.querySelector('#mg-cancelBtn');
            ...
            if (flipped.length === 2) {
              ...
              if (flipped[0].dataset.emoji === flipped[1].dataset.emoji) {
                ...
                if (matched === cards.length) {
                  ...
                  const playReg = await this.registerMiniGamePlay('memoryGame');
                  if (playReg?.blocked) {
                    overlay.remove();
                    showAlert('warning', 'Límite alcanzado', `Disponible en ${playReg.cooldown?.remainingLabel || 'pronto'} (${playReg.cooldown?.windowLabel || ''}).`);
                    this.renderMiniGamesSection();
                  ...
                  setTimeout(() => {
                    overlay.remove();
                    showAlert('success', '¡Completado!', `${moves} movimientos = +${reward} <span class="icon-ecoxionum"></span>`);
                    this.renderMiniGamesSection();
                  }, 800);
                }
              } else {
      ...
      async playNumberGuess() {
        ...
        overlay.innerHTML = `
          <div class="minigame-modal">
            <h2 style="margin-bottom:10px;">�x}� Adivina el Número</h2>
            <p>Entre 1 y 100</p>
        ...
        const input = overlay.querySelector('#ng-input');
        const hint = overlay.querySelector('#ng-hint');
        const btn = overlay.querySelector('#ng-guessBtn');
        const processingEl = overlay.querySelector('#ng-processing');
        const cancelBtn = overlay.querySelector('#ng-cancelBtn');
        ...
        const guess = async () => {
          ...
          if (num === target) {
            ...
            const playReg = await this.registerMiniGamePlay('numberGuess');
            if (playReg?.blocked) {
              overlay.remove();
              showAlert('warning', 'Límite alcanzado', `Disponible en ${playReg.cooldown?.remainingLabel || 'pronto'} (${playReg.cooldown?.windowLabel || ''}).`);
              this.renderMiniGamesSection();
            ...
            setTimeout(() => {
              overlay.remove();
              showAlert('success', '¡Correcto!', `${attempts} intentos = +${reward} <span class="icon-ecoxionum"></span>`);
              this.renderMiniGamesSection();
            }, 800);
          } else if (num < target) {
      ...
      async playMathChallenge() {
        ...
        overlay.innerHTML = `
          <div class="minigame-modal">
            <h2 style="margin-bottom:10px;">ðŸ§® Reto MatemÃ¡tico</h2>
        ...
        const problemEl = overlay.querySelector('#mc-problem');
        const optionsEl = overlay.querySelector('#mc-options');
        const scoreEl = overlay.querySelector('#mc-score');
        const timeEl = overlay.querySelector('#mc-time');
        const processingEl = overlay.querySelector('#mc-processing');
        const cancelBtn = overlay.querySelector('#mc-cancelBtn');
        const startBtn = overlay.querySelector('#mc-startBtn');
        ...
        const startMathGame = () => {
          ...
          const interval = setInterval(async () => {
            ...
            if (timeLeft <= 0) {
              ...
              const playReg = await this.registerMiniGamePlay('mathChallenge');
              if (playReg?.blocked) {
                overlay.remove();
                showAlert('warning', 'Límite alcanzado', `Disponible en ${playReg.cooldown?.remainingLabel || 'pronto'} (${playReg.cooldown?.windowLabel || ''}).`);
                this.renderMiniGamesSection();
              ...
              setTimeout(() => {
                overlay.remove();
                showAlert('success', '¡Tiempo!', `${score} correctas = +${reward} <span class="icon-ecoxionum"></span>`);
                this.renderMiniGamesSection();
              }, 800);
            }
          }, 1000);
      ...
      async playReactionTest() {
        ...
        const area = overlay.querySelector('#rt-area');
        const h2 = area.querySelector('h2');
        const processingEl = overlay.querySelector('#rt-processing');
        const cancelBtn = overlay.querySelector('#rt-cancelBtn');
        let startTime = 0;
        let waiting = true;
        let timeout;
        let hardTimeout;
        ...
        area.addEventListener('click', async (e) => {
          ...
          if (waiting) {
            ...
            const playReg = await this.registerMiniGamePlay('reactionTest');
            if (playReg?.blocked) {
              overlay.remove();
              showAlert('warning', 'Límite alcanzado', `Disponible en ${playReg.cooldown?.remainingLabel || 'pronto'} (${playReg.cooldown?.windowLabel || ''}).`);
              this.renderMiniGamesSection();
            ...
            setTimeout(() => {
              overlay.remove();
              showAlert('success', '¡Rápido!', `${time}ms = +${Math.max(5, reward)} <span class="icon-ecoxionum"></span>`);
              this.renderMiniGamesSection();
            }, 800);
          }
        });
      ...
      async playSimonSays() {
        ...
        overlay.innerHTML = `
          <div class="minigame-modal">
            <h2 style="margin-bottom:10px;">ðŸŽ¹ Simon Dice</h2>
        ...
        const msgEl = overlay.querySelector('#ss-msg');
        const roundEl = overlay.querySelector('#ss-round');

        const playSequence = async () => {
          if (isGameOver) return;
          playing = true;
          if (msgEl) msgEl.textContent = 'Observa la secuencia...';
          sequence.push(Math.floor(Math.random() * 4));

          for (let i = 0; i < sequence.length; i++) {
            if (isGameOver) return;
            await new Promise(r => setTimeout(r, 600));
            const btn = overlay.querySelector(`#sb-${sequence[i]}`);
            if (!btn) return;
            btn.style.opacity = '1';
          ...
          playing = false;
          playerSeq = [];
          if (msgEl) msgEl.textContent = '¡Tu turno!';
        };

        const btns = overlay.querySelectorAll('.simon-btn');
        const processingEl = overlay.querySelector('#ss-processing');
        const cancelBtn = overlay.querySelector('#ss-cancelBtn');
        ...
        btns.forEach((btn, i) => {
          ...
          btn.addEventListener('click', async () => {
            ...
            if (playerSeq[playerSeq.length - 1] !== sequence[playerSeq.length - 1]) {
              ...
              cancelBtn.style.display = 'none';
              if (msgEl) {
                msgEl.textContent = '¡Secuencia incorrecta!';
                msgEl.style.color = 'var(--warn)';
              }

              const reward = Math.max(10, (round - 1) * 5);
              const playReg = await this.registerMiniGamePlay('simonSays');
              if (playReg?.blocked) {
                overlay.remove();
                showAlert('warning', 'Límite alcanzado', `Disponible en ${playReg.cooldown?.remainingLabel || 'pronto'} (${playReg.cooldown?.windowLabel || ''}).`);
                this.renderMiniGamesSection();
              ...
              setTimeout(() => {
                overlay.remove();
                showAlert('info', 'Fin del juego', `Llegaste a la ronda ${round}. +${reward} <span class="icon-ecoxionum"></span>`);
                this.renderMiniGamesSection();
              }, 800);
              return;
            }
            ...
            if (playerSeq.length === sequence.length) {
              playing = true;
              if (msgEl) msgEl.textContent = '¡Bien hecho!';
              round++;
      ...
      async giveReward(amount, source) {
        try {
          ...
          if (missionReward > 0) {
            setTimeout(() => {
              showAlert('success', '¡Misión Completada!', `Has jugado 3 mini-juegos. Recompensa: +${missionReward} <span class="icon-ecoxionum"></span>`);
            }, 500);
          }
        } catch (error) {
    ...
    let dailyRewardsUiInterval = null;
    let dailyMissionRuntimeInterval = null;
    function initDailyRewards() {
      dailyRewards.updateDailyUI();
      dailyRewards.startMiniGameRealtimeTicker();

      if (dailyRewardsUiInterval) clearInterval(dailyRewardsUiInterval);
      dailyRewardsUiInterval = setInterval(() => {
        dailyRewards.updateDailyUI();
      }, 60000);
    }

    function startDailyMissionRuntime() {
      if (dailyMissionRuntimeInterval) clearInterval(dailyMissionRuntimeInterval);
      dailyMissionRuntimeInterval = setInterval(() => {
        if (typeof dailyRewards?.trackEcoxionTime === 'function') {
          dailyRewards.trackEcoxionTime(60).catch(() => { });
        }
      }, 60000);
    }
      ...
      <div class="missions-footer">
        ...
        ${allComplete ? `
          <button id="awaken-btn-fragmented" class="awaken-button-fragmented" onclick="triggerAwakening('${extId}');">
            âœ¨ DESPERTAR EXTENSIÃ“N
          </button>
        ` : `
          <div style="display:flex;gap:12px;justify-content:center;margin-top:20px;">
            <button class="btn" onclick="refreshMissionsModal('${extId}');" style="background:rgba(100,167,255,.2);border:1px solid var(--accent);color:var(--accent);">ðŸ”„ Actualizar</button>
  ...
  `;
    ...
    function showAlert(type, title, message, duration = 5000) {
      ...
      const safeTitle = normalizeUiText(String(title || ''), '');
      const safeMessage = normalizeUiText(String(message || ''), '');
  ...
  `;
    ...
    // Normaliza texto remoto/local para evitar mojibake y HTML crudo visible en UI.
    ...
    function decodeLikelyCp1252Utf8(input) {
    ...
    function normalizeUiText(value, fallback = '') {
      ...
      let fixed = raw
        .replace(/<span class="icon-ecoxionum"><\/span>/gi, '🪙')
        .replace(/<\/?span[^>]*>/gi, '')
        .replace(/<\/?div[^>]*>/gi, '')
        .replaceAll('&nbsp;', ' ');

      const suspicious = /[\u00C3\u00C2\u00E2\u00F0\uFFFD]|Ã|Â|â|ð|�|\?\?/.test(fixed);
      if (suspicious) {
        const map = {
          ...
          'Ã±': 'ñ', 'Ã‘': 'Ñ', 'Â¿': '¿', 'Â¡': '¡',
          'â€¢': '•', 'â€“': '–', 'â€”': '—', 'â€™': '’', 'â€œ': '“', 'â€': '”', 'â€¦': '…',
          'âœ…': '✅', 'âœ•': '✕', 'âœ“': '✓', 'âš ï¸': '⚠️', 'âš¡': '⚡', 'â­': '⭐', 'âœ¨': '✨',
          'ðŸ“ˆ': '📈', 'ðŸ›’': '🛒', 'ðŸ’³': '💳', 'ðŸ§©': '🧩', 'ðŸ“¦': '📦', 'ðŸŽ': '🎁', 'âš™ï¸': '⚙️',
          'ðŸ‘¤': '👤', 'ðŸŒŒ': '🌌', 'ðŸŒ¿': '🌿', 'ðŸï¸': '🏝️', 'ðŸ”': '🔐', 'ðŸ”—': '🔗',
          'ðŸ—‘ï¸': '🗑️', 'ðŸ”¥': '🔥', 'ðŸŽ‰': '🎉', 'ðŸ“±': '📱', 'ðŸ“Š': '📊',
          'ðŸŽ²': '🎲', 'ðŸ–±ï¸': '🖱️', 'â›ï¸': '⛏️', 'â¹ï¸': '⏹️', 'ðŸ†': '🏆',
          'ðŸª™': '🪙'
        };
        Object.keys(map).forEach((k) => {
          if (fixed.includes(k)) fixed = fixed.split(k).join(map[k]);
        });
        try {
          const decoded = decodeURIComponent(escape(fixed));
          if (decoded && decoded.trim()) fixed = decoded;
        } catch { }
        const cpDecoded = decodeLikelyCp1252Utf8(fixed);
        const badBefore = (fixed.match(/[ÃÂâð�]/g) || []).length;
        const badAfter = (cpDecoded.match(/[ÃÂâð�]/g) || []).length;
        if (badAfter < badBefore) fixed = cpDecoded;
      }

      fixed = fixed
        .replaceAll('extensi?n', 'extensión')
        .replaceAll('Extensi?n', 'Extensión')
        .replaceAll('configuraci?n', 'configuración')
        .replaceAll('Configuraci?n', 'Configuración')
        .replaceAll('sesi?n', 'sesión')
        .replaceAll('Sesi?n', 'Sesión')
        .replaceAll('T?tulo', 'Título')
        .replaceAll('?V2 disponible', '✨ V2 disponible')
        .replaceAll('?? Predeterminada', '🛡️ Predeterminada')
        .replaceAll('?? Activa', '🔵 Activa')
        .replaceAll('?? Iniciar Sesi?n', '🔐 Iniciar Sesión')
        .replaceAll('?? Registrarse y Vincular', '📝 Registrarse y Vincular')
        .replaceAll('?? Cerrar Sesi?n', '🚪 Cerrar Sesión')
        .replaceAll('? Vinculado', '✓ Vinculado')
        .replace(/(\d+)\s+\?\?/g, '$1 🪙');
      return fixed;
    }
    ...
    function normalizeVisibleMojibake(root = document.body) {
      ...
      while ((node = walker.nextNode())) {
        ...
        const fixed = normalizeUiText(original, original);
      ...
      if (root.querySelectorAll) {
        root.querySelectorAll('[title],[placeholder],[aria-label]').forEach((el) => {
          ['title', 'placeholder', 'aria-label'].forEach((attr) => {
            ...
            if (!val) return;
            const fixed = normalizeUiText(val, val);
    ...
    function startUiNormalizationObserver() {
      if (window.__ecoMojibakeObserverStarted) return;
      window.__ecoMojibakeObserverStarted = true;
      ...
      queue();
      window.__ecoMojibakeObserver = observer;
    }
    ...
    /* =========================
       ...
       ========================= */
    /* =========================
       NAVEGACI�N (Rediseño Estilo Zen)
       ========================= */
    ...
    <div class="normal-appbar-shell">
      <div class="normal-top-grid">
        <div class="normal-brand-wrap">
          <div class="normal-logo-box">E</div>
          <div class="normal-title-block">
            <h1>ECOXION</h1>
            <div class="normal-title-sub" id="brand-subtitle">Conectado</div>
          </div>
        </div>

        <div class="normal-user-wrap">
          <div id="user-info" class="normal-user-pill" style="border-color: rgba(100, 167, 255, 0.3); background: rgba(100, 167, 255, 0.08);">
            <span class="avatar">ðŸ‘¤</span>
            <span class="normal-user-meta">
              <span id="username-display" class="normal-user-name" style="color: var(--accent);">OceanandWild</span>
              <span id="username-state" class="normal-user-state">Cuenta local</span>
            </span>
          </div>
          <div class="normal-balance-pill">
            <span style="font-size:1rem;"><span class="icon-ecoxionum"></span></span>
            <span class="eco-balance-value" data-role="eco-balance">1733</span>
          </div>
        </div>
      </div>
  ...
  `;
    ...
    <div style="padding:28px 16px;border-bottom:2px solid rgba(100, 167, 255, 0.2);position:relative;overflow:hidden;background:linear-gradient(180deg, rgba(15,23,42,0.8), rgba(30,41,59,0.6));">
      ...
      <div class="brand" style="flex-direction:column;align-items:center;position:relative;z-index:1;gap:12px;">
        <div class="logo-container" style="width:72px;height:72px;">
          <div class="logo-ring" style="width:72px;height:72px;">
            <img src="assets/ecoxion.png" alt="Ecoxion Logo" class="logo-image" onerror="this.style.display='none';this.nextElementSibling.style.display='block';" style="display: none;">
            <div class="logo-core" style="display: block;"></div>
          </div>
        </div>
        <div style="text-align:center;">
          <h1 class="brand-title" style="font-size:22px;margin:0 0 8px;letter-spacing:1.5px;">ECOXION</h1>
          <div class="brand-subtitle" id="zen-brand-subtitle" style="font-size:11px;opacity:0.8;display:flex;align-items:center;justify-content:center;gap:6px;">Conectado</div>
        </div>
      </div>
    </div>
    
    <div style="padding:16px 12px 8px;border-bottom:1px solid rgba(255,255,255,.05);">
      <div id="zen-user-info" style="display: flex; align-items: center; gap: 10px; padding: 10px 12px; background: rgba(100, 167, 255, 0.1); border-radius: 10px; border: 1px solid rgba(100, 167, 255, 0.3); margin-bottom: 8px;">
        <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg, var(--accent), var(--accent-2));display:flex;align-items:center;justify-content:center;font-size:16px;box-shadow:0 2px 8px rgba(100,167,255,.3);">ðŸ‘¤</div>
        <div style="flex:1;min-width:0;">
          <div id="zen-username" style="font-size:13px;font-weight:600;color:var(--ink-0);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">OceanandWild</div>
          <div style="font-size:11px;color:var(--ink-1);opacity:0.7;">Usuario</div>
        </div>
      </div>
    </div>
    <nav class="tabs" data-role="main-tabs" style="padding:10px 12px 6px;display:flex;flex-direction:column;gap:6px;"></nav>
    ...
    <div style="padding:16px 12px;border-top:2px solid rgba(100, 167, 255, 0.15);position:relative;overflow:hidden;">
      ...
      <div class="ecoxionums-balance" style="position:relative;z-index:1;">
        ...
        <div style="display:flex;flex-direction:column;align-items:center;gap:4px;">
          <span class="eco-balance-value" data-role="eco-balance" style="font-size:18px;">1733</span>
  ...
  `;
    ...
    function renderTabs() {
      ...
      const pillsMarkup = tabs.map(tab => `
    <button class="nav-pill" data-tab="${tab.id}" aria-selected="false" style="position:relative;overflow:hidden;padding:${isZen ? '10px 12px' : '8px 16px'}; border-radius:${isZen ? '12px' : '20px'}; border:1px solid transparent; background:rgba(255,255,255,.03); color:var(--ink-1); font-size:13px; font-weight:600; display:flex; align-items:center; gap:8px; cursor:pointer; transition:all 0.2s ease; white-space:nowrap; width:${isZen ? '100%' : 'auto'};">
  ...
  `).join('');
      ...
      tabsContainers.forEach((container) => {
        container.innerHTML = pillsMarkup;
        ...
        const updatePillStyles = (selectedId) => {
          ...
          if (isActive) {
            btn.style.background = isZen
              ? 'linear-gradient(135deg, rgba(100,167,255,.2), rgba(179,136,255,.14))'
              : 'rgba(100,167,255,.15)';
            btn.style.color = 'var(--accent)';
            btn.style.borderColor = isZen ? 'rgba(100,167,255,.5)' : 'rgba(100,167,255,.3)';
            btn.style.boxShadow = isZen ? 'none' : '0 2px 8px rgba(0,0,0,0.2)';
          } else {
            btn.style.background = 'rgba(255,255,255,.03)';
            btn.style.color = 'var(--ink-1)';
            btn.style.borderColor = isZen ? 'rgba(255,255,255,.08)' : 'transparent';
            btn.style.boxShadow = 'none';
          }
          const indicator = btn.querySelector('.active-indicator');
          if (indicator) indicator.style.width = isZen ? '0' : (isActive ? '72%' : '0');
        });
        };
    ...
    function switchTab(tabId) {
      if (tabId !== 'rewards' && typeof dailyRewards?.stopMiniGameRealtimeTicker === 'function') {
        dailyRewards.stopMiniGameRealtimeTicker();
      }
      currentTab = tabId;
      ...
      setTimeout(() => {
        if (typeof normalizeVisibleMojibake === 'function') normalizeVisibleMojibake();
      }, 0);
    }
    ...
    <div style="display:flex;flex-wrap:wrap;gap:16px;align-items:flex-start;justify-content:space-between;">
      ...
      <div style="display:flex;gap:12px;flex-wrap:wrap;">
        <div style="min-width:130px;padding:10px 12px;border-radius:10px;background:rgba(100,167,255,.08);border:1px solid rgba(100,167,255,.4);">
          ...
          <div style="font-size:18px;font-weight:700;color:var(--accent);" id="dashboard-eco-balance">1733</div>
        </div>
  ...
  `;
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,.1);">
        <div style="font-size:32px;position:relative;width:48px;height:48px;">
          ...
          ${extInfo.awakened ? '<span style="position:absolute;top:-8px;right:-8px;font-size:16px;filter:drop-shadow(0 0 8px rgba(100,167,255,0.8));">âœ¨</span>' : ''}
        </div>
        <div style="flex:1;">
        ...
        (async () => {
          ...
          if (panelContent) {
            try {
              ...
              if (typeof dailyRewards?.trackOpenedExtension === 'function') {
                dailyRewards.trackOpenedExtension(extInfo.id).catch(() => { });
              }
            } catch (e) {
        ...
        if (isZen) {
          ...
          const actionButtons = `
            ...
            ${ext.hasV2 && !ext.awakened ? `
              <button class="btn" onclick="showMissionsModal('${ext.id}')" style="background:linear-gradient(135deg, #a855f7, #6366f1); border:none; color:white; font-size:12px; padding:6px 12px;">âœ¨ V2</button>
            ` : ''}
          `;
          ...
          item.innerHTML = `
            ...
            <div class="item-info">
              ...
              <div class="item-meta">
                ...
                <span style="font-size:12px; color:var(--ink-1); font-weight:600;">${ext.type === 'passive' ? 'âš¡ Pasiva' : '?? Activa'}</span>
          ...
          item.innerHTML = `
            ...
            <div style="display:flex; align-items:start; gap:20px; padding-top:8px;">
              <div style="font-size:48px; position:relative; flex-shrink:0; width:64px; height:64px;">
                ...
                ${ext.awakened ? '<span style="position:absolute; top:-4px; right:-4px; font-size:20px; filter:drop-shadow(0 0 8px rgba(100,167,255,0.8));">âœ¨</span>' : ''}
              </div>
              <div style="flex:1; min-width:0;">
                <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px; flex-wrap:wrap;">
                  ...
                  <span style="padding:4px 10px; background:rgba(100,167,255,.15); border:1px solid rgba(100,167,255,.3); border-radius:8px; font-size:11px; font-weight:600; color:var(--accent); text-transform:uppercase;">
                    ${ext.type === 'passive' ? 'âš¡ Pasiva' : '?? Activa'}
                  </span>
                  ${ext.enabled ?
              '<span style="padding:4px 10px; background:rgba(16,185,129,.15); border:1px solid rgba(16,185,129,.3); border-radius:8px; font-size:11px; font-weight:600; color:var(--ok);">âœ“ Habilitada</span>' :
              '<span style="padding:4px 10px; background:rgba(239,68,68,.15); border:1px solid rgba(239,68,68,.3); border-radius:8px; font-size:11px; font-weight:600; color:var(--danger);">âœ— Deshabilitada</span>'
            }
                </div>
                ...
                <div style="display:flex; gap:10px; flex-wrap:wrap;">
                  ${ext.type === 'passive' ? `
                    <button class="btn" onclick="toggleExtension('${ext.id}')" style="background:${ext.enabled ? 'var(--warn)' : 'var(--ok)'}; border-color:${ext.enabled ? 'var(--warn)' : 'var(--ok)'}; min-width:140px;">
                      ${ext.enabled ? 'â¸ï¸ Deshabilitar' : 'â–¶ï¸ Habilitar'}
                    </button>
                  ` : `
                    <button class="btn primary" onclick="switchTab('dashboard')" style="min-width:140px;">ðŸ“± Abrir Panel</button>
                  `}
                  ${!ext.defaultInstall ? `
                    <button class="btn" onclick="uninstallExtension('${ext.id}')" style="background:var(--danger); border-color:var(--danger); min-width:140px;">ðŸ—‘ï¸ Desinstalar</button>
                  ` : `
                  ...
                  `}
                  ${ext.hasV2 && !ext.awakened ? `
                    <button class="btn" onclick="showMissionsModal('${ext.id}')" style="background:linear-gradient(135deg, rgba(100,167,255,.2), rgba(179,136,255,.2)); border:1px solid var(--accent); color:var(--accent); min-width:140px;">âœ¨ Despertar V2</button>
                  ` : ''}
                </div>
              </div>
            </div>
    ...
    <div class="eco-rewards-shell">
      <div class="eco-rewards-side">
        ...
        <div class="card">
          <div class="dr-reward-card">
            <div style="font-size:3em;margin-bottom:8px;">ðŸŽ</div>
            ...
            <div class="dr-reward-amount" id="dr-dailyAmount">+80 <span class="icon-ecoxionum"></span></div>
            <button class="dr-claim-btn" id="dr-claimDailyBtn" onclick="dailyRewards.claimDaily()">ðŸŽ Reclamar</button>
            <div id="dr-dailyTimer" style="margin-top:12px;color:var(--ink-1);font-size:0.9em;"></div>
          </div>
        </div>
        <div class="card">
          <h3 style="margin:0 0 12px;color:var(--ink-0);">ðŸ“Š Estadisticas</h3>
      ...
      <div class="eco-rewards-main">
        <div class="card" style="margin-bottom:20px;">
          <h3 style="margin:0 0 12px;color:var(--ink-0);">ðŸ“‹ Misiones Diarias</h3>
          <div class="dr-missions-grid" id="dr-missionsList"></div>
        </div>
        <div class="card" style="margin-bottom:20px;">
          <h3 style="margin:0 0 12px;color:var(--ink-0);">ðŸŽ® Minijuegos</h3>
  ...
  `;
      ...
      if (!document.getElementById('daily-reward-style-fix')) {
        ...
        style.textContent = `
          ...
          .dr-cooldown-overlay { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,0.45); backdrop-filter: blur(2px); pointer-events: none; z-index: 2; }
      ...
      content.innerHTML = `
        <div class="card" style="margin-bottom:24px;text-align:center;background:linear-gradient(135deg, rgba(100,167,255,.1), rgba(179,136,255,.1));border:1px solid var(--accent);">
          ...
          <p style="color:var(--ink-1);max-width:600px;margin:0 auto 24px;">
            Adquiere paquetes sorpresa y obtén extensiones aleatorias. ¡Puedes conseguir extensiones �0picas o Legendarias!
          </p>
          <div style="display:inline-flex;align-items:center;gap:8px;padding:8px 16px;background:rgba(0,0,0,.3);border-radius:20px;border:1px solid rgba(255,255,255,.1);">
            <span>Tu saldo:</span>
            <span style="font-weight:700;color:var(--ok);font-size:18px;">${(state.ecoxionums || 0).toLocaleString()} <span class="icon-ecoxionum"></span></span>
          </div>
        </div>
          ...
          ${Object.values(packagesConfig).map(pkg => `
            <div class="package-card" onclick="buyPackage('${pkg.id}')">
              ...
              <div style="display:flex;justify-content:center;gap:4px;margin-bottom:16px;">
                ...
                ${pkg.probabilities.epica > 0 ? `<span title="Ã‰pica: ${pkg.probabilities.epica}%" style="width:8px;height:8px;border-radius:50%;background:#ffd700;"></span>` : ''}
              ...
              <div class="package-price">
                ${pkg.price} <span class="icon-ecoxionum"></span>
              </div>
            </div>
          `).join('')}
        </div>
      ...
      function triggerOpen() {
        ...
        setTimeout(async () => {
          ...
          if (!state.installed[reward.id]) {
            ...
            msg.innerHTML = `<p style="color:var(--ok);margin-top:12px;">¡Ya la tenías! Compensación: +${compensation} <span class="icon-ecoxionum"></span></p>`;
      ...
      Object.values(defaultExtensions).forEach(ext => {
        ...
        if (isZen) {
          ...
          const actionButton = isInstalled && extInfo.enabled
            ? hasV2Available
              ? `<button class="btn" onclick="showMissionsModal('${ext.id}')" style="background:linear-gradient(135deg, #a855f7, #6366f1); border:none; padding:8px 18px; font-size:13px; font-weight:700;">Despertar âœ¨</button>`
          ...
          item.innerHTML = `
            ...
            <div class="item-info">
              ...
              `<span style="font-weight:700; color:var(--accent); font-size:14px;">${ext.price} <span class="icon-ecoxionum"></span></span>` :
              `<span style="color:var(--ok); font-weight:600; font-size:14px;">${isInstalled ? 'âœ“ Instalada' : 'Gratis'}</span>`
            }
              </div>
            </div>
            <div class="item-actions">
          ...
          item.innerHTML = `
            ...
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-top: 10px;">
                ...
                <div style="display:flex; flex-direction:column; align-items:flex-end; gap:6px;">
                   ...
                   ${extInfo.awakened ? '<span style="font-size:10px; font-weight:700; color:#10b981; padding:2px 6px; background:rgba(16,185,129,0.1); border-radius:4px;">âœ¨ V2</span>' : ''}
                </div>
            </div>
            <div style="flex:1;">
            ...
            ${hasV2Available ? `
              <div style="padding:10px; background:linear-gradient(90deg, rgba(255,215,0,0.1), transparent); border-left:3px solid #ffd700; border-radius:4px;">
                <p style="margin:0; font-size:12px; color:#ffd700; font-weight:600;">�S� ¡Versión V2 Disponible!</p>
              </div>
            ` : ''}
            <div style="border-top:1px solid rgba(255,255,255,0.05); padding-top:16px; display:flex; gap:12px; align-items:center;">
                <div style="flex:1;">
                   ${ext.price > 0 && !isInstalled ?
              `<div style="font-size:18px; font-weight:700; color:var(--ink-0);">${ext.price} <span style="font-size:14px; color:var(--ink-2);"><span class="icon-ecoxionum"></span></span></div>` :
              ...
              ? hasV2Available
                ? `<button class="btn" onclick="showMissionsModal('${ext.id}')" style="background:linear-gradient(135deg, #a855f7, #6366f1); border:none; padding:10px 20px;">Despertar âœ¨</button>`
    ...
    function normalizeEcoxionPlanId(planId) {
      ...
      if (['free', 'plus', 'pro', 'ultra'].includes(raw)) return raw;
      if (raw.includes('orbita') || raw.includes('orbi')) return 'free';
      if (raw.includes('nebula') || raw.includes('pulse')) return 'plus';
      if (raw.includes('quasar') || raw.includes('prime')) return 'pro';
      if (raw.includes('ultra') || raw.includes('nova')) return 'ultra';

      return 'free';
    }

    function getEcoxionPlanName(planId) {
      const key = normalizeEcoxionPlanId(planId);
      return ECOXION_PLAN_THEME[key] || ECOXION_PLAN_THEME.free;
    }

    function getEcoxionPlanRank(planId) {
      const key = normalizeEcoxionPlanId(planId);
      if (key === 'ultra') return 4;
      if (key === 'pro') return 3;
      if (key === 'plus') return 2;
      return 1;
    }

    const PLAN_ADVANTAGE_EXTENSION_CATALOG = [
      { id: 'eco-luck', name: 'Ecoxionum Fortuna', icon: 'ðŸŽ²', gradient: 'linear-gradient(135deg,#ffd86a,#ff8b42)' },
      { id: 'eco-generator', name: 'Generador Ecoxionums', icon: 'âš¡', gradient: 'linear-gradient(135deg,#8cf0ff,#4f8cff)' },
      { id: 'clicky-coin', name: 'Clicky Coin', icon: '<span class="icon-ecoxionum"></span>', gradient: 'linear-gradient(135deg,#64a7ff,#9c7dff)' },
      { id: 'eco-stock', name: 'Eco Stock Market', icon: 'ðŸ“ˆ', gradient: 'linear-gradient(135deg,#56f2b0,#3f9bff)' },
      { id: 'quick-surveys', name: 'Quick Surveys', icon: 'ðŸ“‹', gradient: 'linear-gradient(135deg,#7ee0ff,#8e9dff)' },
      { id: 'smart-notes', name: 'Smart Notes', icon: 'ðŸ“', gradient: 'linear-gradient(135deg,#91a8ff,#4cc9f0)' }
    ];

    const PLAN_ADVANTAGE_MATRIX_DEFAULT = {
      free: {
        'eco-luck': ['Tabla base de probabilidades.', 'Costo por tirada: 25 <span class="icon-ecoxionum"></span>.'],
    ...
    async function loadEcoxionPlanConfig(force = false) {
      const now = Date.now();
      if (!force && ecoxionPlanConfigCache.config && (now - ecoxionPlanConfigCache.ts) < ECOXION_PLAN_CONFIG_TTL_MS) {
        return ecoxionPlanConfigCache.config;
      }
      try {
        const res = await fetch(`${API_GLOBAL}/api/ecoxion/plans/config?nocache=${Date.now()}`, { cache: 'no-store' });
        if (res.ok) {
          const payload = await res.json();
          const cfg = payload?.config || null;
          if (cfg && typeof cfg === 'object') {
            ecoxionPlanConfigCache = { ts: now, config: cfg };
            return cfg;
          }
        }
      } catch (_err) {}
      const fallback = {
        planAdvantages: PLAN_ADVANTAGE_MATRIX_DEFAULT,
        fortuneOdds: FORTUNE_ODDS_DEFAULT
      };
      ecoxionPlanConfigCache = { ts: now, config: fallback };
      return fallback;
    }

    function getPlanAdvantageMatrix() {
      const cfg = ecoxionPlanConfigCache.config;
      return (cfg && cfg.planAdvantages && typeof cfg.planAdvantages === 'object')
        ? cfg.planAdvantages
        : PLAN_ADVANTAGE_MATRIX_DEFAULT;
    }

    function getFortuneOddsConfig() {
      const cfg = ecoxionPlanConfigCache.config;
      return (cfg && cfg.fortuneOdds && typeof cfg.fortuneOdds === 'object')
        ? cfg.fortuneOdds
        : FORTUNE_ODDS_DEFAULT;
    }

    let planAdvantageModalState = { open: false, planId: 'free', extensionId: 'eco-luck' };

    function ensurePlanAdvantageModal() {
      if (document.getElementById('plan-adv-overlay')) return;
      const wrapper = document.createElement('div');
      wrapper.innerHTML = `
        <div id="plan-adv-overlay" class="eco-adv-overlay" onclick="if(event.target===this) closePlanAdvantagesModal()">
          <div class="eco-adv-modal">
            <div class="eco-adv-header">
              <div>
                <div id="plan-adv-title" style="font-size:18px;font-weight:800;color:#e8f5ff;">Expandir Ventajas</div>
                <div id="plan-adv-subtitle" style="font-size:12px;color:var(--ink-1);">Detalle por extensión</div>
              </div>
              <button class="btn" style="padding:8px 12px;" onclick="closePlanAdvantagesModal()">Cerrar</button>
            </div>
            <div class="eco-adv-body">
              <div id="plan-adv-ext-list" class="eco-adv-ext-list"></div>
              <div id="plan-adv-detail" class="eco-adv-detail"></div>
            </div>
          </div>
        </div>
      `;
    ...
    function renderPlanAdvantagesModal() {
      ensurePlanAdvantageModal();
      const planId = normalizeEcoxionPlanId(planAdvantageModalState.planId || 'free');
      const planName = getEcoxionPlanName(planId);
      const matrixAll = getPlanAdvantageMatrix();
      const matrix = matrixAll[planId] || matrixAll.free || {};
      const extList = document.getElementById('plan-adv-ext-list');
      const detail = document.getElementById('plan-adv-detail');
      const title = document.getElementById('plan-adv-title');
      const subtitle = document.getElementById('plan-adv-subtitle');
      if (!extList || !detail || !title || !subtitle) return;

      const validExtensionIds = PLAN_ADVANTAGE_EXTENSION_CATALOG.map((x) => x.id);
      if (!validExtensionIds.includes(planAdvantageModalState.extensionId)) {
        planAdvantageModalState.extensionId = validExtensionIds[0];
      }

      title.textContent = `Expandir Ventajas · ${planName}`;
      subtitle.textContent = 'Selecciona una extensión para ver beneficios concretos del plan.';

      extList.innerHTML = PLAN_ADVANTAGE_EXTENSION_CATALOG.map((ext) => {
        const isActive = ext.id === planAdvantageModalState.extensionId;
        return `
          <button class="eco-adv-ext-item ${isActive ? 'active' : ''}" onclick="openPlanAdvantagesModal('${planId}','${ext.id}')">
            <div class="eco-adv-model" style="background:${ext.gradient};">${ext.icon}</div>
            <div style="text-align:left;">
              <div style="font-size:13px;font-weight:700;color:#e8f5ff;">${ext.name}</div>
              <div style="font-size:11px;color:var(--ink-1);">${planName}</div>
            </div>
          </button>
        `;
      }).join('');

      const selected = PLAN_ADVANTAGE_EXTENSION_CATALOG.find((x) => x.id === planAdvantageModalState.extensionId) || PLAN_ADVANTAGE_EXTENSION_CATALOG[0];
      const benefits = matrix[selected.id] || ['Sin ventaja específica documentada en este plan.'];
      detail.innerHTML = `
        <div style="display:flex;align-items:center;gap:12px;">
          <div class="eco-adv-model" style="width:52px;height:52px;font-size:26px;background:${selected.gradient};">${selected.icon}</div>
          <div>
            <div style="font-size:18px;font-weight:800;color:#e8f5ff;">${selected.name}</div>
            <div style="font-size:12px;color:var(--ink-1);">Ventajas activas en ${planName}</div>
          </div>
        </div>
        <ul class="eco-adv-benefits">
          ${benefits.map((b) => `<li>${b}</li>`).join('')}
        </ul>
      `;
    }

    window.openPlanAdvantagesModal = async function (planId, extensionId = '') {
      await loadEcoxionPlanConfig(false);
      planAdvantageModalState.open = true;
      planAdvantageModalState.planId = normalizeEcoxionPlanId(planId || 'free');
      if (extensionId) planAdvantageModalState.extensionId = extensionId;
      renderPlanAdvantagesModal();
      const overlay = document.getElementById('plan-adv-overlay');
      if (overlay) overlay.classList.add('active');
    };

    window.closePlanAdvantagesModal = function () {
      planAdvantageModalState.open = false;
      const overlay = document.getElementById('plan-adv-overlay');
    ...
    async function renderSubscriptions() {
      ...
      if (state.oceanPayLinked && state.userId) {
        try {
          ...
          if (res.ok) {
            currentSubscription = await res.json();
            const serverPlan = normalizeEcoxionPlanId(currentSubscription?.plan || 'free');
            if (serverPlan && serverPlan !== state.subscriptionPlan) {
              state.subscriptionPlan = serverPlan;
              state.ecoxionMiniGamePlan = serverPlan;
              state.ecoxionMiniGamePlanSyncedAt = Date.now();
              await saveState(state);
            }
          }
        } catch (e) {
      ...
      const activePlan = normalizeEcoxionPlanId(currentSubscription?.plan || state.subscriptionPlan || 'free');
      const plans = [
        {
          id: 'free',
          icon: 'ðŸ›°ï¸',
          title: getEcoxionPlanName('free'),
          priceLabel: 'Gratis',
          cycle: 'Sin vencimiento',
          features: ['Acceso base a extensiones', 'Funciones generales', 'Panel local completo', 'Minijuegos: 3 usos · recarga cada 24h']
        },
        {
          id: 'plus',
          icon: 'â˜„ï¸',
          title: getEcoxionPlanName('plus'),
          price: 420,
          priceLabel: '420 <span class="icon-ecoxionum"></span>',
          cycle: 'cada 30 dias',
          features: ['Extensiones mejoradas', 'Misiones premium', 'Prioridad en recompensas', 'Minijuegos: 3 usos · recarga cada 12h']
        },
        {
          id: 'pro',
          icon: 'ðŸª',
          title: getEcoxionPlanName('pro'),
          price: 750,
          priceLabel: '750 <span class="icon-ecoxionum"></span>',
          cycle: 'cada 30 dias',
          recommended: true,
          features: ['Todo Nebula Pulse', 'Acceso anticipado', 'Perfiles de rendimiento avanzados', 'Minijuegos: 5 usos · recarga cada 6h']
        },
        {
          id: 'ultra',
          icon: 'ðŸŒ ',
          title: getEcoxionPlanName('ultra'),
          price: 1250,
          priceLabel: '1250 <span class="icon-ecoxionum"></span>',
          cycle: 'cada 30 dias',
          features: ['Todo Quasar Prime', 'Toolset experto completo', 'Canal prioritario + extras visuales', 'Minijuegos: 5 usos · recarga cada 2h + reset instantáneo']
        }
      ];

      const fallbackPlan = normalizeEcoxionPlanId(state.subscriptionPlan || 'free');
      const fallbackPlanLabel = getEcoxionPlanName(fallbackPlan);
      const serverPlanLabel = getEcoxionPlanName(currentSubscription?.plan);

      const subscriptionMeta = currentSubscription ? `
        <div class="eco-sub-meta">
          <div><strong>Plan activo:</strong> ${serverPlanLabel}</div>
      ...
      ` : (fallbackPlan !== 'free'
        ? `<div class="eco-sub-meta"><div><strong>Plan activo:</strong> ${fallbackPlanLabel}</div><div>Sincronizacion con servidor pendiente. Beneficios locales activos.</div></div>`
        : `<div class="eco-sub-meta"><div><strong>Plan activo:</strong> ${getEcoxionPlanName('free')}</div><div>Sin suscripcion premium activa.</div></div>`);
        ...
        <div class="eco-subs-shell">
          ...
          <div class="eco-subs-grid">
            ${plans.map((plan) => {
              ...
              const activeRank = getEcoxionPlanRank(activePlan);
              const planRank = getEcoxionPlanRank(plan.id);
              const downgradeLocked = activeRank > 1 && planRank < activeRank;
              ...
              const badge = isActive
                ...
                : (downgradeLocked
                  ? '<span class="eco-sub-badge" style="background:#2f3947;color:#b9c6d8;">Bloqueado</span>'
                  : (plan.recommended ? '<span class="eco-sub-badge recommended">Recomendado</span>' : ''));
              const action = plan.id === 'free'
                ? (downgradeLocked
                  ? '<button class="btn" style="width:100%;opacity:.7;cursor:not-allowed;" disabled="">No disponible desde tu plan actual</button>'
              ...
              return `
                <article class="${cardClass}">
                  ...
                  <button class="eco-plan-expand-btn" onclick="openPlanAdvantagesModal('${plan.id}')">Expandir Ventajas</button>
                  ${action}
                </article>
              `;
            }).join('')}
          ...
          <div class="eco-compare">
            <div class="eco-compare-title">Comparativa de Ventajas</div>
            <div class="eco-compare-grid">
              <div class="eco-compare-cell head feature">Ventaja</div>
              <div class="eco-compare-cell head">${getEcoxionPlanName('free')}</div>
              <div class="eco-compare-cell head">${getEcoxionPlanName('plus')}</div>
              <div class="eco-compare-cell head">${getEcoxionPlanName('pro')}</div>
              <div class="eco-compare-cell head">${getEcoxionPlanName('ultra')}</div>

              <div class="eco-compare-cell feature">Misiones premium</div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>

              <div class="eco-compare-cell feature">Acceso anticipado</div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>

              <div class="eco-compare-cell feature">Recarga minijuegos</div>
              <div class="eco-compare-cell">24h</div>
              <div class="eco-compare-cell">12h</div>
              <div class="eco-compare-cell">6h</div>
              <div class="eco-compare-cell">2h</div>

              <div class="eco-compare-cell feature">Usos de minijuegos por ventana</div>
              <div class="eco-compare-cell">3</div>
              <div class="eco-compare-cell">3</div>
              <div class="eco-compare-cell">5</div>
              <div class="eco-compare-cell">5</div>

              <div class="eco-compare-cell feature">Reset instantáneo con Ecoxionums</div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>

              <div class="eco-compare-cell feature">Canal prioritario + extras visuales</div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-x">âœ•</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">âœ“</span></div>
    ...
    window.selectPlan = async function (planId) {
      ...
      const planLabels = { plus: getEcoxionPlanName('plus'), pro: getEcoxionPlanName('pro'), ultra: getEcoxionPlanName('ultra') };
      ...
      if (!planPrices[planId]) {
        showAlert('info', 'Plan base', `El plan ${getEcoxionPlanName('free')} no requiere compra.`);
        return;
      }

      const currentRank = getEcoxionPlanRank(state.subscriptionPlan || 'free');
      const targetRank = getEcoxionPlanRank(planId);
      if (targetRank < currentRank) {
        showAlert('warning', 'Cambio bloqueado', `No puedes bajar de ${getEcoxionPlanName(state.subscriptionPlan)} a ${getEcoxionPlanName(planId)}.`);
        return;
      }
      ...
      try {
        ...
        if (currentTab === 'rewards') {
          await dailyRewards.renderMiniGamesSection();
        }
    ...
    window.cancelSubscription = async function () {
      ...
      try {
        ...
        state.ecoxionMiniGamePlan = 'free';
        state.ecoxionMiniGamePlanSyncedAt = Date.now();
        await saveState(state);
        if (currentTab === 'rewards') {
          await dailyRewards.renderMiniGamesSection();
          ...
          <div style="flex:1;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
              ...
              <div style="text-align:right;">
                ...
                <div style="font-size:32px; font-weight:900; color:var(--ok); text-shadow:0 0 20px rgba(16,185,129,0.3);">${totalEcoxionums.toLocaleString()} <span class="icon-ecoxionum"></span></div>
              </div>
            </div>
        ...
        <div class="stats-grid-premium">
          ...
          <div class="stat-card-premium">
             ...
             <div style="display:flex; flex-wrap:wrap; gap:12px;">
                <div class="achievement-icon ${totalHistorical > 100 ? 'achievement-unlocked' : ''}" title="Primeros 100 XP"><span class="icon-ecoxionum"></span></div>
                <div class="achievement-icon ${activeExtensions >= 5 ? 'achievement-unlocked' : ''}" title="Coleccionista (5+ Apps)">ðŸ“±</div>
                <div class="achievement-icon ${dailyStreak >= 7 ? 'achievement-unlocked' : ''}" title="Fidelidad (Racha 7 días)">�x�</div>
                <div class="achievement-icon ${totalHistorical > 1000 ? 'achievement-unlocked' : ''}" title="Millonario Eco"><span class="icon-ecoxionum"></span></div>
                <div class="achievement-icon ${state.oceanPayLinked ? 'achievement-unlocked' : ''}" title="Ciudadano Verificado">ðŸŒŠ</div>
             </div>
        ...
        <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(400px, 1fr)); gap:24px;">
          ...
          <div class="card" style="margin:0; border-radius:24px;">
            <h3 style="margin:0 0 24px; font-size:20px; display:flex; align-items:center; gap:12px;">
              <span style="width:36px; height:36px; background:rgba(255,215,0,0.1); border-radius:10px; display:flex; align-items:center; justify-content:center; color:#FFD700;">ðŸ“Š</span>
              Distribución de Ingresos
            </h3>
            <div style="display:flex; flex-direction:column; gap:20px;">
              ${[
          { label: 'Clicky Coin', value: clickyEarned, color: '#FFD700', icon: '<span class="icon-ecoxionum"></span>' },
          { label: 'Generador', value: genEarned, color: 'var(--accent)', icon: 'âš¡' },
          { label: 'Misiones', value: dailyEarned, color: 'var(--ok)', icon: 'ðŸŽ' },
          { label: 'Eco Luck', value: luckEarned, color: 'var(--accent-2)', icon: 'ðŸŽ²' }
        ].map(item => {
          ...
          return `
                  <div>
                    <div style="display:flex; justify-content:space-between; margin-bottom:10px; font-size:14px;">
                      ...
                      <span style="font-weight:700;">${item.value.toLocaleString()} <span class="icon-ecoxionum"></span> <small style="font-weight:400; opacity:0.6; margin-left:4px;">(${Math.round(perc)}%)</small></span>
                    </div>
          ...
          <div class="card" style="margin:0; border-radius:24px;">
            <h3 style="margin:0 0 24px; font-size:20px; display:flex; align-items:center; gap:12px;">
              <span style="width:36px; height:36px; background:rgba(100,167,255,0.1); border-radius:10px; display:flex; align-items:center; justify-content:center; color:var(--accent);">ðŸ•’</span>
              Metas de Hoy
            </h3>
            <div style="display:flex; flex-direction:column; gap:16px;">
              <div class="activity-item">
                 <div style="width:44px; height:44px; border-radius:14px; background:rgba(100,167,255,0.1); display:flex; align-items:center; justify-content:center; color:var(--accent); font-size:20px;">ðŸ“‹</div>
                 <div style="flex:1;">
              ...
              <div class="activity-item">
                 <div style="width:44px; height:44px; border-radius:14px; background:rgba(16,185,129,0.1); display:flex; align-items:center; justify-content:center; color:var(--ok); font-size:20px;">ðŸŽ®</div>
                 <div style="flex:1;">
              ...
              <div class="activity-item">
                 <div style="width:44px; height:44px; border-radius:14px; background:rgba(255,215,0,0.1); display:flex; align-items:center; justify-content:center; color:#FFD700; font-size:20px;"><span class="icon-ecoxionum"></span></div>
                 <div style="flex:1;">
    ...
    async function renderCommerce() {
      ...
      <div style="display:inline-flex;flex-direction:column;gap:8px;padding:20px 40px;background:rgba(0,0,0,.2);border-radius:16px;border:1px solid rgba(255,255,255,.1);">
        ...
        <div style="font-size:42px;font-weight:800;color:var(--ok);text-shadow:0 0 20px rgba(16,185,129,.3);">
          ${(state.ecoxionums || 0).toLocaleString()} <span class="icon-ecoxionum"></span>
        </div>
      </div>
    </div>
    ...
    <div class="grid cols-2" style="margin-bottom:24px;">
      ...
      <div class="card" style="background:var(--bg-1);border:1px solid ${state.oceanPayLinked ? 'var(--accent)' : 'var(--warn)'};">
        ...
        <div style="margin-bottom:20px;">
          <div style="display:flex;justify-content:space-between;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,.05);">
            ...
            <span style="font-weight:600;color:${state.oceanPayLinked ? 'var(--ok)' : 'var(--warn)'};">
              ${state.oceanPayLinked ? '? Vinculado' : 'âš ï¸ No vinculado'}
            </span>
          </div>
          ${state.oceanPayLinked ? `
            <div style="display:flex;justify-content:space-between;margin-bottom:12px;">
        ...
        ${state.oceanPayLinked ? `
          ...
          <button class="btn primary" onclick="updateBalance()" style="width:100%;">ðŸ”„ Sincronizar Saldo</button>
        ` : `
          <button class="btn primary" onclick="openOceanPay()" style="width:100%;">ðŸŒŠ Conectar Ocean Pay</button>
        `}
      </div>
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
        <h3 style="margin:0;">ðŸ“œ Historial de Transacciones</h3>
        <button onclick="renderCommerce()" style="background:transparent;border:none;color:var(--accent);cursor:pointer;">ðŸ”„</button>
      </div>
    ...
    `;
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:rgba(255,255,255,.03);border-radius:8px;border-left:3px solid ${isIncoming ? 'var(--ok)' : 'var(--warn)'};">
                ...
                <div style="font-weight:700;color:${isIncoming ? 'var(--ok)' : 'var(--warn)'};">
                  ${isIncoming ? '+' : ''}${tx.monto} <span class="icon-ecoxionum"></span>
                </div>
              </div>
      `;
          }).join('');
    ...
    async function showTransferModal() {
      ...
      <div class="card" style="width:100%;max-width:450px;animation:slideUp 0.3s ease-out;">
          <h2 style="margin:0 0 20px;display:flex;align-items:center;gap:10px;">
            <span>ðŸ’¸</span> Enviar Ecoxionums
          </h2>
          ...
          <div style="margin-bottom:20px;padding:16px;background:rgba(100,167,255,.1);border-radius:8px;border:1px solid var(--accent);">
             ...
             <div style="font-size:24px;font-weight:700;color:var(--accent);">${(state.ecoxionums || 0).toLocaleString()} <span class="icon-ecoxionum"></span></div>
          </div>
          ...
          <div style="display:flex;gap:12px;">
            ...
            <button class="btn primary" id="tx-send" style="flex:1;">Enviar ðŸ’¸</button>
          </div>
        </div>
      `;
    ...
    async function renderSettings() {
      ...
      <div style="margin-top:24px;">
        ...
        <select id="theme-select" style="width:100%;padding:10px;border-radius:8px;background:var(--bg-1);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);">
          <option value="espacial" ${state.currentTheme === 'espacial' ? 'selected' : ''}>ðŸŒŒ Espacial</option>
          <option value="naturaleza" ${state.currentTheme === 'naturaleza' ? 'selected' : ''}>ðŸŒ¿ Naturaleza</option>
          <option value="tropical" ${state.currentTheme === 'tropical' ? 'selected' : ''}>ðŸï¸ Tropical</option>
        </select>
      </div>
      ...
      <div style="margin-top:32px;padding-top:24px;border-top:1px solid rgba(255,255,255,.1);">
        ...
        ${state.oceanPayLinked
          ? `<p style="color:var(--ok);margin:0 0 12px;">âœ… Vinculado con Ocean Pay</p>
          ...
          : `<p style="color:var(--ink-1);margin:0 0 12px;">No vinculado con Ocean Pay</p>
             <button class="btn primary" onclick="openOceanPay()" style="width:100%;">ðŸŒŠ Vincular Ocean Pay</button>`
        }
      </div>

      ${state.username === 'OceanandWild'
          ? `<div style="margin-top:32px;padding-top:24px;border-top:1px solid rgba(255,255,255,.1);">
             <h3 style="margin:0 0 12px;font-size:16px;">ðŸ› ï¸ Panel de Admin</h3>
    ...
    /* =========================
       CERRAR SESIÃ“N Y ELIMINAR CUENTA
       ========================= */
    ...
    window.toggleOceanPayMode = function () {
      ...
      if (isOpRegisterMode) {
        if (title) title.textContent = 'ðŸŒŠ Crear Cuenta Ocean Pay';
        if (btn) btn.textContent = 'ðŸ“ Registrarse y Vincular';
        if (toggleMsg) toggleMsg.textContent = '¿Ya tienes cuenta? Inicia sesión';
      } else {
        if (title) title.textContent = 'ðŸŒŠ Ocean Pay Access';
    ...
    async function initializeApp() {
      const state = await loadState();
      startUiNormalizationObserver();
      // Render temprano para evitar flash/desaparicion de tabs mientras cargan tareas async.
      if (state.currentTheme) applyTheme(state.currentTheme);
      if (state.darkMode) document.body.classList.add('dark-theme');
      if (state.zenMode) document.body.classList.add('zen-layout');
      if (typeof normalizeVisibleMojibake === 'function') normalizeVisibleMojibake();
      renderNavigation();
    ...
    function updatePomodoroDisplay() {
      ...
      if (progressCircle) {
        // Circumference = 2 * PI * r = 2 * PI * 136 â‰ˆ 854
    ...
    function loadGallery() {
      ...
      grid.innerHTML = images.map((img, index) => `
        <div class="gallery-item" style="position:relative;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,.12);background:var(--bg-1);transition:all .3s;cursor:pointer;" onmouseenter="this.style.transform='translateY(-4px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,.3)'" onmouseleave="this.style.transform='translateY(0)';this.style.boxShadow='none'">
          <img src="data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22%3E%3Crect fill=%22%23333%22 width=%22200%22 height=%22200%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 fill=%22%23999%22 font-size=%2216%22%3EError al cargar%3C/text%3E%3C/svg%3E" alt="${img.title || 'Imagen'}" onclick="openGalleryModal('${img.url}', '${img.title || ''}')" style="width:100%;height:200px;object-fit:cover;display:block;" onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22%3E%3Crect fill=%22%23333%22 width=%22200%22 height=%22200%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 fill=%22%23999%22 font-size=%2216%22%3EError al cargar%3C/text%3E%3C/svg%3E'">
          ${img.title ? `
          ...
          <button onclick="event.stopPropagation();deleteGalleryImage(${index})" style="position:absolute;top:8px;right:8px;background:var(--danger);color:white;border:none;padding:6px 10px;border-radius:6px;cursor:pointer;font-weight:600;font-size:12px;box-shadow:0 2px 8px rgba(0,0,0,.3);">ðŸ—‘ï¸</button>
        </div>
      `).join('');
    }
    ...
    function updateWatchAdsPanel() {
      ...
      if (earnedTodayEl) earnedTodayEl.innerHTML = `${earnedToday.toLocaleString()} <span class="icon-ecoxionum"></span>`;
      if (totalEarnedEl) totalEarnedEl.innerHTML = `${totalEarned.toLocaleString()} <span class="icon-ecoxionum"></span>`;

      const btn = document.getElementById('watch-ad-btn');
      if (btn) {
        btn.disabled = watchedToday >= 10;
        if (watchedToday >= 10) {
          btn.textContent = '�S& Límite diario alcanzado';
          btn.style.opacity = '0.5';
        }
      }
    }
    ...
    function showTriviaResult(correct, reward) {
      ...
      document.getElementById('trivia-result').innerHTML = `
        <div style="font-size:48px;margin-bottom:12px;">${correct ? 'ðŸŽ‰' : 'ðŸ˜”'}</div>
        ...
        return `
          <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid ${completedToday ? 'var(--ok)' : 'rgba(255,255,255,.08)'};">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              ...
              <div style="display:flex;gap:8px;">
                <button onclick="completeHabit(${i})" class="btn ${completedToday ? '' : 'primary'}" style="padding:8px 12px;" ${completedToday ? 'disabled' : ''}>${completedToday ? 'âœ…' : 'âœ“'}</button>
                <button onclick="deleteHabit(${i})" class="btn" style="padding:8px 12px;">ðŸ—‘ï¸</button>
              </div>
            </div>
    ...
    window.addHabit = function () {
      ...
      const icon = document.getElementById('habit-icon')?.value || 'ðŸ’ª';
    ...
    window.searchWeather = async function () {
      ...
      try {
        ...
        const weatherIcons = { 'Sunny': 'â˜€ï¸', 'Clear': 'ðŸŒ™', 'Partly cloudy': 'â›…', 'Cloudy': 'â˜ï¸', 'Overcast': 'â˜ï¸', 'Rain': 'ðŸŒ§ï¸', 'Heavy rain': 'â›ˆï¸', 'Snow': 'â„ï¸', 'Mist': 'ðŸŒ«ï¸', 'Fog': 'ðŸŒ«ï¸' };
        const desc = current.weatherDesc[0].value;
        const icon = weatherIcons[desc] || 'ðŸŒ¤ï¸';
    ...
    function initSnakeGame(container) {
      container.innerHTML = `
        <div style="text-align:center;padding:20px;background:var(--bg-1);border-radius:12px;">
          <h4 style="margin:0 0 16px;color:var(--accent);">ðŸ Snake</h4>
    ...
    function initMemoryGame(container) {
      const emojis = ['??', '??', 'ðŸŽ¯', 'ðŸŽª', 'ðŸŽ¨', 'ðŸŽ­', 'ðŸŽª', '??', '??', 'ðŸŽ¯', 'ðŸŽ¨', 'ðŸŽ­'];
      ...
      container.innerHTML = `
        <div style="text-align:center;padding:20px;background:var(--bg-1);border-radius:12px;">
          <h4 style="margin:0 0 16px;color:var(--accent);">ðŸ§  Memory</h4>
    ...
    function saveArcadeScore(game, score, isTime = false) {
      ...
      if (isNewRecord) {
        ...
        updateArcadeHighScores();
        showAlert('success', '�x�  NEW HIGH SCORE!', `Nuevo récord en ${game}: ${score}`);
    ...
    function initArcadeMemory(viewport) {
      ...
      const emojis = ['ðŸš€', 'ðŸª', 'ðŸ‘½', 'â˜„ï¸', 'ðŸŒŒ', 'ðŸ”­', 'ðŸ›¸', 'ðŸ›°ï¸'];
      const cards = [...emojis, ...emojis].sort(() => Math.random() - 0.5);
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        ...
        grid.innerHTML += `
          <div style="text-align:center;padding:8px;border-radius:8px;font-size:13px;position:relative;
            ...
            cursor:pointer;transition:transform 0.2s;" onmouseenter="this.style.transform='scale(1.1)'" onmouseleave="this.style.transform='scale(1)'">
            ${day}
            ${dotsHtml}
          </div>
        `;
    ...
    function renderCalendarEvents() {
      ...
      container.innerHTML = upcoming.map((e, i) => {
        let typeIcon = e.type === 'work' ? 'ðŸ’¼' : (e.type === 'important' ? 'â­' : 'ðŸ‘¤');
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:${typeColor};border-left:3px solid ${typeBorder};border-radius:0 8px 8px 0;margin-bottom:4px;">
          ...
          <button onclick="deleteCalendarEvent(${i})" class="btn" style="padding:6px;background:transparent;color:var(--ink-1);">âœ•</button>
        </div>
      `}).join('');
    }
    ...
    /* =========================
       ECLIPSER - LOGICA
       ========================= */
    const ECLIPSER_EARLY_COMPLETE_MS = 10000;
    const ECLIPSER_CLAIM_CACHE_KEY = 'ecx.eclipser.claimed.v1';
    const eclipserState = {
      upcoming: [],
      active: [],
      eventIndex: {},
    ...
    function toApiDateTimeFromInput(inputId) {
    ...
    function getEclipserTypeLabel(type) {
      const map = {
        solar_total: 'Solar Total',
        solar_partial: 'Solar Parcial',
        lunar_total: 'Lunar Total',
        lunar_partial: 'Lunar Parcial',
        annular: 'Anular'
      };
      return map[type] || 'Desconocido';
    }

    function getClaimCache() {
      try {
        const raw = localStorage.getItem(ECLIPSER_CLAIM_CACHE_KEY) || '{}';
    ...
    function writeClaimCache(cache) {
      localStorage.setItem(ECLIPSER_CLAIM_CACHE_KEY, JSON.stringify(cache || {}));
    }

    function isLocallyClaimed(userId, eventId) {
      const uid = String(userId || '').trim();
      const id = Number(eventId || 0);
      if (!uid || !id) return false;
      const cache = getClaimCache();
      return Boolean(cache[uid] && cache[uid][String(id)]);
    }

    function markLocallyClaimed(userId, eventId) {
      const uid = String(userId || '').trim();
      const id = Number(eventId || 0);
      if (!uid || !id) return;
      const cache = getClaimCache();
      if (!cache[uid]) cache[uid] = {};
      cache[uid][String(id)] = Date.now();
      writeClaimCache(cache);
    }

    function normalizeEclipserEvent(ev) {
      if (!ev || typeof ev !== 'object') return null;
      const startMs = new Date(ev.startsAt || ev.startAt || '').getTime();
      const endMs = new Date(ev.endsAt || ev.endAt || '').getTime();
    ...
    function hasEventEnded(ev, nowTs = Date.now()) {
      return Number(ev?.endMs || 0) > 0 && nowTs >= Number(ev.endMs);
    }

    function isServerClaimedFlag(ev) {
      return Boolean(ev?.alreadyClaimed || ev?.claimed || ev?.hasClaimed || ev?.participated || ev?.rewardClaimed);
    }

    function setEclipserEventIndex() {
      const idx = {};
      [...(eclipserState.upcoming || []), ...(eclipserState.active || [])].forEach((ev) => {
        const n = normalizeEclipserEvent(ev);
        if (n) idx[n.id] = n;
      });
      eclipserState.eventIndex = idx;
    }

    async function resolveEclipserEventById(eventId, allowRefresh = true) {
      const id = Number(eventId || 0);
      if (!Number.isFinite(id) || id <= 0) return null;
      const local = eclipserState.eventIndex[id]
        || [...(eclipserState.active || []), ...(eclipserState.upcoming || [])].map(normalizeEclipserEvent).find((e) => e && e.id === id);
      if (local) return local;
      if (!allowRefresh) return null;
      await Promise.race([
        window.refreshEclipserData(),
        new Promise((resolve) => setTimeout(resolve, 1300))
      ]);
      return eclipserState.eventIndex[id] || null;
    }

    async function eclipserFetchJson(url, timeoutMs = 4500) {
    ...
    function closeEclipserViewer() {
      if (typeof eclipserState.resizeHandler === 'function') {
        window.removeEventListener('resize', eclipserState.resizeHandler);
      }
      eclipserState.resizeHandler = null;
      if (eclipserState.viewLoop) cancelAnimationFrame(eclipserState.viewLoop);
      eclipserState.viewLoop = null;
      const ov = eclipserState.modalRefs?.overlay || document.getElementById('eclipser-view-overlay');
      if (ov && ov.parentNode) ov.parentNode.removeChild(ov);
      eclipserState.modalRefs = null;
      eclipserState.currentView = null;
      eclipserState.autoClaimInFlight = false;
    }

    function ensureEclipserModal() {
      const old = document.getElementById('eclipser-view-overlay');
      if (old && old.parentNode) old.parentNode.removeChild(old);

      const overlay = document.createElement('div');
      overlay.id = 'eclipser-view-overlay';
      overlay.style.cssText = [
        'position:fixed',
        'inset:0',
        'display:flex',
      ...
      overlay.innerHTML = `
        <div id="eclipser-view-shell" style="position:relative;width:min(980px,96vw);max-height:92vh;overflow:auto;border:1px solid rgba(255,255,255,.2);border-radius:14px;background:rgba(7,12,24,.99);box-shadow:0 20px 42px rgba(0,0,0,.75);padding:14px;">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;">
            <strong id="eclipser-view-title">Eclipser 3D</strong>
            <button class="btn" id="eclipser-close-btn" type="button">Cerrar</button>
          </div>
          <div style="margin-top:8px;font-size:12px;color:var(--ink-1);">El eclipse evoluciona segun el tiempo real del evento.</div>
          <div id="eclipser-three-host" style="width:100%;height:430px;min-height:320px;margin-top:10px;border-radius:10px;border:1px solid rgba(255,255,255,.12);background:#020511;overflow:hidden;"></div>
          <div style="margin-top:10px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
            <div id="eclipser-watch-status" style="font-size:13px;color:var(--ink-1);">Sincronizando eclipse...</div>
            <button class="btn primary" id="eclipser-claim-btn" type="button" disabled>Reclamar recompensa</button>
          </div>
        </div>
      `;
      document.body.appendChild(overlay);
      const refs = {
        overlay,
        shell: overlay.querySelector('#eclipser-view-shell'),
        host: overlay.querySelector('#eclipser-three-host'),
        status: overlay.querySelector('#eclipser-watch-status'),
        title: overlay.querySelector('#eclipser-view-title'),
        claimBtn: overlay.querySelector('#eclipser-claim-btn'),
        closeBtn: overlay.querySelector('#eclipser-close-btn')
      };
      refs.closeBtn?.addEventListener('click', closeEclipserViewer);
      refs.claimBtn?.addEventListener('click', () => window.claimEclipserReward && window.claimEclipserReward());
      eclipserState.modalRefs = refs;
      return refs;
    }

    function getEclipserTheme(type) {
      const lunar = String(type || '').includes('lunar');
      const annular = String(type || '') === 'annular';
      return {
        skyA: lunar ? '#120719' : '#081224',
        skyB: lunar ? '#2a0d10' : '#02050f',
        sun: lunar ? '#d87455' : '#ffd06f',
        halo: lunar ? 'rgba(255,119,85,.28)' : 'rgba(255,198,108,.3)',
        moon: lunar ? '#2b2336' : '#1a2635',
        annular
      };
    }
    ...
    async function tryClaimEclipserReward(event, opts = {}) {
      const current = await loadState();
      const userId = current?.userId;
      if (!userId || !event?.id) return { ok: false, error: 'Usuario invalido' };
      if (isLocallyClaimed(userId, event.id)) return { ok: true, alreadyClaimed: true };
      const watchedSeconds = Math.max(0, Math.floor((Date.now() - eclipserState.viewStartedAt) / 1000));
      try {
        const res = await fetch(`${API_GLOBAL}/api/ecoxion/eclipses/${event.id}/participate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (data.alreadyClaimed || Number(data.reward || 0) > 0) {
          markLocallyClaimed(userId, event.id);
        }
        if (!opts.silent) {
          if (data.alreadyClaimed) showAlert('info', 'Eclipser', 'Ya reclamaste este eclipse.');
          else showAlert('success', 'Eclipser', `Recompensa recibida: +${Number(data.reward || 0)} Ecoxionums`);
        }
        await updateBalance();
        return { ok: true, alreadyClaimed: Boolean(data.alreadyClaimed), reward: Number(data.reward || 0) };
      } catch (err) {
        if (!opts.silent) showAlert('error', 'Eclipser', err.message || 'Error de conexion');
        return { ok: false, error: err?.message || 'Error' };
      }
    }

    function startEclipserRenderLoop(event) {
      const refs = eclipserState.modalRefs;
      if (!refs?.host) return;
      refs.host.innerHTML = '';
      const canvas = document.createElement('canvas');
      canvas.style.cssText = 'width:100%;height:100%;display:block;';
      refs.host.appendChild(canvas);
      const ctx = canvas.getContext('2d');
      if (!ctx) return;
      ...
      eclipserState.resizeHandler = resize;
      resize();
      window.addEventListener('resize', resize, { passive: true });

      const draw = async () => {
        if (!eclipserState.currentView || eclipserState.currentView.id !== event.id) return;
        const now = Date.now();
        const theme = getEclipserTheme(event.eclipseType);
        ...
        for (let i = 0; i < 48; i++) {
          const sx = (i * 137 + Math.floor(now / 45)) % w;
          const sy = (i * 97) % h;
        ...
        const moonR = sunR * 0.93;
        const completeAtMs = Number(event.endMs) - ECLIPSER_EARLY_COMPLETE_MS;
        const travelDuration = Math.max(1000, completeAtMs - Number(event.startMs));
        const phase = clamp01((now - Number(event.startMs)) / travelDuration);
        const moonX = cx + ((1 - phase) * -2.8 * sunR);
        const moonY = cy;

        const corona = ctx.createRadialGradient(cx, cy, sunR * 0.2, cx, cy, sunR * 3.2);
        corona.addColorStop(0, theme.halo);
        ...
        if (theme.annular && phase >= 0.98) {
          ctx.strokeStyle = 'rgba(255,222,150,.95)';
          ctx.lineWidth = Math.max(2, sunR * 0.06);
          ctx.beginPath();
          ctx.arc(cx, cy, sunR * 0.88, 0, Math.PI * 2);
          ctx.stroke();
        }

        const secondsToEnd = Math.max(0, Math.ceil((Number(event.endMs) - now) / 1000));
        const reachedPeak = now >= completeAtMs;
        const ended = now >= Number(event.endMs);
        if (refs.status) {
          if (now < Number(event.startMs)) refs.status.textContent = `Comienza en ${Math.ceil((Number(event.startMs) - now) / 1000)}s`;
          else if (!reachedPeak) refs.status.textContent = `Alineacion en progreso: ${Math.round(phase * 100)}%`;
          else if (!ended) refs.status.textContent = `Eclipse completo. Finaliza en ${secondsToEnd}s`;
          else refs.status.textContent = 'Evento finalizado.';
        }

        if (refs.claimBtn) {
          refs.claimBtn.disabled = !ended;
          refs.claimBtn.textContent = `Reclamar ${Number(event.rewardAmount || 0)} Ecoxionums`;
        }

        if (ended && !eclipserState.autoClaimInFlight) {
          eclipserState.autoClaimInFlight = true;
          await tryClaimEclipserReward(event, { silent: true });
          closeEclipserViewer();
          await refreshEclipserData();
          return;
        }
        eclipserState.viewLoop = requestAnimationFrame(draw);
      };
      eclipserState.viewLoop = requestAnimationFrame(draw);
    }

    window.openEclipserViewer = async function (eventId) {
      const current = await loadState();
      const userId = current?.userId || '';
      const event = await resolveEclipserEventById(eventId, true);
      if (!event) {
        showAlert('warning', 'Eclipser', 'No se encontro ese eclipse.');
        return;
      }
      if (isServerClaimedFlag(event) || isLocallyClaimed(userId, event.id)) {
        showAlert('info', 'Eclipser', 'Este eclipse ya fue reclamado y no puede abrirse de nuevo.');
        return;
      }
      if (hasEventEnded(event)) {
        await tryClaimEclipserReward(event, { silent: true });
        await refreshEclipserData();
        showAlert('info', 'Eclipser', 'Ese eclipse ya finalizo.');
        return;
      }

      const refs = ensureEclipserModal();
      eclipserState.currentView = event;
      eclipserState.viewStartedAt = Date.now();
      if (refs.title) refs.title.textContent = `${event.title} (${getEclipserTypeLabel(event.eclipseType)})`;
      if (refs.status) refs.status.textContent = 'Sincronizando eclipse...';
      if (refs.claimBtn) {
        refs.claimBtn.disabled = true;
        refs.claimBtn.textContent = `Reclamar ${Number(event.rewardAmount || 0)} Ecoxionums`;
      }
      startEclipserRenderLoop(event);
    };

    window.openAnyEclipserViewer = async function () {
      await refreshEclipserData();
      const current = await loadState();
      const userId = current?.userId || '';
      const now = Date.now();
      const candidate = [...(eclipserState.active || []), ...(eclipserState.upcoming || [])]
        .map(normalizeEclipserEvent)
        .filter(Boolean)
        .find((ev) => !hasEventEnded(ev, now) && !isServerClaimedFlag(ev) && !isLocallyClaimed(userId, ev.id));
      if (!candidate) {
        showAlert('warning', 'Eclipser', 'No hay eclipses disponibles para ver ahora.');
        return;
      }
      await window.openEclipserViewer(candidate.id);
    };

    window.claimEclipserReward = async function () {
      const event = eclipserState.currentView;
      if (!event) return;
      const result = await tryClaimEclipserReward(event, { silent: false });
      if (result.ok) {
        closeEclipserViewer();
        await refreshEclipserData();
      }
    ...
    window.createCustomEclipserEvent = async function () {
      const current = await loadState();
      try {
        const res = await fetch(`${API_GLOBAL}/api/ecoxion/eclipses/custom`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId: current.userId,
            username: current.username,
            planId: current.subscriptionPlan || 'free',
            eclipseType: document.getElementById('eclipser-custom-type')?.value,
            startsAt: toApiDateTimeFromInput('eclipser-custom-start'),
            endsAt: toApiDateTimeFromInput('eclipser-custom-end'),
            customName: document.getElementById('eclipser-custom-name')?.value || ''
          })
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data?.error || 'No se pudo crear eclipse personalizado.');
        showAlert('success', 'Eclipser', 'Eclipse personalizado creado.');
        await refreshEclipserData();
      } catch (err) {
        showAlert('error', 'Eclipser', err.message || 'Error');
      }
    };

    function buildEclipserEventCard(ev, active) {
      const badge = ev.eclipseKind === 'admin' ? 'Admin' : 'Custom';
      return `
        <div class="eclipser-event-card" data-eclipser-id="${ev.id}" style="padding:10px;border:1px solid ${active ? 'rgba(16,185,129,.35)' : 'rgba(255,255,255,.12)'};border-radius:10px;background:${active ? 'rgba(16,185,129,.08)' : 'rgba(255,255,255,.02)'};">
          <div style="display:flex;justify-content:space-between;gap:8px;align-items:center;flex-wrap:wrap;">
            <strong style="color:var(--ink-0);">${ev.title}</strong>
            <span style="font-size:11px;padding:4px 8px;border-radius:999px;background:rgba(56,189,248,.14);border:1px solid rgba(56,189,248,.35);">${badge}</span>
          </div>
          <div style="margin-top:4px;font-size:12px;">Tipo: ${getEclipserTypeLabel(ev.eclipseType)}</div>
          <div style="margin-top:2px;font-size:12px;">Inicio: ${formatDateHuman(ev.startsAt)} | Fin: ${formatDateHuman(ev.endsAt)}</div>
          <div style="margin-top:2px;font-size:12px;color:var(--accent);">Reward: +${Number(ev.rewardAmount || 0)} Ecoxionums</div>
          <div style="margin-top:8px;display:flex;justify-content:flex-end;">
            <button type="button" class="btn ${active ? 'primary' : ''} eclipser-open-btn" data-eclipser-open="${ev.id}">${active ? 'Participar y Ver' : 'Ver Eclipse 3D'}</button>
          </div>
        </div>
      `;
    }

    function renderEclipserList() {
      const upcomingBox = document.getElementById('eclipser-upcoming');
      const activeBox = document.getElementById('eclipser-active');
      const now = Date.now();
      const userId = String(eclipserState.currentUserId || '');
      const isVisible = (ev) => !hasEventEnded(ev, now) && !isServerClaimedFlag(ev) && !isLocallyClaimed(userId, ev.id);
      const upcoming = (eclipserState.upcoming || []).map(normalizeEclipserEvent).filter((ev) => ev && isVisible(ev));
      const active = (eclipserState.active || []).map(normalizeEclipserEvent).filter((ev) => ev && isVisible(ev));

      if (upcomingBox) {
        upcomingBox.innerHTML = upcoming.length
          ? upcoming.map((ev) => buildEclipserEventCard(ev, false)).join('')
          : '<div style="opacity:.8;">Sin eclipses programados.</div>';
      }
      if (activeBox) {
        activeBox.innerHTML = active.length
          ? active.map((ev) => buildEclipserEventCard(ev, true)).join('')
          : '<div style="opacity:.8;">No hay eclipse activo en este momento.</div>';
      }
    }

    function ensureEclipserDelegation() {
      if (eclipserState.delegated) return;
      eclipserState.delegated = true;
      document.addEventListener('click', (ev) => {
        const btn = ev.target?.closest?.('[data-eclipser-open]');
        if (!btn) return;
        ev.preventDefault();
        const id = Number(btn.getAttribute('data-eclipser-open') || 0);
        if (!Number.isFinite(id) || id <= 0) return;
        window.openEclipserViewer(id);
      }, true);
    }

    window.refreshEclipserData = async function () {
      const current = await loadState();
      eclipserState.currentUserId = String(current?.userId || '');
      try {
        const [upData, acData] = await Promise.all([
          eclipserFetchJson(`${API_GLOBAL}/api/ecoxion/eclipses/upcoming?userId=${encodeURIComponent(current.userId || '')}`),
          eclipserFetchJson(`${API_GLOBAL}/api/ecoxion/eclipses/active`)
        ]);
        eclipserState.upcoming = Array.isArray(upData?.events) ? upData.events.map(normalizeEclipserEvent).filter(Boolean) : [];
        eclipserState.active = Array.isArray(acData?.events) ? acData.events.map(normalizeEclipserEvent).filter(Boolean) : [];
      } catch (_) {
        eclipserState.upcoming = [];
        eclipserState.active = [];
      }
      setEclipserEventIndex();
      renderEclipserList();
    };

    window.initEclipser = async function () {
      const current = await loadState();
      const plan = String(current?.subscriptionPlan || '').toLowerCase();
      const ultraZone = document.getElementById('eclipser-ultra-zone');
      if (ultraZone) ultraZone.style.display = plan.includes('ultra') ? 'block' : 'none';

      const openAnyBtn = document.getElementById('eclipser-open-any-btn');
      if (openAnyBtn && openAnyBtn.dataset.bound !== '1') {
        openAnyBtn.dataset.bound = '1';
        openAnyBtn.addEventListener('click', (ev) => {
          ev.preventDefault();
          window.openAnyEclipserViewer();
        });
      }

      ensureEclipserDelegation();
      await refreshEclipserData();
    };
