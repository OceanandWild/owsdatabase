...
function Ensure-Shortcuts {
  ...
  $installDir = Join-Path $env:LOCALAPPDATA "Programs\Savage Space Animals"
  $exePath = Join-Path $installDir "Savage Space Animals.exe"
  if (-not (Test-Path $exePath)) {
    Write-Warning "No se encontro el ejecutable instalado: $exePath"
    return
  }

  $desktopLnk = Join-Path ([Environment]::GetFolderPath("Desktop")) "Savage Space Animals.lnk"
  $programsLnk = Join-Path $env:APPDATA "Microsoft\Windows\Start Menu\Programs\Savage Space Animals.lnk"
...
if (-not $SkipBuild) {
  Write-Step "Compilando Savage Space Animals con signAndEditExecutable=true"
  npm run dist
...
if (-not $resolvedInstaller) {
  $outputDir = "C:\builds\savage-space-animals"
  $latest = Get-ChildItem -Path $outputDir -Filter "Savage.Space.Animals.Setup.*.exe" -ErrorAction SilentlyContinue |
...
if (-not $SkipInstall) {
  Write-Step "Cerrando instancias previas de Savage Space Animals"
  Get-Process "Savage Space Animals" -ErrorAction SilentlyContinue | Stop-Process -Force
...
Write-Host "Siguiente paso: abre Savage Space Animals desde Inicio y anclalo de nuevo a la barra de tareas." -ForegroundColor Green
