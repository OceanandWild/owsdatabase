    ...
    async function flushQueuedStateSync() {
      ...
      if (stateSyncQueued) {
        // Si llegaron ms cambios durante el sync, enviamos solo el ltimo snapshot.
    ...
    const defaultExtensions = {
      "notes": {
        id: "notes",
        name: "Notas",
        description: "Escribe y organiza tus notas rpidamente (Funcin Activa: abre un panel en el dashboard).",
        rarity: "comun",
        type: "active",
        version: "1.0.0",
        version2: "2.0.0",
        hasV2: true,
        v2Features: ["Bsqueda avanzada", "Etiquetas y categoras", "Exportacin a PDF"],
        icon: "?",
        price: 0,
        missions: [
          { id: "create_3_notes", title: "Crea 3 notas", description: "Escribe y guarda al menos 3 notas diferentes", target: 3, type: "count" },
          { id: "note_length", title: "Nota extensa", description: "Crea una nota con ms de 100 caracteres", target: 1, type: "length" },
          { id: "notes_mastery", title: "Maestra: Bibliotecario", description: "Acumula 20 notas guardadas para demostrar tu sabidura", target: 20, type: "count" }
        ],
        async getPanelHTML() {
        ...
        <div id="notes-panel">
          ${isV2 ? `
            <div style="margin-bottom: 16px;padding:12px;background:linear-gradient(135deg, rgba(100,167,255,.1), rgba(179,136,255,.1));border-radius:8px;border:1px solid var(--accent);margin-bottom:16px;">
              ...
              <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">
                ...
                <input type="text" id="note-tags" placeholder="? Etiquetas (separadas por coma)" style="flex:1;min-width:200px;padding:8px;border-radius:6px;background:var(--bg-1);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);">
              </div>
            </div>
          ` : `
          ...
          <div style="margin-bottom: 16px;">
            <input type="text" id="note-title" placeholder="Ttulo" style="width:100%;padding:10px;border-radius:8px;background:var(--bg-1);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);margin-bottom:8px;">
      ...
      "dark-mode": {
        id: "dark-mode",
        name: "Dark Mode",
        description: "Activa un tema oscuro OLED para toda la interfaz (Funcin Pasiva: se activa inmediatamente).",
        rarity: "rara",
        type: "passive",
        version: "1.1.0",
        icon: "?",
        price: 0,
        getPanelHTML() {
          return `<p>Dark Mode est activo. Disfruta de la interfaz oscura con ajustes dinmicos segn la hora del da.</p>`;
        },
        async onEnable() {
          const state = await loadState();
          state.darkMode = true;
      ...
      "zen-mode": {
        id: "zen-mode",
        name: "Zen Mode",
        description: "Redisea la interfaz a un formato minimalista y enfocado (Funcin Pasiva: se activa inmediatamente).",
        rarity: "comun",
        type: "passive",
        version: "1.0.0",
        icon: "?",
        price: 0,
        getPanelHTML() {
          return `<p>Zen Mode est activo. Disfruta de una interfaz ms limpia y organizada.</p>`;
        },
        async onEnable() {
          const state = await loadState();
          state.zenMode = true;
      ...
      "blog-creator": {
        ...
        <div id="blog-creator" style="padding:20px; color:var(--ink-1);">
          <div class="card" style="margin-bottom:24px; background:rgba(255,255,255,0.02); border-color:rgba(100,167,255,0.2);">
            <h3 style="margin:0 0 16px; color:var(--accent); font-size:18px;">? Crear Nueva Entrada</h3>
            <div style="display:flex; flex-direction:column; gap:12px;">
              <input type="text" id="blog-title" placeholder="Ttulo impactante..." 
              ...
              <textarea id="blog-content" placeholder="Escribe el contenido de tu blog aqu..." 
      ...
      "clicky-coin": {
        id: "clicky-coin",
        name: "Clicky Coin",
        description: "Gana Ecoxionums haciendo clic. Extensin gratuita para obtener monedas.",
        rarity: "comun",
        type: "active",
        version: "1.0.0",
        version2: "2.0.0",
        hasV2: true,
        v2Features: ["Ms recompensas por clic", "Multiplicadores de tiempo", "Logros y desafos"],
        icon: "?",
        price: 0,
        defaultInstall: false,
        missions: [
          { id: "click_100", title: "100 clics", description: "Haz 100 clics en total en la moneda", target: 100, type: "clicks" },
          { id: "daily_limit", title: "Lmite diario", description: "Alcanza el lmite diario de clics al menos una vez", target: 1, type: "daily_limit" },
          { id: "clicky_mastery", title: "Maestra: Magnate", description: "Acumula 5,000 Ecoxionums generados (Histrico)", target: 5000, type: "earned" }
        ],
        getPanelHTML() {
          return `
          ...
          <div style="margin-bottom:20px;padding:20px;background:linear-gradient(135deg, rgba(255,215,0,.1), rgba(255,182,72,.1));border-radius:12px;border:1px solid rgba(255,215,0,.3);text-align:center;">
            ...
            <div style="margin-top:16px;padding:10px;background:rgba(100,167,255,.1);border-radius:8px;border:1px solid var(--accent);">
              <p style="margin:0;font-size:11px;color:var(--ink-1);">
                ? <strong>Lmite diario:</strong> Mximo 50 clics por da (100-150 Ecoxionums diarios)
              </p>
            </div>
            ...
            <div id="autoclicker-warning" style="display:none;margin-top:12px;padding:12px;background:rgba(255,107,107,.15);border-radius:8px;border:2px solid var(--danger);text-align:center;">
              <p style="margin:0;font-size:12px;color:var(--danger);font-weight:600;">
                ? Auto-clicker detectado. Los clics demasiado rpidos no son vlidos.
              </p>
            </div>
      ...
      "eco-generator": {
        ...
        missions: [
          ...
          { id: "streak_3", title: "Racha de 3 das", description: "Mantn una racha de 3 das consecutivos", target: 3, type: "streak" },
          { id: "generator_mastery", title: "Maestra: Constancia Pura", description: "Mantn una racha de 7 das consecutivos", target: 7, type: "streak" }
        ],
        getPanelHTML() {
          return `
        <div id="eco-generator-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:24px;background:linear-gradient(135deg, rgba(80, 200, 120, 0.1), rgba(0, 150, 255, 0.1));border-radius:16px;border:1px solid rgba(80, 200, 120, 0.3);position:relative;overflow:hidden;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
              <h4 style="margin:0;color:var(--accent);font-size:18px;display:flex;align-items:center;gap:8px;">
                ...
                <span>Reactor de Energa</span>
              </h4>
              <span id="streak-badge" style="background:linear-gradient(90deg, #FF6B6B, #FF8E53);color:white;padding:4px 10px;border-radius:20px;font-size:11px;font-weight:700;">Racha: 0 das</span>
            </div>
          ...
          <div style="margin-top:16px;padding:12px 16px;background:linear-gradient(90deg, rgba(255, 171, 0, 0.1), transparent);border-left:3px solid #ffab00;border-radius:0 8px 8px 0;">
            <p style="margin:0;font-size:12px;color:#ffab00;">
              <strong>? Bono de Racha:</strong> +25% tras 3 das, +50% tras 7 das. ?No pierdas tu racha!
            </p>
          </div>
        </div>
      `;
      ...
      "eco-luck": {
        ...
        description: "Arriesga Ecoxionums para intentar ganar mucho ms. Todo queda a la suerte.",
        rarity: "raro",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="eco-luck-panel" style="padding:16px;">
          <div style="border:1px solid rgba(255,215,0,0.28);border-radius:16px;background:linear-gradient(145deg, rgba(255,215,0,0.08), rgba(255,136,0,0.05));padding:16px;margin-bottom:16px;">
            ...
            <p style="margin:8px 0 0;color:var(--ink-1);font-size:13px;">Inviertes <b>25 ?</b> por tirada y puedes recuperar desde <b>0</b> hasta <b>250 ?</b>.</p>
            ...
            <div id="eco-luck-plan-boost" style="margin-top:10px;font-size:12px;color:#bde6ff;">? Plan base activo: puedes mejorar tu suerte con planes premium.</div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-bottom:10px;">
            <div style="background:var(--bg-1);padding:10px;border-radius:10px;text-align:center;">
              <div style="font-size:10px;color:var(--ink-1);text-transform:uppercase;">ltimo resultado</div>
      ...
      "calculator": {
        id: "calculator",
        name: "Calculadora",
        description: "Calculadora cientfica con historial de operaciones.",
        rarity: "comun",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="calculator-panel" style="padding:16px;">
          <div style="max-width:400px;margin:0 auto;">
            ...
            <div style="display:grid;grid-template-columns:repeat(4, 1fr);gap:8px;margin-top:8px;">
              ...
              <button class="calc-btn" onclick="calcSqrt()">v</button>
            </div>
          </div>
        </div>
        <style>
      ...
      "pomodoro": {
        ...
        description: "Tcnica Pomodoro para mejorar tu productividad. 25 min trabajo, 5 min descanso.",
        rarity: "poco_comun",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="pomodoro-panel" style="padding:16px;">
          <div style="max-width:400px;margin:0 auto;">
            ...
            <div style="background:linear-gradient(135deg, rgba(80, 200, 120, 0.1), rgba(0, 150, 255, 0.1));padding:16px;border-radius:12px;display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
              <div>
                 <div style="font-size:12px;color:var(--ink-1);margin-bottom:2px;">Recompensa por sesin</div>
      ...
      "image-gallery": {
        id: "image-gallery",
        name: "Galera de Imgenes",
        description: "Guarda y organiza tus imgenes favoritas con URLs.",
        rarity: "poco_comun",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="gallery-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            ...
            <input type="text" id="gallery-title" placeholder="Ttulo (opcional)" style="width:100%;padding:10px;border-radius:8px;background:var(--bg-2);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);margin-bottom:8px;">
            <button class="btn primary" onclick="addImageToGallery()" style="width:100%;">Agregar a Galera</button>
          </div>
          ...
          <div id="gallery-empty" style="text-align:center;padding:40px;color:var(--ink-1);display:none;">
            ...
            <p style="margin:0;">No hay imgenes en tu galera</p>
            <p style="margin:8px 0 0;font-size:13px;opacity:.7;">Agrega URLs de imgenes para comenzar</p>
          </div>
        </div>
      ...
      "survey-rewards": {
        ...
        <div id="survey-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:20px;background:linear-gradient(135deg, rgba(100,167,255,.1), rgba(179,136,255,.1));border-radius:12px;border:1px solid var(--accent);text-align:center;">
            ...
            <p style="margin:0 0 16px;color:var(--ink-1);font-size:14px;">Responde encuestas rpidas y gana entre 5-15 Ecoxionums por encuesta.</p>
          ...
          <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            <h4 style="margin:0 0 12px;font-size:15px;">? Tus Estadsticas</h4>
            <div style="display:flex;flex-direction:column;gap:8px;">
              <div style="display:flex;justify-content:space-between;align-items:center;">
      ...
      "comms-generator": {
        ...
        missions: [
          ...
          { id: "comms_mastery", title: "Maestra: Embajador", description: "Genera 10 comunicados oficiales.", target: 10, type: "count" }
        ],
        getPanelHTML() {
          return `
            <style>
            ...
            <div class="comms-container">
              <div class="comms-form">
                ...
                <label>Autor</label>
                <input type="text" id="comm-author" value="Administracin" oninput="updateCommsPreview()">
                
                <label>Mensaje</label>
                <textarea id="comm-body" rows="6" placeholder="Escribe tu mensaje aqu..." oninput="updateCommsPreview()"></textarea>
                
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                  <div>
                    <label>Diseo Visual</label>
                    <select id="comm-template" onchange="updateCommsPreview()">
                      <option value="standard">Estndar</option>
              ...
              <div class="comms-preview-container" id="comm-preview-bg">
                <div class="comms-preview comms-tmpl-standard" id="comm-card">
                  <div class="preview-header">
                    <div class="preview-title" id="prev-title">TTULO DEL COMUNICADO</div>
                    <div class="preview-subtitle" id="prev-subtitle">Subttulo o Departamento</div>
                  </div>
                  ...
                  <div class="preview-body" id="prev-body">
                    Escribe tu mensaje en el panel de la izquierda para ver cmo queda en tiempo real.
                  </div>
        ...
        async onEnable() {
          window.applyTextPreset = function () {
            ...
            const presets = {
              notice: {
                title: "AVISO GENERAL",
                subtitle: "Administracin Central",
              ...
              urgent: {
                ...
                body: "ATENCIN: Se ha detectado una anomala en el sector 7.\n\nEvacuar inmediatamente. Esto no es un simulacro.",
                template: "alert"
              },
              decree: {
                title: "DECRETO REAL",
                subtitle: "La Corona",
                body: "Od, ciudadanos del reino.\n\nPor orden de Su Majestad, se declara da de festividad nacional en honor a los hroes de la tormenta.",
                template: "royal"
              },
              system: {
                title: "SYSTEM LOG",
            ...
            if (data) {
              ...
              // Opcional: Cambiar tambin el diseo visual si coincide
          ...
          window.updateCommsPreview = function () {
            const title = document.getElementById('comm-title').value || 'TTULO DEL COMUNICADO';
            const subtitle = document.getElementById('comm-subtitle').value || 'Subttulo o Departamento';
            const date = document.getElementById('comm-date').value || new Date().toISOString().split('T')[0];
            const author = document.getElementById('comm-author').value || 'ADMINISTRACIN';
      ...
      "trivia-challenge": {
        ...
        <div id="trivia-panel" style="padding:16px;">
          ...
          <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            <h4 style="margin:0 0 12px;font-size:15px;">? Estadsticas</h4>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      ...
      "music-player": {
        id: "music-player",
        name: "Music Player",
        description: "Reproductor de msica con estaciones de radio online. Escucha mientras trabajas.",
        rarity: "poco_comun",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="music-panel" style="padding:16px;">
          <div style="padding:24px;background:linear-gradient(135deg, rgba(236,72,153,.12), rgba(168,85,247,.12));border-radius:16px;border:1px solid rgba(236,72,153,.3);text-align:center;margin-bottom:20px;">
            ...
            <div id="music-now-playing" style="font-size:18px;font-weight:600;color:var(--ink-0);margin-bottom:4px;">Selecciona una estacin</div>
            <div id="music-station-name" style="font-size:13px;color:var(--ink-1);">---</div>
          </div>
          <div style="display:flex;gap:12px;justify-content:center;margin-bottom:20px;">
            <button class="btn" onclick="musicPrev()" style="width:50px;height:50px;border-radius:50%;font-size:20px;">?</button>
            <button id="music-play-btn" class="btn primary" onclick="musicToggle()" style="width:70px;height:70px;border-radius:50%;font-size:28px;">?</button>
            <button class="btn" onclick="musicNext()" style="width:50px;height:50px;border-radius:50%;font-size:20px;">?</button>
          </div>
          <div style="margin-bottom:16px;">
      ...
      "habit-tracker": {
        id: "habit-tracker",
        name: "Habit Tracker",
        description: "Crea y sigue tus h?H?bitos diarios. Gana Ecoxionums por mantener rachas.",
        rarity: "raro",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="habit-panel" style="padding:16px;">
          <div style="margin-bottom:20px;padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid rgba(255,255,255,.08);">
            <h4 style="margin:0 0 12px;font-size:15px;">? Nuevo H?H?bito</h4>
            <div style="display:flex;gap:8px;">
              <input type="text" id="habit-name" placeholder="Nombre del h?H?bito..." style="flex:1;padding:10px;border-radius:8px;background:var(--bg-2);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);">
          ...
          <div id="habits-empty" style="text-align:center;padding:40px;color:var(--ink-1);display:none;">
            <div style="font-size:48px;margin-bottom:12px;">?</div>
            <p>No tienes h?H?bitos. Crea uno!</p>
          </div>
      ...
      "weather-widget": {
        id: "weather-widget",
        name: "Weather Widget",
        description: "Consulta el clima actual y pronstico de cualquier ciudad.",
        rarity: "comun",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="weather-panel" style="padding:16px;">
          ...
          <div id="weather-display" style="padding:32px;background:linear-gradient(135deg, rgba(59,130,246,.15), rgba(147,197,253,.15));border-radius:20px;border:1px solid rgba(59,130,246,.3);text-align:center;">
            <div id="weather-icon" style="font-size:72px;margin-bottom:8px;">?</div>
          ...
          <div id="weather-details" style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-top:16px;">
            ...
            <div style="padding:16px;background:var(--bg-1);border-radius:12px;text-align:center;">
              ...
              <div style="font-size:11px;color:var(--ink-1);">Sensacin</div>
            </div>
          </div>
        </div>
      `;
      ...
      "eco-miner": {
        id: "eco-miner",
        name: "Eco Miner",
        description: "Minera automtica de Ecoxionums. Djalo funcionando y gana pasivamente.",
        rarity: "epica",
        type: "active",
        version: "1.0.0",
        icon: "?",
        ...
        <div id="miner-panel" style="padding:16px;">
          <div style="padding:24px;background:linear-gradient(135deg, rgba(245,158,11,.12), rgba(251,191,36,.12));border-radius:16px;border:2px solid rgba(245,158,11,.4);text-align:center;margin-bottom:20px;">
            <div id="miner-animation" style="font-size:64px;margin-bottom:16px;">?</div>
            ...
            <button id="miner-toggle-btn" class="btn primary" onclick="toggleMiner()" style="width:100%;font-size:16px;padding:14px;">? Iniciar Minera</button>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            <div style="padding:16px;background:var(--bg-1);border-radius:12px;text-align:center;">
            ...
            <div style="padding:16px;background:var(--bg-1);border-radius:12px;text-align:center;">
              ...
              <div style="font-size:12px;color:var(--ink-1);">Esta Sesin</div>
            </div>
          </div>
        </div>
      `;
      ...
      "mini-games": {
        id: "mini-games",
        name: "Ecoxion Arcade",
        description: "Estacin de juegos retro sin lmites. Snake, Memory y el nuevo Neon Jump. Compite por el high score!",
        rarity: "raro",
        type: "active",
        version: "2.0.0",
        icon: "?",
          ...
          <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:20px;">
            ...
            <div onclick="startArcadeGame('neon-jump')" class="arcade-card">
              ...
              <div class="arcade-info">
                <h4>Neon Jump</h4>
                <p>Esquiva obstculos en este endless runner nen.</p>
            ...
            <div onclick="startArcadeGame('snake')" class="arcade-card">
              ...
              <div class="arcade-info">
                <h4>Cyber Snake</h4>
                <p>El clsico inmortal con un toque cyberpunk.</p>
                <div class="arcade-score">
                  <span>? High Score:</span>
      ...
      "quick-chat": {
        ...
        <div id="chat-panel" style="padding:20px; color:var(--ink-1); display:flex; flex-direction:column; height:450px;">
          ...
          <div id="chat-messages" style="flex:1; overflow-y:auto; display:flex; flex-direction:column; gap:12px; padding:10px; background:rgba(0,0,0,0.2); border-radius:16px; border:1px solid rgba(255,255,255,0.05); margin-bottom:16px; scroll-behavior:smooth;">
            <div style="text-align:center; padding:20px; color:var(--ink-2); font-size:13px;">Iniciando conexin segura...</div>
          </div>
          ...
          <div style="display:flex; gap:10px; background:rgba(255,255,255,0.03); padding:10px; border-radius:16px; border:1px solid rgba(255,255,255,0.08);">
            <input type="text" id="chat-input" placeholder="Enva un mensaje a la comunidad..." 
      ...
      "mini-calendar": {
        ...
        description: "Calendario avanzado con categoras y lista de agenda.",
        rarity: "poco_comun",
        type: "active",
        version: "2.0.0",
        icon: "?",
        ...
        <div id="calendar-panel" style="padding:20px;">
          ...
          <div style="background:var(--bg-1); border-radius:16px; padding:20px; box-shadow:0 8px 24px rgba(0,0,0,0.1);">
            ...
            <div id="add-event-form" style="display:none; margin-bottom:20px; padding-bottom:20px; border-bottom:1px solid rgba(255,255,255,0.08); animation: fadeIn 0.3s ease;">
              <input type="text" id="event-title" placeholder="Ttulo del evento..." style="width:100%; padding:12px; border-radius:10px; background:var(--bg-2); border:1px solid rgba(255,255,255,0.1); color:var(--ink-0); margin-bottom:10px;">
    ...
    function loadNotes() {
      ...
      if (notes.length === 0) {
        list.innerHTML = '<p style="color:var(--ink-1);text-align:center;padding:20px;">No hay notas an. Crea tu primera nota arriba.</p>';
        return;
      }
    ...
    <div class="card" style="margin-bottom:12px;padding:16px;">
      ...
      <div style="display:flex;gap:8px;">
        ...
        <button class="btn" onclick="deleteNote(${i})">? Eliminar</button>
      </div>
    </div>
  `).join('');
    }
    ...
    window.saveNote = async function () {
      ...
      if (!title || !content) {
        showAlert('warning', 'Campos vacos', 'Por favor completa el ttulo y contenido de la nota.');
        return;
      }
      ...
      // Verificar misin de nota extensa (ms de 100 caracteres)
    ...
    window.viewNoteDetail = async function (index) {
      ...
      if (!wasViewed) {
        ...
        // Actualizar tracking de misin "use_notes"
        const state = await loadState();
        if (!state.missionProgress) state.missionProgress = {};
        if (!state.missionProgress.notes) state.missionProgress.notes = {};
        ...
        // Contar cuntas notas nicas han sido vistas
    ...
    <div class="missions-container" style="max-width:700px;">
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding-top:16px;border-top:1px solid rgba(255,255,255,.1);">
        ...
        <button class="btn" onclick="deleteNote(${index}); this.closest('.missions-modal').remove();" style="background:rgba(255,107,107,.2);border-color:var(--danger);color:var(--danger);">? Eliminar</button>
      </div>
    </div>
  `;
    ...
    <div class="card" style="margin-bottom:12px;padding:16px;border-left:3px solid var(--accent);">
      ...
      <p style="margin:0;color:var(--ink-1);font-size:13px;">Ecoxion ha sido renovado con una base ms limpia y moderna.</p>
    </div>
  `;
    }
    ...
    window.publishBlog = async function () {
      ...
      if (!title || !content) {
        showAlert('error', 'Campos vacos', 'Completa ttulo y contenido.');
        return;
      }

      const state = await loadState();
      const author = state.username || 'Annimo';
      ...
      try {
        ...
        showAlert('error', 'Error', 'No se pudo publicar el blog. Intenta ms tarde.');
      }
    };
    ...
    async function loadBlogs() {
      ...
      try {
        ...
        if (!Array.isArray(blogs) || blogs.length === 0) {
          list.innerHTML = '<div style="text-align:center; padding:40px; color:var(--ink-2); background:rgba(255,255,255,0.01); border-radius:12px; border:1px dashed rgba(255,255,255,0.1);">No hay blogs an. S el primero en compartir!</div>';
          return;
        }
          ...
          return `
            ...
            >
              <div style="display:flex; align-items:center; gap:12px; margin-bottom:14px;">
                ...
                <div>
                  <div style="font-size:13px; font-weight:700; color:var(--ink-0);">${blog.author || 'Annimo'}</div>
              ...
              <div style="margin-top:16px; display:flex; gap:10px;">
                ...
                <button class="btn like-btn" style="padding:6px 12px; font-size:12px; opacity:0.8;" onclick="likeBlog(${blog.id}, this)">? ${blog.likes || 0}</button>
              </div>
            </div>
          `;
        }).join('');
      } catch (e) {
        list.innerHTML = '<div style="color:var(--danger); text-align:center; padding:20px;">? Error al conectar con la red de blogs.</div>';
      }
    }
    ...
    async function updateGeneratorPanel() {
      ...
      // Actualizar estadsticas y nueva UI
      const streakBadge = document.getElementById('streak-badge');
      if (streakBadge) streakBadge.textContent = `Racha: ${generatorData.streak || 0} das`;
      ...
      if (canClaim) {
        statusEl.innerHTML = '<p style="color:var(--ok);margin:0;font-weight:700;">? Energa acumulada!</p>';
        claimBtn.disabled = false;
        claimBtn.innerHTML = 'Reclamar Recompensa';
        claimBtn.style.opacity = '1';
      } else {
        ...
        statusEl.innerHTML = `<p style="color:var(--ink-1);margin:0;">Generando energa... <br>Disponible en: <strong>${h}h ${m}m</strong></p>`;
    ...
    window.claimDailyReward = async function () {
      ...
      if (lastClaim) {
        ...
        if (hoursSinceLastClaim < 20) {
          const hoursLeft = 20 - hoursSinceLastClaim;
          showAlert('warning', 'An no disponible', `Debes esperar ${Math.ceil(hoursLeft)} horas ms.`);
          return;
        }
      }
      ...
      // Guardar datos especficos del generador
      ...
      if (newStreak > 1) message += `. Racha: ${newStreak} das ?`;

      showAlert('success', 'Recompensa reclamada', message);

      // Animacin simple de confeti (si existiera librera, por ahora visual)
    ...
    async function updateEcoLuckPanel(currentState = null) {
      ...
      // NO guardamos aqu para evitar bucles, solo leemos
      ...
      if (!data.lastResult) {
        lastResultEl.textContent = 'An no has jugado.';
      ...
      if (planBoostEl) {
        ...
        planBoostEl.innerHTML = isBoosted
          ...
          : `? Plan base activo: puedes mejorar tu suerte con planes premium.`;
      }
    }
    ...
    window.playEcoLuck = async function () {
      ...
      // Usar nuestro helper para asegurar sincrona
      ...
      if (resultPop) {
        resultPop.textContent = prize > 0 ? `+${prize} ?` : 'Sin premio';
    ...
    // Lock para prevenir clics simultneos
    let isProcessingClick = false;

    // Sistema de deteccin de auto-clicker
    let clickHistory = []; // Array de timestamps de clics
    const MAX_CLICKS_PER_SECOND = 8; // Mximo 8 clics por segundo (humano normal)
    const CLICK_HISTORY_WINDOW = 1000; // Ventana de 1 segundo
    const AUTOCLICKER_PENALTY_MS = 3000; // Penalizacin de 3 segundos si se detecta auto-clicker

    window.clickCoin = async function () {
      const now = Date.now();
      if (isProcessingClick) return;

      // Limpiar clics antiguos (ms de 1 segundo)
      ...
      try {
        ...
        // Verificar lmite diario
        if (clickyData.clicksToday >= 50) {
          showAlert('warning', 'Lmite alcanzado', 'Has alcanzado el lmite diario de 50 clics. Vuelve maana.');
          if (coinBtn) {
            coinBtn.style.pointerEvents = 'auto';
            coinBtn.style.opacity = '1';
          }
        ...
        // NO agregamos al balance principal todava (se hace al retirar)
    ...
    window.withdrawEcoxionums = async function () {
      ...
      try {
        ...
        await updateBalance(); // Ahora s podemos llamar a updateBalance con seguridad
        ...
        showAlert('error', 'Error', error.message || 'No se pudo completar el retiro. Intenta ms tarde.');
      }
    };
    ...
    async function updateWithdrawButton(currentState = null) {
      ...
      // Habilitar botn solo si hay fondos y est vinculado
    ...
    window.awakenExtension = async function (extId) {
      ...
      if (!ext || !ext.hasV2) {
        showAlert('error', 'Error', 'Esta extensin no tiene versin V2 disponible.');
        return;
      }
      ...
      if (!extInfo) {
        showAlert('error', 'Error', 'La extensin debe estar instalada para despertarla.');
        return;
      }

      if (extInfo.awakened) {
        showAlert('info', 'Ya despertada', 'Esta extensin ya est en su versin V2.');
        return;
      }

      // Verificar si todas las misiones estn completas
      ...
      // Si todas las misiones estn completas, iniciar Sincronizacin de Despertar
      showUltimateAwakening(ext, async () => {
        ...
        showAlert('success', 'Sincronizacin Completa!', `${ext.name} ha alcanzado el nivel V2. Nuevas capacidades desbloqueadas.`);
      });
    };

    function showUltimateAwakening(extOrId, onComplete) {
      // 0. Resolver extensin si es string
      const ext = typeof extOrId === 'string' ? defaultExtensions[extOrId] : extOrId;
      if (!ext) {
        console.error('Extensin no encontrada para animacin:', extOrId);
      ...
      // Fase 2: Carga de Energa (Partculas implosionan RPIDO)
    ...
    window.exportNotes = function () {
      showAlert('info', 'Exportar PDF', 'Funcin V2: Exportacin a PDF prximamente.');
    };
    ...
    const wordBattle = {
      ...
      init() {
        ...
        // Conectar al servidor produccin
      ...
      copyRoomCode() {
        ...
        this.showMessage('Cdigo copiado', 'success');
      },
      ...
      updatePlayersList(currentTurnId) {
        ...
        grid.innerHTML = this.players.map(p => {
          ...
          const livesHtml = Array(3).fill(0).map((_, i) => i < p.lives ? '?' : '?').join('');
    ...
    const miniGamesConfig = {
      clickGame: { id: 'clickGame', name: 'Click Rpido', description: 'Haz tantos clicks como puedas en 10s.', color: 'linear-gradient(135deg, #667eea, #764ba2)', model: 'coin' },
      ...
      numberGuess: { id: 'numberGuess', name: 'Adivina el Nmero', description: 'Adivina el nmero entre 1 y 100.', color: 'linear-gradient(135deg, #84fab0, #8fd3f4)', model: 'dice' },
      mathChallenge: { id: 'mathChallenge', name: 'Reto Matemtico', description: 'Resuelve operaciones matemticas.', color: 'linear-gradient(135deg, #ff9a9e, #fecfef)', model: 'calculator' },
      reactionTest: { id: 'reactionTest', name: 'Test de Reaccin', description: 'Click cuando cambie el color.', color: 'linear-gradient(135deg, #a18cd1, #fbc2eb)', model: 'bolt' },
    ...
    const dailyRewards = {
      ...
      async claimDaily() {
        ...
        // Sincronizar con servidor ANTES de actualizar UI si est vinculado
        if (state.oceanPayLinked && state.userId) {
          try {
            const res = await fetch(`${API_GLOBAL}/ocean-pay/ecoxionums/change`, {
              method: 'POST',
      ...
      async updateDailyUI() {
        ...
        if (btn && timer) {
          if (timeSince >= oneDay) {
            ...
            timer.textContent = `Prxima recompensa en ${hours}h ${minutes}m`;
      ...
      async tryGrantMissionReward(missionId, rewardAmount, successMessage) {
        ...
        showAlert('success', 'Misin completada!', successMessage || `+${rewardAmount} Ecoxionums`);
      ...
      async promptMiniGameReset(gameId) {
        ...
        if (planId !== 'ultra') {
          showAlert('info', 'Plan Ultra Nova requerido', `La recarga instantnea cuesta ${cost} Ecoxionums y est disponible solo en Ultra Nova.`);
        ...
        if (!result?.ok) {
          ...
          if (result?.reason === 'not_locked') {
            showAlert('info', 'No es necesario', `${gameName} ya est disponible.`);
            return;
          }
          ...
          showAlert('error', 'No se pudo recargar', 'Intntalo de nuevo en unos segundos.');
          return;
        }

        showAlert('success', 'Recarga aplicada', `${gameName} qued disponible. -${cost} Ecoxionums.`);
      ...
      async refreshMiniGameRealtimeUI() {
        ...
        container.querySelectorAll('[data-minigame-card="1"]').forEach((card) => {
          ...
          if (pillEl) pillEl.textContent = `? Recarga: ${cd.remainingLabel}`;
        });
              ...
              <div style="margin-top:8px; position:relative; z-index:3;">
                ${isUltraPlan ? `
                  <button data-mini-reset="${game.id}" style="width:100%; border:none; border-radius:12px; padding:10px 12px; font-weight:700; color:#07121f; background:linear-gradient(90deg,#41f3ff,#7ef9ff); cursor:pointer;">
                    ? Recargar ahora  ${resetCost} Ecoxionums
                  </button>
                ` : `
                  <div style="padding:10px 12px; border-radius:12px; background:rgba(255,184,77,0.08); border:1px solid rgba(255,184,77,0.28); color:#ffd08a; font-size:12px; line-height:1.35;">
                    Reset instantneo disponible con <b>Ultra Nova</b>. Puedes recargar minijuegos agotados por ${resetCost} Ecoxionums.
                  </div>
            ...
            ${isLocked ? `
              <div class="dr-cooldown-overlay">
                <div class="dr-cooldown-pill">
                  ...
                  <span class="dr-cooldown-pill-text">? Recarga: ${cooldown.remainingLabel}</span>
                </div>
              </div>
            ` : ''}
          `;
      ...
      async playClickGame() {
        ...
        if (gameData?.cooldown?.locked) {
          showAlert('warning', 'Lmite alcanzado', `Disponible en ${gameData.cooldown.remainingLabel} (${gameData.cooldown.windowLabel}).`);
          return;
        }
        ...
        const interval = setInterval(async () => {
          ...
          if (timeLeft <= 0) {
            ...
            if (playReg?.blocked) {
              overlay.remove();
              showAlert('warning', 'Lmite alcanzado', `Disponible en ${playReg.cooldown?.remainingLabel || 'pronto'} (${playReg.cooldown?.windowLabel || ''}).`);
              this.renderMiniGamesSection();
              return;
            }

            await this.giveReward(reward, 'Click Rpido');
      ...
      async playNumberGuess() {
        ...
        overlay.innerHTML = `
          <div class="minigame-modal">
            <h2 style="margin-bottom:10px;">? Adivina el Nmero</h2>
            <p>Entre 1 y 100</p>
            <div id="ng-hint" class="hint-text" style="font-size:1.2em; font-weight:700; margin:15px 0;">Empieza!</div>
            <input type="number" id="ng-input" class="game-input" placeholder="Tu nmero" style="width:100%; font-size:1.5em; text-align:center; margin-bottom:16px; border-radius:12px;">
        ...
        const guess = async () => {
          ...
          if (num === target) {
            ...
            await this.giveReward(reward, 'Adivina el Nmero');
            ...
            hint.textContent = '? Ms alto';
            hint.style.color = 'var(--accent)';
          } else {
            hint.textContent = '? Ms bajo';
      ...
      async playMathChallenge() {
        ...
        overlay.innerHTML = `
          <div class="minigame-modal">
            <h2 style="margin-bottom:10px;">? Reto Matemtico</h2>
        ...
        const checkMath = (val) => {
          ...
          if (val === currentAnswer) {
            ...
            timeLeft -= 3; // Penalizacin reducida para balance
        ...
        const startMathGame = () => {
          ...
          const interval = setInterval(async () => {
            ...
            if (timeLeft <= 0) {
              ...
              await this.giveReward(reward, 'Reto Matemtico');
      ...
      async playReactionTest() {
        ...
        area.addEventListener('click', async (e) => {
          ...
          if (waiting) {
            ...
            await this.giveReward(Math.max(5, reward), 'Test de Reaccin');

            setTimeout(() => {
              overlay.remove();
              showAlert('success', 'Rpido!', `${time}ms = +${Math.max(5, reward)} ?`);
              this.renderMiniGamesSection();
            }, 800);
          }
        });
      ...
      async giveReward(amount, source) {
        try {
          ...
          // Verificar si se complet la misin "Jugador" (play_3_minigames)
          const today = new Date().toDateString();
          let missionReward = 0;
          ...
          if (missionReward > 0) {
            setTimeout(() => {
              showAlert('success', 'Misin Completada!', `Has jugado 3 mini-juegos. Recompensa: +${missionReward} ?`);
            }, 500);
          }
        } catch (error) {
    ...
    async function checkMissionProgress(extId) {
      ...
      for (const mission of ext.missions) {
        ...
        // Verificar progreso segn el tipo de misin
        switch (mission.type) {
          case 'count':
            ...
            break;
          case 'length':
            // Para nota extensa - verificar si hay alguna nota con ms de 100 caracteres
            if (extId === 'notes') {
              const notes = JSON.parse(localStorage.getItem('ecoxion.notes') || '[]');
  ...
  `;
      ...
      // Si todas estn completas, animar el botn fragmentado
      if (allComplete) {
        setTimeout(() => {
      ...
      window.triggerAwakening = async function (extId) {
        ...
        setTimeout(async () => {
          await showUltimateAwakening(extId, async () => {
            ...
            showAlert('success', 'Despertada!', 'La extensin ha sido despertada a V2.');
          });
        }, 300);
      };
    ...
    function showAlert(type, title, message, duration = 5000) {
      ...
      alert.innerHTML = `
    <span class="alert-icon">${type === 'success' ? '?' : type === 'error' ? '?' : type === 'warning' ? '?' : '?'}</span>
    <div>
  ...
  `).join('');
    ...
    async function renderDashboard() {
      ...
      <div class="card" style="text-align:center;padding:40px;">
        ...
        <p style="color:var(--ink-1);">Ve al Marketplace para instalar tu primera extensin.</p>
  ...
  `;
    ...
    `;
        ...
        // Cargar contenido del panel de forma asncrona
        (async () => {
      ...
      if (clickyPanel) {
        updateClickyCoinPanel();
        // Actualizar botn de retiro peridicamente
      ...
      if (installedExtensions.length === 0) {
        content.innerHTML = `
          <div class="card" style="text-align:center;padding:60px 40px;">
            ...
            <p style="color:var(--ink-1);margin:0 0 24px;">Explora el Marketplace para descubrir extensiones increbles</p>
            ...
            <div style="display:flex; align-items:start; gap:20px; padding-top:8px;">
              ...
              '<span style="padding:4px 10px; background:rgba(239,68,68,.15); border:1px solid rgba(239,68,68,.3); border-radius:8px; font-size:11px; font-weight:600; color:var(--danger);">? Deshabilitada</span>'
                ...
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:12px; flex-wrap:wrap;">
                  <span style="font-size:12px; color:var(--ink-1);"><strong>Versin:</strong> ${ext.awakened && ext.version2 ? ext.version2 : ext.version}</span>
                ...
                <div style="display:flex; gap:10px; flex-wrap:wrap;">
                  ...
                  `}
                  ${!ext.defaultInstall ? `
                    <button class="btn" onclick="uninstallExtension('${ext.id}')" style="background:var(--danger); border-color:var(--danger); min-width:140px;">? Desinstalar</button>
                  ` : `
    ...
    window.toggleExtension = async function (extId) {
      ...
      if (newStatus) {
        showAlert('success', 'Extensin habilitada', `${ext.name} ha sido habilitada.`);
        if (typeof ext.onEnable === 'function') ext.onEnable(extInfo);
      } else {
        showAlert('info', 'Extensin deshabilitada', `${ext.name} ha sido deshabilitada.`);
    ...
    window.uninstallExtension = async function (extId) {
      ...
      if (ext.defaultInstall) {
        showAlert('warning', 'No se puede desinstalar', 'Esta extensin es predeterminada y no se puede desinstalar.');
        return;
      }

      const confirmed = confirm(`Ests seguro de que deseas desinstalar "${ext.name}"?\n\nEsta accin no se puede deshacer.`);
      ...
      showAlert('success', 'Extensin desinstalada', `${ext.name} ha sido desinstalada correctamente.`);
      renderMyExtensions();
    };
  ...
  `;
    ...
    const packagesConfig = {
      'starter': {
        ...
        description: 'Ideal para nuevos exploradores. Contiene extensiones bsicas.',
        price: 100,
        icon: '?',
        color: 'var(--ok)',
      ...
      'advanced': {
        ...
        description: 'Mejora tu arsenal con herramientas ms potentes.',
        price: 250,
        icon: '?',
        color: 'var(--accent)',
      ...
      'elite': {
        id: 'elite',
        name: 'Paquete de lite',
        description: 'Solo para los ms exigentes. Alta probabilidad de rarezas.',
        price: 600,
        icon: '?',
        color: 'var(--accent-2)',
      ...
      'developer': {
        ...
        description: 'Herramientas enfocadas en productividad y cdigo.',
        price: 350,
        icon: '??',
        color: '#F59E0B',
        theme: 'dev',
      ...
      content.innerHTML = `
        <div class="card" style="margin-bottom:24px;text-align:center;background:linear-gradient(135deg, rgba(100,167,255,.1), rgba(179,136,255,.1));border:1px solid var(--accent);">
          ...
          <p style="color:var(--ink-1);max-width:600px;margin:0 auto 24px;">
            Adquiere paquetes sorpresa y obtn extensiones aleatorias. Puedes conseguir extensiones picas o Legendarias!
          </p>
          ...
          ${Object.values(packagesConfig).map(pkg => `
            <div class="package-card" onclick="buyPackage('${pkg.id}')">
              ...
              <div style="display:flex;justify-content:center;gap:4px;margin-bottom:16px;">
                ...
                ${pkg.probabilities.epica > 0 ? `<span title="pica: ${pkg.probabilities.epica}%" style="width:8px;height:8px;border-radius:50%;background:#ffd700;"></span>` : ''}
                ...
                ${pkg.probabilities.poco_comun > 0 ? `<span title="Poco Comn: ${pkg.probabilities.poco_comun}%" style="width:8px;height:8px;border-radius:50%;background:#64a7ff;"></span>` : ''}
                <span title="Comn: ${pkg.probabilities.comun}%" style="width:8px;height:8px;border-radius:50%;background:#b4c2df;"></span>
              </div>
    ...
    async function openPackage(pkg) {
      ...
      // 2. Seleccionar extensin de esa rareza
      ...
      if (pkg.theme) {
        // Si el paquete tiene tema, intentar filtrar por tema (esto requerira que las extensiones tengan tags o categoras)
        // Por ahora, simularemos preferencia por ciertas extensiones segn el ID del paquete
        if (pkg.id === 'gamer') {
      ...
      // Si no hay extensiones de esa rareza (ej. legendaria), bajar de categora
    ...
    function showOpeningAnimation(pkg, reward) {
      ...
      // 2. Click para abrir (o auto despus de unos segundos)
      ...
      function triggerOpen() {
        ...
        setTimeout(async () => {
          ...
          if (!state.installed[reward.id]) {
            ...
            await saveState(state);
            showAlert('success', 'Nueva extensin!', `${reward.name} aadida a tu coleccin.`);
          } else {
            // Ya la tiene: dar compensacin
            const compensation = Math.floor(pkg.price * 0.3);
            state.ecoxionums += compensation;

            // Sincronizar compensacin
            if (state.oceanPayLinked && state.userId) {
              await fetch(`${API_GLOBAL}/ocean-pay/ecoxionums/change`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                  userId: state.userId,
                  amount: compensation,
                  concepto: `Compensacin repetida: ${reward.name}`,
                  origen: 'Ecoxion'
                })
              }).catch(console.warn);
            }
            ...
            msg.innerHTML = `<p style="color:var(--ok);margin-top:12px;">?Ya la tenas! Compensacin: +${compensation} ?</p>`;
    ...
    async function renderMarketplace() {
      ...
      header.innerHTML = `
        ...
        <p style="color:var(--ink-1); font-size:16px; max-width:600px; margin:${isZen ? '0' : '0 auto'};">
           Potencia tu experiencia con herramientas, juegos y utilidades diseadas para el ecosistema.
        </p>
      `;
      content.appendChild(header);
          ...
          item.innerHTML = `
            ...
            ${hasV2Available ? `
              <div style="padding:10px; background:linear-gradient(90deg, rgba(255,215,0,0.1), transparent); border-left:3px solid #ffd700; border-radius:4px;">
                <p style="margin:0; font-size:12px; color:#ffd700; font-weight:600;">? Versin V2 Disponible!</p>
              </div>
            ` : ''}
      ...
      // Renderizar los iconos Canvas despus de insertar en el DOM
      Object.values(defaultExtensions).forEach(ext => {
        const rarityColors = { comun: '#94a3b8', poco_comun: '#64a7ff', rara: '#60a5fa', epica: '#a855f7', legendaria: '#f59e0b' };
    ...
    async function installExtension(id) {
      ...
      if (ext.price > 0) {
        if (state.ecoxionums < ext.price) {
          showAlert('error', 'Saldo insuficiente', `Necesitas ${ext.price} Ecoxionums para comprar esta extensin.`);
          return;
        }
      }

      // Instalacin Optimista
      ...
      showAlert('success', 'Extensin instalada', `${ext.name} se ha instalado correctamente.`);
      if (currentTab === 'marketplace') renderMarketplace();
      else if (currentTab === 'dashboard') renderDashboard();

      // Sincronizacin en segundo plano si hay precio y Ocean Pay
      if (ext.price > 0 && state.oceanPayLinked && state.userId) {
        fetch(`${API_GLOBAL}/ocean-pay/ecoxionums/change`, {
          ...
          body: JSON.stringify({
            userId: state.userId,
            amount: -ext.price,
            concepto: `Compra de extensin: ${ext.name}`,
            origen: 'Ecoxion'
          })
    ...
    async function uninstallExtension(id) {
      ...
      showAlert('info', 'Extensin desinstalada', 'La extensin se ha desinstalado correctamente.');
    ...
    const PLAN_ADVANTAGE_EXTENSION_CATALOG = [
      { id: 'eco-luck', name: 'Ecoxionum Fortuna', icon: '?', gradient: 'linear-gradient(135deg,#ffd86a,#ff8b42)' },
      { id: 'eco-generator', name: 'Generador Ecoxionums', icon: '?', gradient: 'linear-gradient(135deg,#8cf0ff,#4f8cff)' },
      { id: 'clicky-coin', name: 'Clicky Coin', icon: '?', gradient: 'linear-gradient(135deg,#64a7ff,#9c7dff)' },
      { id: 'eco-stock', name: 'Eco Stock Market', icon: '?', gradient: 'linear-gradient(135deg,#56f2b0,#3f9bff)' },
      { id: 'quick-surveys', name: 'Quick Surveys', icon: '?', gradient: 'linear-gradient(135deg,#7ee0ff,#8e9dff)' },
      { id: 'smart-notes', name: 'Smart Notes', icon: '?', gradient: 'linear-gradient(135deg,#91a8ff,#4cc9f0)' }
    ];

    const PLAN_ADVANTAGE_MATRIX_DEFAULT = {
      free: {
        'eco-luck': ['Tabla base de probabilidades.', 'Costo por tirada: 25 ?.'],
        'eco-generator': ['1 reclamo cada 20h.', 'Bono de racha estndar.'],
        'clicky-coin': ['Lmite diario estndar (50 clics).'],
        'eco-stock': ['Acceso al mercado base.'],
        'quick-surveys': ['Encuestas normales sin prioridad.'],
        'smart-notes': ['Funciones base de edicin y guardado local.']
      },
      plus: {
        'eco-luck': ['Suerte aumentada: sube chance de x3/x10.', 'Prdida total reducida frente al plan base.'],
        'eco-generator': ['Eficiencia de generacin mejorada.', 'Mejor rendimiento en rachas intermedias.'],
        'clicky-coin': ['Mejor respuesta visual y recompensas consistentes.'],
        'eco-stock': ['Panel de movimiento con lectura ms rpida.'],
        'quick-surveys': ['Acceso a ms encuestas activas por ciclo.'],
        'smart-notes': ['Capas de organizacin adicionales.']
      },
      pro: {
        ...
        'clicky-coin': ['Optimizacin de flujo en sesiones largas.'],
        'eco-stock': ['Mejoras de seales y lectura de tendencia.'],
        'quick-surveys': ['Prioridad de tareas con mejor recompensa media.'],
        'smart-notes': ['Herramientas avanzadas de estructura y foco.']
      },
      ultra: {
        'eco-luck': ['Suerte Ultra Nova: mxima probabilidad de x3/x10.', 'Mitigacin alta de tiradas fallidas.'],
        'eco-generator': ['Rendimiento mximo y consolidacin de rachas.', 'Mejor estabilidad en ciclos largos.'],
        'clicky-coin': ['Flujo experto + mejor consistencia de sesin.'],
        'eco-stock': ['Lectura avanzada con ejecucin de alto nivel.'],
    ...
    async function loadEcoxionPlanConfig(force = false) {
      ...
      try {
        ...
        const res = await fetch(`${API_GLOBAL}/api/ecoxion/plans/config?nocache=${Date.now()}`, {
          cache: 'no-store',
    ...
    function ensurePlanAdvantageModal() {
          ...
          <div class="eco-adv-modal">
            <div class="eco-adv-header">
              <div>
                ...
                <div id="plan-adv-subtitle" style="font-size:12px;color:var(--ink-1);">Detalle por extensin</div>
              </div>
              <button type="button" class="btn" data-close-plan-adv="1" style="padding:8px 12px;">Cerrar</button>
            </div>
      ...
      bindPlanAdvantageEventsOnce();
    }

    function primeEcoxionPlanConfigFallback() {
      if (ecoxionPlanConfigCache.config && typeof ecoxionPlanConfigCache.config === 'object') return;
      ecoxionPlanConfigCache = {
        ts: Date.now(),
        config: {
          planAdvantages: PLAN_ADVANTAGE_MATRIX_DEFAULT,
          fortuneOdds: FORTUNE_ODDS_DEFAULT
    ...
    function bindPlanAdvantageEventsOnce() {
      if (planAdvantageEventsBound) return;
      planAdvantageEventsBound = true;
      document.addEventListener('click', async (event) => {
        const overlay = event.target && event.target.id === 'plan-adv-overlay' ? event.target : null;
        if (overlay) {
          closePlanAdvantagesModal();
          return;
        }

        const closeBtn = event.target && event.target.closest ? event.target.closest('[data-close-plan-adv]') : null;
        if (closeBtn) {
        ...
        const expandBtn = event.target && event.target.closest ? event.target.closest('.eco-plan-expand-btn[data-open-plan]') : null;
        if (expandBtn) {
          event.preventDefault();
          event.stopPropagation();
          await openPlanAdvantagesModal(expandBtn.getAttribute('data-open-plan') || 'free');
          return;
        }

        const extBtn = event.target && event.target.closest ? event.target.closest('.eco-adv-ext-item[data-plan-id][data-extension-id]') : null;
        if (extBtn) {
          event.preventDefault();
          await openPlanAdvantagesModal(
            extBtn.getAttribute('data-plan-id') || 'free',
            extBtn.getAttribute('data-extension-id') || ''
          );
        }
      });
    }

    function bindPlanExpandButtons(scopeRoot) {
      const root = scopeRoot || document;
      const buttons = Array.from(root.querySelectorAll('.eco-plan-expand-btn[data-open-plan]'));
      buttons.forEach((btn) => {
        if (btn.dataset.boundPlanAdv === '1') return;
        btn.dataset.boundPlanAdv = '1';
        btn.addEventListener('click', async (event) => {
          event.preventDefault();
          event.stopPropagation();
          await openPlanAdvantagesModal(btn.getAttribute('data-open-plan') || 'free');
        });
      });
    }

    function renderInlinePlanAdvantages(planId) {
      const normalizedPlan = normalizeEcoxionPlanId(planId || 'free');
      const matrixAll = getPlanAdvantageMatrix();
      const matrix = matrixAll[normalizedPlan] || matrixAll.free || {};
      return PLAN_ADVANTAGE_EXTENSION_CATALOG.map((ext) => {
        const benefits = Array.isArray(matrix[ext.id]) && matrix[ext.id].length
          ? matrix[ext.id]
          : ['Sin ventaja especfica documentada en este plan.'];
        return `
          <div style="border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:10px;background:rgba(255,255,255,.02);display:grid;gap:6px;">
            <div style="display:flex;align-items:center;gap:8px;">
              <div class="eco-adv-model" style="width:28px;height:28px;font-size:14px;background:${ext.gradient};">${ext.icon}</div>
              <div style="font-size:12px;font-weight:700;color:#d9ecff;">${ext.name}</div>
            </div>
            <ul style="margin:0;padding-left:16px;display:grid;gap:4px;">
              ${benefits.map((b) => `<li style="font-size:11px;color:var(--ink-1);line-height:1.35;">${b}</li>`).join('')}
            </ul>
          </div>
        `;
      }).join('');
    ...
    function renderPlanAdvantagesModal() {
      ...
      subtitle.textContent = 'Selecciona una extensin para ver beneficios concretos del plan.';

      extList.innerHTML = PLAN_ADVANTAGE_EXTENSION_CATALOG.map((ext) => {
        const isActive = ext.id === planAdvantageModalState.extensionId;
        return `
          <button type="button" class="eco-adv-ext-item ${isActive ? 'active' : ''}" data-plan-id="${planId}" data-extension-id="${ext.id}">
      ...
      const benefits = matrix[selected.id] || ['Sin ventaja especfica documentada en este plan.'];
      detail.innerHTML = `
    ...
    function openPlanAdvantagesModal(planId, extensionId = '') {
      primeEcoxionPlanConfigFallback();
      ...
      loadEcoxionPlanConfig(false)
        .then(() => {
          if (!planAdvantageModalState.open) return;
          renderPlanAdvantagesModal();
        })
        .catch(() => {});
    }
    window.openPlanAdvantagesModal = openPlanAdvantagesModal;

    function closePlanAdvantagesModal() {
      planAdvantageModalState.open = false;
      const overlay = document.getElementById('plan-adv-overlay');
      if (overlay) overlay.classList.remove('active');
    }
    window.closePlanAdvantagesModal = closePlanAdvantagesModal;
    ...
    async function renderSubscriptions() {
      ...
      const plans = [
        ...
        },
        {
          ...
          priceLabel: '1250 ?',
          cycle: 'cada 30 dias',
          features: ['Todo Quasar Prime', 'Toolset experto completo', 'Canal prioritario + extras visuales', 'Minijuegos: 5 usos  recarga cada 2h + reset instantneo']
        }
      ];
      ...
      ` : (fallbackPlan !== 'free'
        ? `<div class="eco-sub-meta"><div><strong>Plan activo:</strong> ${fallbackPlanLabel}</div><div>Sincronizaci?n con servidor pendiente. Beneficios locales activos.</div></div>`
        ...
        <div class="eco-subs-shell">
          <div class="eco-subs-header">
            <div>
              ...
              <p style="margin:8px 0 0;color:var(--ink-1);">Sincronizadas con Ocean Pay (renovaci?n y saldo en tiempo real).</p>
            </div>
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
              <button
                type="button"
                id="eco-plan-expand-active"
                class="eco-plan-expand-btn"
                data-open-plan="${activePlan || 'free'}"
                style="width:auto;min-width:210px;">
                Expandir Ventajas (Plan activo)
              </button>
            </div>
            ${subscriptionMeta}
          </div>
          ...
          <div class="eco-subs-grid">
            ${plans.map((plan) => {
              ...
              const action = plan.id === 'free'
                ...
                : (isActive
                  ? '<button class="btn" style="width:100%;background:rgba(255,107,107,.15);border-color:var(--danger);color:var(--danger);" onclick="cancelSubscription()">Cancelar renovaci?n</button>'
                  : (downgradeLocked
                    ? `<button class="btn" style="width:100%;opacity:.7;cursor:not-allowed;" disabled>No disponible desde tu plan actual</button>`
              ...
              return `
                <article class="${cardClass}">
                  ...
                  <details style="border:1px solid rgba(100,167,255,.2);border-radius:12px;overflow:hidden;background:rgba(8,15,28,.42);">
                    <summary class="eco-plan-expand-btn" style="list-style:none;">Expandir Ventajas</summary>
                    <div style="padding:10px;display:grid;gap:8px;max-height:240px;overflow:auto;">
                      ${renderInlinePlanAdvantages(plan.id)}
                    </div>
                  </details>
                  ${action}
                </article>
          ...
          <div class="eco-compare">
            ...
            <div class="eco-compare-grid">
              ...
              <div class="eco-compare-cell"><span class="eco-compare-x">?</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">?</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">?</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">?</span></div>
              ...
              <div class="eco-compare-cell feature">Reset instantneo con Ecoxionums</div>
              <div class="eco-compare-cell"><span class="eco-compare-x">?</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-x">?</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-x">?</span></div>
              <div class="eco-compare-cell"><span class="eco-compare-check">?</span></div>
      ...
      `;
      bindPlanExpandButtons(content);
      const quickExpand = document.getElementById('eco-plan-expand-active');
      if (quickExpand && quickExpand.dataset.boundPlanAdv !== '1') {
        quickExpand.dataset.boundPlanAdv = '1';
        quickExpand.addEventListener('click', async (event) => {
          event.preventDefault();
          event.stopPropagation();
          await openPlanAdvantagesModal(quickExpand.getAttribute('data-open-plan') || activePlan || 'free');
        });
      }
    }
    ...
    window.cancelSubscription = async function () {
      ...
      const confirmed = confirm('Cancelar la renovaci?n automatica? Mantendras beneficios hasta la fecha de vencimiento.');
      if (!confirmed) return;
      ...
      try {
        ...
        showAlert('success', 'Suscripcion cancelada', 'La renovaci?n automatica fue cancelada correctamente.');
        setTimeout(() => renderSubscriptions(), 350);
      } catch (error) {
    ...
    async function renderStatistics() {
      const content = document.getElementById('content');
      content.innerHTML = '<div style="text-align:center;padding:40px;color:var(--ink-1);">Cargando estadsticas maestras...</div>';
      ...
      // Datos simulados de actividad (para el grfico)
        ...
        <div class="stats-grid-premium">
          ...
          <div class="stat-card-premium">
             ...
             <div style="display:flex; flex-wrap:wrap; gap:12px;">
                ...
                <div class="achievement-icon ${dailyStreak >= 7 ? 'achievement-unlocked' : ''}" title="Fidelidad (Racha 7 das)">?</div>
        ...
        <div style="margin-bottom:20px;">
          <div style="display:flex;justify-content:space-between;margin-bottom:12px;padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,.05);">
            ...
            <span style="font-weight:600;color:${state.oceanPayLinked ? 'var(--ok)' : 'var(--warn)'};">
              ${state.oceanPayLinked ? '? Vinculado' : '? No vinculado'}
            </span>
          </div>
          ${state.oceanPayLinked ? `
            <div style="display:flex;justify-content:space-between;margin-bottom:12px;">
    ...
    `;
      ...
      // Cargar historial si est conectado
      if (state.oceanPayLinked && state.userId) {
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding:12px;background:rgba(255,255,255,.03);border-radius:8px;border-left:3px solid ${isIncoming ? 'var(--ok)' : 'var(--warn)'};">
                <div>
                  <div style="font-weight:600;font-size:14px;margin-bottom:2px;">${tx.concepto || 'Transaccin'}</div>
    ...
    async function showTransferModal() {
      ...
      modal.querySelector('#tx-send').onclick = async () => {
        ...
        if (!amount || amount <= 0) return showAlert('error', 'Cantidad invlida', 'Ingresa una cantidad mayor a 0.');
        ...
        try {
          ...
          if (res.ok) {
            ...
            renderCommerce(); // Recargar historial (cuando est implementado)
          } else {
            showAlert('error', 'Error', data.error || 'Fall la transferencia.');
            btn.textContent = 'Enviar ?';
            btn.disabled = false;
          }
        } catch (e) {
          console.error(e);
          showAlert('error', 'Error de conexin', 'No se pudo conectar con el servidor.');
          btn.textContent = 'Enviar ?';
          btn.disabled = false;
        }
      };
    ...
    async function renderSettings() {
      ...
      <div style="margin-top:24px;">
        ...
        <select id="theme-select" style="width:100%;padding:10px;border-radius:8px;background:var(--bg-1);border:1px solid rgba(255,255,255,.12);color:var(--ink-0);">
          ...
          <option value="tropical" ${state.currentTheme === 'tropical' ? 'selected' : ''}>? Tropical</option>
        </select>
      </div>
      ...
      <div style="margin-top:24px;">
        <label style="display:flex;align-items:center;gap:12px;cursor:pointer;">
          ...
          <div>
            ...
            <small style="color:var(--ink-1);font-size:11px;">"Dark Mode": Esta es una funcin pasiva que se activa inmediatamente al alternar. No tiene un panel de control.</small>
          </div>
        </label>
      </div>
      ...
      <div style="margin-top:24px;">
        <label style="display:flex;align-items:center;gap:12px;cursor:pointer;">
          ...
          <div>
            ...
            <small style="color:var(--ink-1);font-size:11px;">"Zen Mode": Esta es una funcin pasiva que se activa inmediatamente al alternar. No tiene un panel de control.</small>
          </div>
        </label>
      </div>
      ...
      ${state.username === 'OceanandWild'
          ? `<div style="margin-top:32px;padding-top:24px;border-top:1px solid rgba(255,255,255,.1);">
             <h3 style="margin:0 0 12px;font-size:16px;">? Panel de Admin</h3>
      ...
      <button class="btn primary" onclick="saveSettings()" style="margin-top:24px;">Guardar Configuracin</button>
    </div>
      `;
    ...
    window.saveSettings = function () {
      showAlert('success', 'Configuracin guardada', 'Tus preferencias se han guardado correctamente.');
    };
    ...
    window.adminRequestEcoxionums = async function () {
      ...
      if (isNaN(amount) || amount <= 0) {
        showAlert('error', 'Error', 'Cantidad invlida');
        return;
      }

      await awardEcoxionums(amount, 'Generacin Admin');
    ...
    async function updateBalance() {
      ...
      // Si est vinculado con Ocean Pay, obtener desde servidor para asegurar sincrona
      if (state.oceanPayLinked && state.userId) {
        try {
    ...
    async function awardEcoxionums(amount, concepto = 'Recompensa') {
      ...
      if (state.oceanPayLinked && state.userId) {
        try {
          ...
          if (!token) {
            showAlert('warning', 'Sesin Expirada', 'Tus ganancias no se guardan en la nube. Inicia sesin de nuevo.');
          ...
          if (res.ok) {
            ...
            if (res.status === 401) {
              showAlert('error', 'Sesin Caducada', 'Inicia sesin en Ocean Pay nuevamente para guardar tu progreso.');
            } else {
          ...
          showAlert('warning', 'Sin Conexin', 'Tus ganancias se guardaron localmente pero no en la nube.');
        }
      }
      return state.ecoxionums;
    }
    ...
    function renderNavigation() {
      ...
      existingTabs.forEach(tab => {
        ...
        if (parent && (parent.classList.contains('appbar') || parent.classList.contains('zen-sidebar'))) {
          // Ya se elimin con el padre
        } else {
          tab.remove();
        }
      });

      // Crear nueva navegacin
      ...
      // Pequeo delay para asegurar que el DOM est listo
      setTimeout(() => {
        renderTabs();
        updateBalance();
        updateUserInfo();
    ...
    async function openOceanPay() {
      // Si ya est vinculado, solo mostrar mensaje o panel de usuario
      const state = await loadState();
      if (state.oceanPayLinked) {
        showAlert('info', 'Ya vinculado', 'Tu cuenta Ocean Pay est activa.');
        return;
      }
    ...
    async function submitOceanPayLogin() {
      ...
      try {
        ...
        if (res.ok && data.success) {
          ...
          if (err) err.textContent = data.error || 'Credenciales invlidas. Verifica usuario y contrasea.';
          if (navigator.vibrate) navigator.vibrate(200);
        }
      } catch (e) {
        console.error(e);
        if (err) err.textContent = 'Error de conexin con el servidor Ocean Pay.';
      }
    ...
    function skipOceanPay() {
      ...
      showAlert('info', 'Modo Local', 'Puedes vincular tu cuenta ms tarde en Ajustes.');
    }

    /* =========================
       CERRAR SESIN Y ELIMINAR CUENTA
       ========================= */
    async function unlinkOceanPay() {
      const confirmed = confirm('Ests seguro de que deseas cerrar sesin de Ocean Pay?\n\nTus Ecoxionums se mantendrn en tu cuenta de Ocean Pay.');
      ...
      try {
        ...
        // Mantener el username pero limpiar vinculacin
        ...
        showAlert('success', 'Sesin cerrada', 'Has cerrado sesin de Ocean Pay correctamente.');

        // Recargar configuracin para mostrar cambios
        renderTab('settings');
      } catch (error) {
        console.error('Error al cerrar sesin:', error);
        showAlert('error', 'Error', 'Hubo un problema al cerrar sesin. Intenta de nuevo.');
      }
    }
    ...
    window.toggleOceanPayMode = function () {
      ...
      if (isOpRegisterMode) {
        ...
        if (toggleMsg) toggleMsg.textContent = '?Ya tienes cuenta? Inicia sesin';
      } else {
        if (title) title.textContent = '? Ocean Pay Access';
        if (btn) btn.textContent = '? Iniciar Sesi?n';
        if (toggleMsg) toggleMsg.textContent = '?No tienes cuenta? Regstrate aqu';
      }
    };
    ...
    window.submitOceanPayAuth = async function () {
      ...
      try {
        ...
        if (res.ok && data.success) {
          ...
          if (currentLocal > 0) {
            try {
              ...
              if (syncData.success) {
                ...
                showAlert('success', 'Sincronizado', `Se han aadido ${currentLocal} Ecoxionums a tu cuenta Cloud.`);
              } else {
                ...
                const state = await loadState();
                state.ecoxionums = data.ecoxionums || 0;
                showAlert('warning', 'Sincronizacin parcial', 'No se pudo sincronizar el saldo local. Contacta soporte.');
              }
            } catch (e) {
          ...
          if (err) err.textContent = data.error || 'Error en la autenticacin.';
          if (navigator.vibrate) navigator.vibrate(200);
        }
      } catch (e) {
        console.error(e);
        if (err) err.textContent = 'Error de conexin con el servidor.';
      }
    ...
    async function initializeApp() {
      ...
      if (opUser.id) {
        // Ya est vinculado, actualizar estado
        ...
        // Mostrar modal despus de un pequeo delay solo si no se salt antes
        setTimeout(() => {
      ...
      if (!document.getElementById('ocean-pay-modal').classList.contains('hidden')) {
        return; // No mostrar alerta si el modal est abierto
      }
    ...
    window.calcEquals = function () {
      ...
      try {
        // Evaluar la expresin de forma segura
        ...
        if (window.pomodoroState.timeLeft <= 0) {
          ...
          if (!window.pomodoroState.isBreak) {
            ...
            if (workTime >= 20) {
              ...
              showAlert('success', 'Sesin completada!', 'Buen trabajo. (Sin recompensa por ser < 20 min)');
            }
            ...
            // Termin descanso
            pomodoroReset();
            showAlert('info', 'Descanso terminado', 'Listo para otra sesin?');
            if (navigator.vibrate) navigator.vibrate(200);
          }
        }
      }, 1000);
    ...
    function loadGallery() {
      ...
      grid.innerHTML = images.map((img, index) => `
        <div class="gallery-item" style="position:relative;border-radius:12px;overflow:hidden;border:1px solid rgba(255,255,255,.12);background:var(--bg-1);transition:all .3s;cursor:pointer;" onmouseenter="this.style.transform='translateY(-4px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,.3)'" onmouseleave="this.style.transform='translateY(0)';this.style.boxShadow='none'">
          ...
          <button onclick="event.stopPropagation();deleteGalleryImage(${index})" style="position:absolute;top:8px;right:8px;background:var(--danger);color:white;border:none;padding:6px 10px;border-radius:6px;cursor:pointer;font-weight:600;font-size:12px;box-shadow:0 2px 8px rgba(0,0,0,.3);">?</button>
        </div>
      `).join('');
    }
    ...
    window.addImageToGallery = function () {
      ...
      loadGallery();
      showAlert('success', 'Imagen agregada', 'La imagen se ha agregado a tu galera.');
    };

    window.deleteGalleryImage = function (index) {
      if (!confirm('Eliminar esta imagen de la galera?')) return;
      ...
      loadGallery();
      showAlert('info', 'Imagen eliminada', 'La imagen se ha eliminado de tu galera.');
    };
    ...
    function loadSurveys() {
      const surveys = [
        { id: 1, title: "Qu te parece Ecoxion?", reward: 10, questions: ["Cmo calificaras tu experiencia?", "Qu mejoraras?"] },
        { id: 2, title: "Preferencias de extensiones", reward: 15, questions: ["Qu tipo de extensiones prefieres?", "Cunto pagaras por una extensin premium?"] },
        { id: 3, title: "Uso de Ecoxionums", reward: 12, questions: ["En qu gastas tus Ecoxionums?", "Te gustara ms formas de ganarlos?"] }
      ];
    ...
    window.completeSurvey = async function (surveyId, reward) {
      ...
      showAlert('success', '?Encuesta completada!', `Has ganado ${reward} Ecoxionums. Gracias por tu opinin! ?`);
      loadSurveys();
    };
    ...
    function updateWatchAdsPanel() {
      ...
      if (btn) {
        ...
        if (watchedToday >= 10) {
          btn.textContent = '? Lmite diario alcanzado';
          btn.style.opacity = '0.5';
        }
      }
    }
    ...
    window.watchAd = async function () {
      ...
      if (state.adsData.watchedToday >= 10) {
        showAlert('warning', 'Lmite alcanzado', 'Has alcanzado el lmite diario de 10 anuncios.');
        return;
      }
    ...
    async function initEcoStock() {
      ...
      let html = '';
      companies.forEach(c => {
        // Fluctuacin aleatoria simple para demo visual
    ...
    window.tradeStock = async function (symbol, amount, price) {
      ...
      if (amount > 0) {
        ...
        showAlert('success', 'Compra exitosa', `Has comprado 1 accin de ${symbol} por ${cost} ?`);
      } else {
        // SELL
        ...
        showAlert('success', 'Venta exitosa', `Has vendido 1 accin de ${symbol} por ${gain} ?`);
      }
    ...
    const triviaQuestions = [
      { category: "Ciencia", question: "Cul es el planeta ms grande del sistema solar?", answers: ["Jpiter", "Saturno", "Neptuno", "Urano"], correct: 0 },
      { category: "Historia", question: "En qu ao cay el Muro de Berln?", answers: ["1989", "1991", "1985", "1990"], correct: 0 },
      { category: "Geografa", question: "Cul es la capital de Australia?", answers: ["Sdney", "Melbourne", "Canberra", "Perth"], correct: 2 },
      { category: "Tecnologa", question: "Quin fund Microsoft?", answers: ["Steve Jobs", "Bill Gates", "Elon Musk", "Mark Zuckerberg"], correct: 1 },
      { category: "Naturaleza", question: "Cul es el animal ms rpido del mundo?", answers: ["Len", "Guepardo", "Halcn peregrino", "guila"], correct: 2 },
      { category: "Arte", question: "Quin pint La Mona Lisa?", answers: ["Miguel ngel", "Rafael", "Leonardo da Vinci", "Botticelli"], correct: 2 },
      { category: "Deportes", question: "En qu pas se celebraron los Juegos Olmpicos de 2016?", answers: ["China", "Brasil", "Reino Unido", "Japn"], correct: 1 },
      { category: "Msica", question: "Cuntas cuerdas tiene un ukelele estndar?", answers: ["4", "6", "5", "8"], correct: 0 }
    ];
          ...
          <div style="padding:16px;background:var(--bg-1);border-radius:12px;border:1px solid ${completedToday ? 'var(--ok)' : 'rgba(255,255,255,.08)'};">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              ...
              <div style="display:flex;gap:8px;">
                ...
                <button onclick="deleteHabit(${i})" class="btn" style="padding:8px 12px;">?</button>
              </div>
            </div>
            <div style="margin-top:8px;font-size:12px;color:var(--ink-1);">Racha: ${h.streak || 0} das</div>
          </div>
        `;
      }).join('');
    }
    ...
    window.addHabit = function () {
      ...
      if (!name) return showAlert('warning', 'Nombre requerido', 'Ingresa un nombre para el h?H?bito');
      ...
      loadHabits();
      showAlert('success', 'H?H?bito creado!', `${icon} ${name} agregado`);
    };
    ...
    window.completeHabit = async function (index) {
      ...
      await awardEcoxionums(reward, `H?H?bito: ${habits[index].name}`);
      loadHabits();
      showAlert('success', 'H?H?bito completado!', `+${reward} Ecoxionums por tu racha de ${habits[index].streak} das`);
    };
    ...
    window.searchWeather = async function () {
      ...
      try {
        ...
        const weatherIcons = { 'Sunny': '?', 'Clear': '?', 'Partly cloudy': '?', 'Cloudy': '?', 'Overcast': '?', 'Rain': '?', 'Heavy rain': '?', 'Snow': '?', 'Mist': '?', 'Fog': '?' };
    ...
    function updateMinerUI() {
      ...
      if (btnEl) btnEl.textContent = minerState.active ? '? Detener Minera' : '? Iniciar Minera';
    }
    ...
    window.toggleMiner = async function () {
      ...
      if (minerState.active) {
        ...
        showAlert('info', 'Minero iniciado', 'Empezars a ganar Ecoxionums pasivamente');
      } else {
        clearInterval(minerState.interval);
        showAlert('info', 'Minero detenido', `Ganaste ${minerState.session} Ecoxionums esta sesin`);
      }
      updateMinerUI();
    };
      ...
      container.innerHTML = `
        <div style="text-align:center;padding:20px;background:var(--bg-1);border-radius:12px;">
          ...
          <div style="margin-top:12px;">
            <span style="color:var(--ink-1);">Puntuacin: </span>
    ...
    async function renderChatMessages() {
      ...
      if (chatMessages.length === 0) {
        container.innerHTML = '<div style="text-align:center; padding:20px; color:var(--ink-2); font-size:13px;">No hay mensajes. S el primero en saludar!</div>';
        return;
      }
    ...
    window.sendChatMessage = async function () {
      ...
      try {
        ...
        showAlert('error', 'Error', 'Problema de conexin con el servidor.');
      } finally {
        input.disabled = false;
        input.focus();
      }
    ...
    function saveArcadeScore(game, score, isTime = false) {
      ...
      if (isNewRecord) {
        ...
        updateArcadeHighScores();
        showAlert('success', '? NEW HIGH SCORE!', `Nuevo rcord en ${game}: ${score}`);
        // Small XP reward for breaking record
        awardEcoxionums(10, `Rcord Arcade: ${game}`);
      }
    }
    ...
    function renderCalendarEvents() {
      ...
      if (upcoming.length === 0) {
        container.innerHTML = '<div style="text-align:center;padding:20px;color:var(--ink-1);font-style:italic;">No hay eventos prximos en la agenda.</div>';
        return;
      }
    ...
    window.addCalendarEvent = function () {
      ...
      if (!title || !date) return showAlert('warning', 'Campos requeridos', 'Ingresa ttulo y fecha');
